...
generate_prolog_backends :-
    ...
    write(S, '        )\n'),
    write(S, '    ).\n\n'),
    %% SSE streaming for API backends (OpenAI/Claude/OpenRouter format)
    write(S, '%% SSE streaming for API backends\n'),
    ...
    write(S, '        close(In)),\n'),
    write(S, '    Response = _{content: Content, tool_calls: ToolCalls}.\n\n'),
...
generate_prolog_agent_loop :-
    ...
    write(S, '    ;\n'),
    write(S, '        AsstMsg = _{role: "assistant", content: Content},\n'),

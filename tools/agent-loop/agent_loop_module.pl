...
alias_category('Other', ["fmt", "str", "$", "find", "grep", "cc"]).
...
slash_command('clear-cache', exact, [comment('Clear tool result cache'),
    help_display('/clear-cache')],
    'Flush the tool result cache').
...
generate_parity_report :-
    open('generated/PARITY_REPORT.md', write, S),
    write(S, '# Cross-Target Parity Report\n\n'),
    write(S, '_Auto-generated by `generate_parity_report/0`. Do not edit manually._\n\n'),
    %% --- shared_logic coverage ---
    write(S, '## shared_logic Methods\n\n'),
    write(S, '| Method | Module | Python | Rust | Elixir | Prolog | Clojure |\n'),
    write(S, '|--------|--------|--------|------|--------|--------|--------|\n'),
    forall(shared_logic(Cat, Method, _Props), (
        (compile_logic(python, Method, _) -> Py = 'Y' ; Py = '-'),
        (compile_logic(rust, Method, _) -> Rs = 'Y' ; Rs = '-'),
        (compile_logic(elixir, Method, _) -> Ex = 'Y' ; Ex = '-'),
        (compile_logic(prolog, Method, _) -> Pl = 'Y' ; Pl = '-'),
        (compile_logic(clojure, Method, _) -> Cl = 'Y' ; Cl = '-'),
        format(S, '| `~w` | ~w | ~w | ~w | ~w | ~w | ~w |~n', [Method, Cat, Py, Rs, Ex, Pl, Cl])
    )),
    %% --- summary counts ---
    findall(M, shared_logic(_, M, _), AllMethods),
    length(AllMethods, Total),
    include([M]>>compile_logic(python, M, _), AllMethods, PyOk), length(PyOk, PyN),
    include([M]>>compile_logic(rust, M, _), AllMethods, RsOk), length(RsOk, RsN),
    include([M]>>compile_logic(elixir, M, _), AllMethods, ExOk), length(ExOk, ExN),
    include([M]>>compile_logic(prolog, M, _), AllMethods, PlOk), length(PlOk, PlN),
    include([M]>>compile_logic(clojure, M, _), AllMethods, ClOk), length(ClOk, ClN),
    format(S, '\n**Total: ~w methods** — Python: ~w, Rust: ~w, Elixir: ~w, Prolog: ~w, Clojure: ~w\n\n', [Total, PyN, RsN, ExN, PlN, ClN]),
    %% --- generated module inventory ---
    write(S, '## Generated Module Inventory\n\n'),
    write(S, '| Target | Modules | Tests |\n'),
    write(S, '|--------|---------|-------|\n'),
    (expand_file_name('generated/python/*.py', PyFiles) -> length(PyFiles, PyFC) ; PyFC = 0),
    (expand_file_name('generated/rust/src/*.rs', RsFiles) -> length(RsFiles, RsFC) ; RsFC = 0),
    (expand_file_name('generated/elixir/lib/agent_loop/*.ex', ExFiles) -> length(ExFiles, ExFC) ; ExFC = 0),
    ...
    write(S, '|-----------|-------|------------|\n'),
    forall(elixir_server(Srv, Wrapped), (
        format(S, '| `AgentLoop.~w` | `AgentLoop.~w` | Y |~n', [Srv, Wrapped])
    )),
    %% --- coverage gaps ---
    write(S, '\n## Coverage Gaps\n\n'),
    write(S, '| Category | Gap | Notes |\n'),
    write(S, '|----------|-----|-------|\n'),
    write(S, '| Context trimming | Not in shared_logic | Too complex (multi-condition loops, platform-specific optimizations) |\n'),
    write(S, '| Session save/load | Not in shared_logic | JSON serialization differs per language |\n'),
    write(S, '| Tool dispatch | Not in shared_logic | Branching + I/O differs per language |\n'),
    write(S, '| Elixir CI | No mix compile/test | Requires Elixir toolchain in CI |\n'),
    close(S),
    format('  Generated PARITY_REPORT.md (generated/PARITY_REPORT.md)~n', []).
...
write_decl_shared_logic_tests(S) :-
    ...
    ;
        forall(member(M, Methods), (
            ...
            format(atom(L6), 'shared_logic(~w) compiles for clojure', [M]),
            format(S, '    decl_assert_true(\'~w\', agent_loop_module:compile_logic(clojure, ~w, _)),~n', [L6, M])
        )),
        write(S, '    true.\n\n')
    ).
...
expand_expr(prolog, none_val, "none").

%% --- Path operations ---
expand_expr(python, path_join(Dir, File), S) :-
    expand_expr(python, Dir, DS),
    expand_expr(python, File, FS),
    format(atom(S), "os.path.join(~w, ~w)", [DS, FS]).
expand_expr(rust, path_join(Dir, File), S) :-
    expand_expr(rust, Dir, DS),
    expand_expr(rust, File, FS),
    format(atom(S), "~w.join(~w)", [DS, FS]).
expand_expr(elixir, path_join(Dir, File), S) :-
    expand_expr(elixir, Dir, DS),
    expand_expr(elixir, File, FS),
    format(atom(S), "Path.join(~w, ~w)", [DS, FS]).
expand_expr(prolog, path_join(Dir, File), S) :-
    expand_expr(prolog, Dir, DS),
    expand_expr(prolog, File, FS),
    format(atom(S), "directory_file_path(~w, ~w, Path)", [DS, FS]).

expand_expr(python, path_exists(Path), S) :-
    expand_expr(python, Path, PS),
    format(atom(S), "os.path.exists(~w)", [PS]).
expand_expr(rust, path_exists(Path), S) :-
    expand_expr(rust, Path, PS),
    format(atom(S), "~w.exists()", [PS]).
expand_expr(elixir, path_exists(Path), S) :-
    expand_expr(elixir, Path, PS),
    format(atom(S), "File.exists?(~w)", [PS]).
expand_expr(prolog, path_exists(Path), S) :-
    expand_expr(prolog, Path, PS),
    format(atom(S), "exists_file(~w)", [PS]).

%% --- String equality ---
expand_expr(python, eq_str(A, B), S) :-
    expand_expr(python, A, AS),
    expand_expr(python, B, BS),
    format(atom(S), "~w == \"~w\"", [AS, BS]).
expand_expr(rust, eq_str(A, B), S) :-
    expand_expr(rust, A, AS),
    expand_expr(rust, B, BS),
    format(atom(S), "~w == \"~w\"", [AS, BS]).
expand_expr(elixir, eq_str(A, B), S) :-
    expand_expr(elixir, A, AS),
    expand_expr(elixir, B, BS),
    format(atom(S), "~w == \"~w\"", [AS, BS]).
expand_expr(prolog, eq_str(A, B), S) :-
    expand_expr(prolog, A, AS),
...
%% --- Prolog emitter ---
%%
%% Prolog methods are predicates. State is a SWI-Prolog dict passed as first arg.
%% Return values are unified with a Result variable (last arg).
%% container(X) where X \= none → predicate with State dict
%% container(none) → standalone predicate
%% Mutable → State in, State1 out
%%
emit_shared_method(S, prolog, MethodName) :-
    shared_logic(_, MethodName, Props),
    member(signature(Name, Args, RetType), Props),
    member(doc(Doc), Props),
    ...
    %% Apply field_map rewrites if present
    atom_string(Body, BodyStr0),
    (member(field_map(prolog, Map), Props) ->
        apply_field_map(BodyStr0, Map, BodyStr1)
    ;
        BodyStr1 = BodyStr0
    ),
...
atomics_to_text([X|Xs], Sep, Result) :-
    ...
    format(atom(Result), "~w~w~w", [X, Sep, Rest]).

%% Write shared block for Prolog target
write_shared_block_prolog(S, Module) :-
    write_shared_block(S, prolog, Module).

write_shared_block_clojure(S, Module) :-
    write_shared_block(S, clojure, Module).
...
%% --- SessionManager: path + exists ---
shared_logic(sessions, session_path, [
    signature(session_path, [session_id], string),
    arg_types([string]),
    doc("Build the filesystem path for a session file."),
    container('SessionManager'),
    body_template("~return_val(path_join(self_direct(sessions_dir), format_str(\"{}.json\", [session_id])))~")
]).

shared_logic(sessions, session_exists, [
    signature(session_exists, [session_id], bool),
    arg_types([string]),
    doc("Check if a session file exists on disk."),
    container('SessionManager'),
    body_template("~return_val(path_exists(path_join(self_direct(sessions_dir), format_str(\"{}.json\", [session_id]))))~")
]).

%% --- Tool safety checks ---
shared_logic(tools, is_tool_destructive, [
    signature(is_destructive, [tool_name], bool),
    arg_types([string]),
    doc("Check if a tool name is in the destructive set (bash, write, edit)."),
    container(none),
    body_template("~return_val(matches_str_set(tool_name, [bash, write, edit]))~")
]).

shared_logic(tools, is_tool_safe, [
    signature(is_safe, [tool_name], bool),
    arg_types([string]),
...
%% --- String set membership (quoted) ---
logic_slot(python, matches_str_set(X, List), S) :-
    maplist([Item, Quoted]>>format(atom(Quoted), "\"~w\"", [Item]), List, QList),
    atomic_list_concat(QList, ', ', Items),
    format(atom(S), "~w in {~w}", [X, Items]).
logic_slot(rust, matches_str_set(X, List), S) :-
    maplist([Item, Quoted]>>format(atom(Quoted), "\"~w\"", [Item]), List, QList),
    atomic_list_concat(QList, ' | ', Items),
    format(atom(S), "matches!(~w.as_str(), ~w)", [X, Items]).
logic_slot(elixir, matches_str_set(X, List), S) :-
    maplist([Item, Quoted]>>format(atom(Quoted), "\"~w\"", [Item]), List, QList),
    atomic_list_concat(QList, ', ', Items),
    format(atom(S), "~w in [~w]", [X, Items]).
logic_slot(prolog, matches_str_set(X, List), S) :-
    maplist([Item, Quoted]>>format(atom(Quoted), "\"~w\"", [Item]), List, QList),
    atomic_list_concat(QList, ', ', Items),
    format(atom(S), "memberchk(~w, [~w])", [X, Items]).
...
%% =============================================================================
%%
%% Prolog design: state is a SWI-Prolog dict (state{field: value, ...}).
%% Results are returned via unification with a Result variable.
%% Mutation creates a new dict via put_dict/4.
%% container('CostTracker') → predicate with State dict as first arg
%% container(none) → standalone predicate

%% --- Print/IO slots ---
logic_slot(prolog, print_flush(X), S) :-
    format(atom(S), "write(~w), flush_output", [X]).

%% --- String/collection slots ---
logic_slot(prolog, str_len(X), S) :- format(atom(S), "atom_length(~w, Len)", [X]).

logic_slot(prolog, contains(Collection, Item), S) :-
    expand_expr(prolog, Collection, CS),
    expand_expr(prolog, Item, IS),
    format(atom(S), "memberchk(~w, ~w)", [IS, CS]).

logic_slot(prolog, not_empty(X), S) :-
    expand_expr(prolog, X, XS), format(atom(S), "~w \\= []", [XS]).

%% Direct struct field access via dict dot notation
logic_slot(prolog, self_direct(F), S) :- format(atom(S), "State.~w", [F]).

%% --- Assignment/mutation slots (Prolog: put_dict) ---
logic_slot(prolog, self_inc(Field, Expr), S) :-
    expand_expr(prolog, Expr, ES),
    format(atom(S), "NewVal is State.~w + ~w, put_dict(~w, State, NewVal, State1)", [Field, ES, Field]).
logic_slot(prolog, self_assign(Field, Expr), S) :-
    expand_expr(prolog, Expr, ES),
...
logic_slot(prolog, self_map_put(Field, Key, Value), S) :-
    ...
    expand_expr(prolog, Value, VS),
    format(atom(S), "put_dict(~w, State.~w, ~w, NewMap), put_dict(~w, State, NewMap, State1)", [KS, Field, VS, Field]).
...
expand_expr(prolog, self_total_cost >= budget, "State.total_cost >= Budget").
expand_expr(prolog, max_zero(Expr), S) :-
...
expand_expr(prolog, max_one(Expr), S) :-
    expand_expr(prolog, Expr, Inner),
    format(atom(S), "max(1, ~w)", [Inner]).
expand_expr(prolog, min_val(A, B), S) :-
    expand_expr(prolog, A, AStr),
    expand_expr(prolog, B, BStr),
    format(atom(S), "min(~w, ~w)", [AStr, BStr]).
expand_expr(prolog, mul(A, B), S) :-
    expand_expr(prolog, A, AStr),
    expand_expr(prolog, B, BStr),
    format(atom(S), "(~w * ~w)", [AStr, BStr]).
expand_expr(prolog, pow(Base, Exp), S) :-
    expand_expr(prolog, Base, BStr),
    expand_expr(prolog, Exp, EStr),
    format(atom(S), "(~w ** ~w)", [BStr, EStr]).
expand_expr(prolog, sub(A, B), S) :-
    expand_expr(prolog, A, AStr),
    expand_expr(prolog, B, BStr),
    format(atom(S), "(~w - ~w)", [AStr, BStr]).
expand_expr(prolog, as_float(X), S) :-
    expand_expr(prolog, X, Inner),
    format(atom(S), "float(~w)", [Inner]).
expand_expr(prolog, div_float(A, B), S) :-
    expand_expr(prolog, A, AStr),
    expand_expr(prolog, B, BStr),
    format(atom(S), "(~w / ~w)", [AStr, BStr]).

%% --- Map access ---
expand_expr(prolog, map_get(Collection, Key), S) :-
    expand_expr(prolog, Collection, CS),
    expand_expr(prolog, Key, KS),
...
resolve_type(prolog, int, "integer").
resolve_type(prolog, float, "float").
resolve_type(prolog, string, "atom").
resolve_type(prolog, bool, "boolean").
resolve_type(prolog, void, "true").
resolve_type(prolog, usize, "nonneg").
resolve_type(prolog, dict, "dict").
...
resolve_type(prolog, ref(T), S) :-
    resolve_type(prolog, T, S).

%% =============================================================================
%% Clojure target — resolve_type, expand_expr, logic_slot, emit_shared_method
%% =============================================================================

%% --- Clojure helper: snake_case to kebab-case ---
clojure_kebab(Atom, Kebab) :-
    atom_string(Atom, Str),
    split_string(Str, "_", "", Parts),
    atomics_to_text(Parts, "-", Kebab).

%% --- Clojure format args as space-separated kebab-case ---
clojure_format_args([], "").
clojure_format_args(Args, Result) :-
    Args \= [],
    maplist([A, KA]>>(clojure_kebab(A, KA)), Args, KArgs),
    atomics_to_text(KArgs, " ", Result).

%% --- resolve_type/3 for Clojure ---
resolve_type(clojure, int, "long").
resolve_type(clojure, float, "double").
resolve_type(clojure, string, "String").
resolve_type(clojure, bool, "Boolean").
resolve_type(clojure, void, "nil").
resolve_type(clojure, usize, "long").
resolve_type(clojure, dict, "IPersistentMap").
resolve_type(clojure, set_of_string, "#{String}").
...
resolve_type(clojure, ref(T), S) :-
    resolve_type(clojure, T, S).
...
logic_slot(clojure, return_val(Expr), S) :-
    Expr \= true, Expr \= false,
    expand_expr(clojure, Expr, ExprStr),
    S = ExprStr.
logic_slot(clojure, self_field(F), S) :-
    clojure_kebab(F, KF), format(atom(S), "(:~w state)", [KF]).
logic_slot(clojure, self_direct(F), S) :-
    clojure_kebab(F, KF), format(atom(S), "(:~w state)", [KF]).
logic_slot(clojure, self_method(M), S) :-
    clojure_kebab(M, KM), format(atom(S), "(~w state)", [KM]).
logic_slot(clojure, print_flush(X), S) :-
    format(atom(S), "(print ~w) (flush)", [X]).
logic_slot(clojure, str_len(X), S) :-
    clojure_kebab(X, KX), format(atom(S), "(count ~w)", [KX]).
logic_slot(clojure, contains(Collection, Item), S) :-
    expand_expr(clojure, Collection, CS),
    expand_expr(clojure, Item, IS),
    format(atom(S), "(contains? ~w ~w)", [CS, IS]).
logic_slot(clojure, not_empty(X), S) :-
    expand_expr(clojure, X, XS), format(atom(S), "(seq ~w)", [XS]).
logic_slot(clojure, self_inc(Field, Expr), S) :-
    clojure_kebab(Field, KF),
    expand_expr(clojure, Expr, ES),
    format(atom(S), "(update state :~w + ~w)", [KF, ES]).
logic_slot(clojure, self_assign(Field, Expr), S) :-
    clojure_kebab(Field, KF),
    expand_expr(clojure, Expr, ES),
    format(atom(S), "(assoc state :~w ~w)", [KF, ES]).
logic_slot(clojure, assign(Var, Expr), S) :-
    clojure_kebab(Var, KV),
    expand_expr(clojure, Expr, ES),
    format(atom(S), "(let [~w ~w]", [KV, ES]).
logic_slot(clojure, self_map_put(Field, Key, Value), S) :-
    clojure_kebab(Field, KF),
    expand_expr(clojure, Key, KS),
    expand_expr(clojure, Value, VS),
    format(atom(S), "(update state :~w assoc ~w ~w)", [KF, KS, VS]).
logic_slot(clojure, self_list_append(Field, Item), S) :-
    clojure_kebab(Field, KF),
    expand_expr(clojure, Item, IS),
...
logic_slot(clojure, call_method(Method, Args), S) :-
    format_args_list(Args, clojure, ArgStr),
    clojure_kebab(Method, KM),
    format(atom(S), "(~w ~w)", [KM, ArgStr]).
logic_slot(clojure, is_empty_check(X), S) :-
    expand_expr(clojure, X, XS),
    format(atom(S), "(empty? ~w)", [XS]).
...
logic_slot(clojure, str_starts_with(Str, Prefix), S) :-
    clojure_kebab(Str, KS), format(atom(S), '(.startsWith ~w "~w")', [KS, Prefix]).
...
logic_slot(clojure, json_decode(X), S) :-
    format(atom(S), "(cheshire.core/parse-string ~w true)", [X]).
logic_slot(clojure, file_write(Path, Content), S) :-
    format(atom(S), "(spit ~w ~w)", [Path, Content]).
logic_slot(clojure, file_read(Path), S) :-
    format(atom(S), "(slurp ~w)", [Path]).

%% --- expand_expr/3 for Clojure (prefix notation) ---

expand_expr(clojure, self_total_cost >= budget, "(>= (:total-cost state) budget)").
expand_expr(clojure, budget - self_total_cost, "(- budget (:total-cost state))").
expand_expr(clojure, none_val, "nil").

expand_expr(clojure, max_zero(Expr), S) :-
    expand_expr(clojure, Expr, Inner),
    format(atom(S), "(max 0.0 ~w)", [Inner]).
expand_expr(clojure, max_one(Expr), S) :-
    expand_expr(clojure, Expr, Inner),
    format(atom(S), "(max 1 ~w)", [Inner]).
expand_expr(clojure, len_of(Expr), S) :-
    expand_expr(clojure, Expr, Inner),
    format(atom(S), "(count ~w)", [Inner]).
expand_expr(clojure, int_div(A, B), S) :-
    expand_expr(clojure, A, AS), expand_expr(clojure, B, BS),
    format(atom(S), "(quot ~w ~w)", [AS, BS]).
expand_expr(clojure, min_val(A, B), S) :-
    expand_expr(clojure, A, AS), expand_expr(clojure, B, BS),
    format(atom(S), "(min ~w ~w)", [AS, BS]).
expand_expr(clojure, add(A, B), S) :-
    expand_expr(clojure, A, AS), expand_expr(clojure, B, BS),
    format(atom(S), "(+ ~w ~w)", [AS, BS]).
expand_expr(clojure, sub(A, B), S) :-
    expand_expr(clojure, A, AS), expand_expr(clojure, B, BS),
    format(atom(S), "(- ~w ~w)", [AS, BS]).
expand_expr(clojure, mul(A, B), S) :-
    expand_expr(clojure, A, AS), expand_expr(clojure, B, BS),
    format(atom(S), "(* ~w ~w)", [AS, BS]).
expand_expr(clojure, div_float(A, B), S) :-
    expand_expr(clojure, A, AS), expand_expr(clojure, B, BS),
    format(atom(S), "(/ ~w ~w)", [AS, BS]).
expand_expr(clojure, pow(Base, Exp), S) :-
    expand_expr(clojure, Base, BS), expand_expr(clojure, Exp, ES),
    format(atom(S), "(Math/pow ~w ~w)", [BS, ES]).
expand_expr(clojure, as_float(X), S) :-
    expand_expr(clojure, X, Inner),
    format(atom(S), "(double ~w)", [Inner]).
expand_expr(clojure, gt(A, B), S) :-
    expand_expr(clojure, A, AS), expand_expr(clojure, B, BS),
    format(atom(S), "(> ~w ~w)", [AS, BS]).
...
expand_expr(clojure, path_exists(Path), S) :-
    expand_expr(clojure, Path, PS),
    format(atom(S), "(.exists (java.io.File. ~w))", [PS]).
...
expand_expr(clojure, str_ends_with(Str, Suffix), S) :-
    expand_expr(clojure, Str, SS),
    format(atom(S), '(.endsWith ~w "~w")', [SS, Suffix]).
expand_expr(clojure, all_keys_present(Dict, Keys), S) :-
    expand_expr(clojure, Dict, DS), expand_expr(clojure, Keys, KS),
    format(atom(S), "(every? #(contains? ~w %) ~w)", [DS, KS]).
expand_expr(clojure, literal_list(Items), S) :-
    format_literal_list(clojure, Items, Inner),
    format(atom(S), "[~w]", [Inner]).
format_literal_item(clojure, X, S) :- format(atom(S), '"~w"', [X]).

%% --- Clojure fixup: convert if/return to (if ...) ---
clojure_fixup_body(In, Out) :-
    split_string(In, "\n", "", Lines),
    clojure_fixup_lines(Lines, FixedLines),
    atomics_to_text(FixedLines, "\n", Out).
...
clojure_fixup_lines([Line|Rest], Result) :-
    ...
    ->
       ...
       ;
           maplist(strip_indent, ThenLines, StrippedThen),
           clojure_fixup_lines(StrippedThen, FixedThen),
           format(atom(IfOpen), "(if ~w", [Cond]),
           indent_lines(FixedThen, IndentedThen),
           (AfterThen = [] ->
               append([IfOpen|IndentedThen], [")"], Result)
           ;
               clojure_fixup_lines(AfterThen, FixedElse),
               indent_lines(FixedElse, IndentedElse),
               append([IfOpen|IndentedThen], IndentedElse, Mid),
               append(Mid, [")"], Result)
           )
       )
    ;
...
emit_shared_method(S, clojure, MethodName) :-
    ...
    %% Docstring
    format(S, '(defn ~w~n', [KName]),
    format(S, '  "~w"~n', [Doc]),
    %% Arg vector
    (Container = none ->
        clojure_format_args(Args, ArgsVec),
        format(S, '  [~w]~n', [ArgsVec])
    ;
        clojure_format_args(Args, ArgsVec),
        (ArgsVec = "" ->
            write(S, '  [state]\n')
        ;
            format(S, '  [state ~w]~n', [ArgsVec])
        )
    ),
    %% Apply field_map rewrites
    atom_string(Body, BodyStr0),
    (member(field_map(clojure, Map), Props) ->
        apply_field_map(BodyStr0, Map, BodyStr1)
    ;
        BodyStr1 = BodyStr0
    ),
    %% Apply Clojure fixup
    clojure_fixup_body(BodyStr1, BodyStr),
    %% Emit body lines
    split_string(BodyStr, "\n", "", Lines),
    forall(member(Line, Lines), (
        format(S, '  ~w~n', [Line])
...
expand_expr(elixir, len_of(Expr), S) :-
    expand_expr(elixir, Expr, Inner),
    format(atom(S), "Enum.count(~w)", [Inner]).
...
generate_rust_cargo_toml :-
    ...
    write(S, '[profile.release]\n'),
    write(S, 'opt-level = 3\n'),
    write(S, 'lto = true\n'),
    write(S, 'codegen-units = 1\n'),
    write(S, 'strip = true\n'),
    write(S, 'panic = "abort"\n'),
    close(S),
...
generate_rust_makefile :-
    output_path(rust, '', SrcDir),
    atom_concat(SrcDir, '../Makefile', Path),
    open(Path, write, S),
    write(S, '# UnifyWeaver Agent Loop — Rust build automation\n'),
    write(S, '# Auto-generated by agent_loop_module.pl\n\n'),
    write(S, 'BINARY_NAME := uwsal\n'),
    write(S, 'INSTALL_DIR := $(HOME)/.local/bin\n\n'),
    write(S, '.PHONY: build release test check clean install uninstall dist wasm wasm-pack help\n\n'),
    write(S, 'help: ## Show this help\n'),
    write(S, '\t@grep -E ''^[a-zA-Z_-]+:.*?## .*$$'' $(MAKEFILE_LIST) | sort | awk ''BEGIN {FS = ":.*?## "}; {printf "  \\033[36m%-15s\\033[0m %s\\n", $$1, $$2}''\n\n'),
    write(S, 'build: ## Build debug binary\n'),
    write(S, '\tcargo build\n\n'),
    write(S, 'release: ## Build optimized release binary (LTO + strip)\n'),
    write(S, '\tcargo build --release\n'),
    write(S, '\t@echo "Binary: target/release/$(BINARY_NAME)"\n'),
    write(S, '\t@ls -lh target/release/$(BINARY_NAME) 2>/dev/null || true\n\n'),
    write(S, 'test: ## Run all tests\n'),
    write(S, '\tcargo test\n\n'),
    write(S, 'check: ## Type-check without building\n'),
    write(S, '\tcargo check\n\n'),
    write(S, 'clean: ## Remove build artifacts\n'),
    write(S, '\tcargo clean\n\n'),
    write(S, 'install: release ## Install binary to ~/.local/bin\n'),
    write(S, '\t@mkdir -p $(INSTALL_DIR)\n'),
    write(S, '\tcp target/release/$(BINARY_NAME) $(INSTALL_DIR)/$(BINARY_NAME)\n'),
    write(S, '\t@echo "Installed $(BINARY_NAME) to $(INSTALL_DIR)/$(BINARY_NAME)"\n'),
    write(S, '\t@echo "Make sure $(INSTALL_DIR) is in your PATH"\n\n'),
    write(S, 'uninstall: ## Remove installed binary\n'),
    write(S, '\trm -f $(INSTALL_DIR)/$(BINARY_NAME)\n'),
    write(S, '\t@echo "Removed $(INSTALL_DIR)/$(BINARY_NAME)"\n\n'),
    write(S, 'dist: release ## Create distribution archive\n'),
    write(S, '\t@mkdir -p dist\n'),
    write(S, '\tcp target/release/$(BINARY_NAME) dist/\n'),
    write(S, '\t@cd dist && tar czf $(BINARY_NAME)-$(shell uname -m)-$(shell uname -s | tr A-Z a-z).tar.gz $(BINARY_NAME)\n'),
    write(S, '\t@echo "Archive: dist/$(BINARY_NAME)-$(shell uname -m)-$(shell uname -s | tr A-Z a-z).tar.gz"\n\n'),
    write(S, 'wasm: ## Build WASM library (requires wasm32-unknown-unknown target)\n'),
    write(S, '\tcargo build --target wasm32-unknown-unknown --features wasm --release\n'),
    write(S, '\t@echo "WASM: target/wasm32-unknown-unknown/release/agent_loop.wasm"\n\n'),
    write(S, 'wasm-pack: ## Build WASM with wasm-pack for web usage\n'),
    write(S, '\twasm-pack build --target web --features wasm\n'),
    write(S, '\t@echo "WASM pack output: pkg/"\n'),
    close(S),
    format('  Generated rust/Makefile~n', []).
...
generate_all(elixir) :-
    write('Generating Elixir target...'), nl,
    output_path(elixir, '', ElixirDir),
    make_directory_path(ElixirDir),
    generate_elixir_mix_exs,
    generate_elixir_costs,
    generate_elixir_streaming,
    generate_elixir_tool_cache,
    generate_elixir_context,
    generate_elixir_mcp_client,
    generate_elixir_output_parser,
    generate_elixir_sessions,
    generate_elixir_retry,
    %% Data layer
...
%% write_elixir_header(+Stream, +Module, +Description)
write_elixir_header(S, Module, Description) :-
    format(S, '# ~w — ~w~n', [Module, Description]),
    format(S, '# Auto-generated by UnifyWeaver agent_loop_module.pl~n', []),
    format(S, '# Do not edit manually.~n~n', []).
...
%% --- mix.exs ---
generate_elixir_mix_exs :-
    target_dir(elixir, LibDir),
    atom_concat(LibDir, '../../mix.exs', Path),
    open(Path, write, S),
    write(S, 'defmodule AgentLoop.MixProject do\n'),
    write(S, '  use Mix.Project\n\n'),
    write(S, '  def project do\n'),
    write(S, '    [\n'),
    write(S, '      app: :agent_loop,\n'),
    write(S, '      version: "0.1.0",\n'),
    write(S, '      elixir: "~> 1.14",\n'),
    write(S, '      start_permanent: Mix.env() == :prod,\n'),
    write(S, '      deps: deps()\n'),
    write(S, '    ]\n'),
    write(S, '  end\n\n'),
    ...
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/mix.exs~n', []).

%% --- CostTracker ---
generate_elixir_costs :-
    output_path(elixir, 'cost_tracker.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.CostTracker', 'Model pricing and cost tracking'),
    write(S, 'defmodule AgentLoop.CostTracker do\n'),
    write(S, '  @moduledoc "Tracks API usage costs across model calls."\n\n'),
    write(S, '  defstruct [\n'),
    write(S, '    total_input_tokens: 0,\n'),
    write(S, '    total_output_tokens: 0,\n'),
    write(S, '    total_cost: 0.0,\n'),
    write(S, '    records: []\n'),
    write(S, '  ]\n\n'),
    write(S, '  @type t :: %__MODULE__{\n'),
    write(S, '    total_input_tokens: non_neg_integer(),\n'),
    write(S, '    total_output_tokens: non_neg_integer(),\n'),
    write(S, '    total_cost: float(),\n'),
    write(S, '    records: [map()]\n'),
    write(S, '  }\n\n'),
    %% Emit shared_logic methods
    write_shared_block_elixir(S, costs),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/lib/agent_loop/cost_tracker.ex~n', []).

%% --- StreamingTokenCounter ---
generate_elixir_streaming :-
    output_path(elixir, 'streaming_token_counter.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.StreamingTokenCounter', 'Token counting for streamed responses'),
    write(S, 'defmodule AgentLoop.StreamingTokenCounter do\n'),
    write(S, '  @moduledoc "Counts tokens and characters during streaming output."\n\n'),
    write(S, '  defstruct [\n'),
    write(S, '    token_count: 0,\n'),
    write(S, '    char_count: 0,\n'),
    write(S, '    show_live: false\n'),
    write(S, '  ]\n\n'),
    write(S, '  @type t :: %__MODULE__{\n'),
    write(S, '    token_count: non_neg_integer(),\n'),
    write(S, '    char_count: non_neg_integer(),\n'),
    write(S, '    show_live: boolean()\n'),
    write(S, '  }\n\n'),
    write_shared_block_elixir(S, streaming),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/lib/agent_loop/streaming_token_counter.ex~n', []).

%% --- ToolResultCache ---
generate_elixir_tool_cache :-
    output_path(elixir, 'tool_result_cache.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.ToolResultCache', 'Cache for tool execution results'),
    write(S, 'defmodule AgentLoop.ToolResultCache do\n'),
    write(S, '  @moduledoc "Caches tool results to avoid redundant execution."\n\n'),
    write(S, '  defstruct [\n'),
    write(S, '    cache: %{},\n'),
    write(S, '    ttl: 300_000,\n'),
    write(S, '    skip_tools: MapSet.new()\n'),
    write(S, '  ]\n\n'),
    write(S, '  @type t :: %__MODULE__{\n'),
    write(S, '    cache: map(),\n'),
    write(S, '    ttl: non_neg_integer(),\n'),
    write(S, '    skip_tools: MapSet.t()\n'),
    write(S, '  }\n\n'),
    write_shared_block_elixir(S, tool_cache),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/lib/agent_loop/tool_result_cache.ex~n', []).

%% --- ContextManager ---
generate_elixir_context :-
    output_path(elixir, 'context_manager.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.ContextManager', 'Conversation context management'),
    write(S, 'defmodule AgentLoop.ContextManager do\n'),
    write(S, '  @moduledoc "Manages conversation message history and context window."\n\n'),
    write(S, '  defstruct [\n'),
    write(S, '    messages: [],\n'),
    write(S, '    max_messages: 100,\n'),
    write(S, '    max_context_tokens: -1,\n'),
    write(S, '    max_chars: -1,\n'),
    write(S, '    max_words: -1,\n'),
    write(S, '    context_mode: "sliding"\n'),
    write(S, '  ]\n\n'),
    write(S, '  @type t :: %__MODULE__{\n'),
    write(S, '    messages: [map()],\n'),
    write(S, '    max_messages: non_neg_integer(),\n'),
    write(S, '    max_context_tokens: integer(),\n'),
    write(S, '    max_chars: integer(),\n'),
    write(S, '    max_words: integer(),\n'),
    write(S, '    context_mode: String.t()\n'),
    write(S, '  }\n\n'),
    ...
    write(S, '    end)\n'),
    write(S, '  end\n\n'),
    write_shared_block_elixir(S, context),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/lib/agent_loop/context_manager.ex~n', []).

%% --- MCPClient ---
generate_elixir_mcp_client :-
    output_path(elixir, 'mcp_client.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.MCPClient', 'MCP client — stdio JSON-RPC 2.0 transport'),
    write(S, 'defmodule AgentLoop.MCPClient do\n'),
    write(S, '  @moduledoc "JSON-RPC 2.0 client for Model Context Protocol servers."\n\n'),
    write(S, '  defstruct [\n'),
    write(S, '    name: "",\n'),
    write(S, '    command: [],\n'),
    write(S, '    env: %{},\n'),
    write(S, '    port: nil,\n'),
    write(S, '    request_id: 0\n'),
    write(S, '  ]\n\n'),
    write(S, '  @type t :: %__MODULE__{\n'),
    write(S, '    name: String.t(),\n'),
    write(S, '    command: [String.t()],\n'),
    write(S, '    env: map(),\n'),
    write(S, '    port: port() | nil,\n'),
    write(S, '    request_id: non_neg_integer()\n'),
    write(S, '  }\n\n'),
    write_shared_block_elixir(S, mcp),
    %% --- Hand-guided MCP protocol methods ---
    write(S, '  @doc "Connect to MCP server via stdio subprocess"\n'),
    write(S, '  @spec connect(t()) :: {:ok, t()} | {:error, String.t()}\n'),
    write(S, '  def connect(%__MODULE__{} = client) do\n'),
    write(S, '    port = Port.open({:spawn_executable, hd(client.command)},\n'),
    write(S, '      [:binary, :exit_status, {:args, tl(client.command)},\n'),
    write(S, '       {:env, Enum.map(client.env, fn {k, v} -> {String.to_charlist(k), String.to_charlist(v)} end)}])\n'),
    write(S, '    client = %{client | port: port}\n'),
    write(S, '    case send_request(client, "initialize", %{}) do\n'),
    write(S, '      {:ok, _resp, client} -> {:ok, client}\n'),
    write(S, '      {:error, reason} -> {:error, reason}\n'),
    write(S, '    end\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Send a JSON-RPC 2.0 request"\n'),
    write(S, '  @spec send_request(t(), String.t(), map()) :: {:ok, map(), t()} | {:error, String.t()}\n'),
    write(S, '  def send_request(%__MODULE__{} = client, method, params) do\n'),
    write(S, '    id = client.request_id + 1\n'),
    write(S, '    client = %{client | request_id: id}\n'),
    ...
    write(S, '          _ -> {:error, "Invalid JSON-RPC response"}\n'),
    write(S, '        end\n'),
    write(S, '    after\n'),
    write(S, '      5_000 -> {:error, "MCP request timeout"}\n'),
    write(S, '    end\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Discover available tools from MCP server"\n'),
    write(S, '  @spec discover_tools(t()) :: {:ok, [map()], t()} | {:error, String.t()}\n'),
    write(S, '  def discover_tools(%__MODULE__{} = client) do\n'),
    write(S, '    case send_request(client, "tools/list", %{}) do\n'),
    write(S, '      {:ok, %{"tools" => tools}, client} -> {:ok, tools, client}\n'),
    write(S, '      {:ok, _, client} -> {:ok, [], client}\n'),
    write(S, '      {:error, reason} -> {:error, reason}\n'),
    write(S, '    end\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Call a tool on the MCP server"\n'),
    write(S, '  @spec call_tool(t(), String.t(), map()) :: {:ok, map(), t()} | {:error, String.t()}\n'),
    write(S, '  def call_tool(%__MODULE__{} = client, tool_name, arguments) do\n'),
    write(S, '    send_request(client, "tools/call", %{"name" => tool_name, "arguments" => arguments})\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Disconnect from MCP server"\n'),
    write(S, '  @spec disconnect(t()) :: :ok\n'),
    write(S, '  def disconnect(%__MODULE__{} = client) do\n'),
    write(S, '    if client.port, do: Port.close(client.port)\n'),
    write(S, '    :ok\n'),
    write(S, '  end\n'),
    write(S, 'end\n\n'),
    %% --- MCPManager module ---
    write(S, 'defmodule AgentLoop.MCPManager do\n'),
    write(S, '  @moduledoc "Manages multiple MCP server connections and routes tool calls."\n\n'),
    write(S, '  defstruct servers: %{}, tools: %{}\n\n'),
    write(S, '  @type t :: %__MODULE__{servers: map(), tools: map()}\n\n'),
    write(S, '  @doc "Add and connect to an MCP server"\n'),
    write(S, '  @spec add_server(t(), String.t(), [String.t()], map()) :: {:ok, t()} | {:error, String.t()}\n'),
    write(S, '  def add_server(%__MODULE__{} = mgr, name, command, env \\\\ %{}) do\n'),
    write(S, '    client = %AgentLoop.MCPClient{name: name, command: command, env: env}\n'),
    write(S, '    case AgentLoop.MCPClient.connect(client) do\n'),
    write(S, '      {:ok, client} ->\n'),
    write(S, '        case AgentLoop.MCPClient.discover_tools(client) do\n'),
    write(S, '          {:ok, tools, client} ->\n'),
    write(S, '            tool_map = Map.new(tools, fn t -> {t["name"], name} end)\n'),
    write(S, '            mgr = %{mgr | servers: Map.put(mgr.servers, name, client),\n'),
    write(S, '                          tools: Map.merge(mgr.tools, tool_map)}\n'),
    write(S, '            {:ok, mgr}\n'),
    write(S, '          {:error, reason} -> {:error, reason}\n'),
    write(S, '        end\n'),
    write(S, '      {:error, reason} -> {:error, reason}\n'),
    ...
    write(S, '  @spec list_tools(t()) :: [String.t()]\n'),
    write(S, '  def list_tools(%__MODULE__{} = mgr), do: Map.keys(mgr.tools)\n\n'),
    write(S, '  @doc "Dispatch a tool call to the appropriate server"\n'),
    write(S, '  @spec dispatch(t(), String.t(), map()) :: {:ok, map(), t()} | {:error, String.t()}\n'),
    write(S, '  def dispatch(%__MODULE__{} = mgr, tool_name, arguments) do\n'),
    write(S, '    case Map.get(mgr.tools, tool_name) do\n'),
    write(S, '      nil -> {:error, "Unknown tool: #{tool_name}"}\n'),
    write(S, '      server_name ->\n'),
    write(S, '        client = Map.get(mgr.servers, server_name)\n'),
    write(S, '        case AgentLoop.MCPClient.call_tool(client, tool_name, arguments) do\n'),
    write(S, '          {:ok, result, client} ->\n'),
    write(S, '            mgr = %{mgr | servers: Map.put(mgr.servers, server_name, client)}\n'),
    write(S, '            {:ok, result, mgr}\n'),
    write(S, '          {:error, reason} -> {:error, reason}\n'),
    write(S, '        end\n'),
    write(S, '    end\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Disconnect all servers"\n'),
    write(S, '  @spec disconnect_all(t()) :: :ok\n'),
    write(S, '  def disconnect_all(%__MODULE__{} = mgr) do\n'),
    write(S, '    Enum.each(mgr.servers, fn {_name, client} ->\n'),
    write(S, '      AgentLoop.MCPClient.disconnect(client)\n'),
    write(S, '    end)\n'),
    write(S, '    :ok\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/lib/agent_loop/mcp_client.ex~n', []).

%% --- OutputParser ---
generate_elixir_output_parser :-
    output_path(elixir, 'output_parser.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.OutputParser', 'Structured output parser — extract JSON from model responses'),
    write(S, 'defmodule AgentLoop.OutputParser do\n'),
    write(S, '  @moduledoc "Extracts structured JSON blocks from model text output."\n\n'),
    write(S, '  defstruct []\n\n'),
    write(S, '  @type t :: %__MODULE__{}\n\n'),
    write_shared_block_elixir(S, output_parser),
    %% --- Hand-guided helper methods ---
    write(S, '  @doc "Extract JSON from fenced code blocks (```json ... ```)"\n'),
    write(S, '  @spec extract_fenced(String.t()) :: [map()]\n'),
    write(S, '  def extract_fenced(text) do\n'),
    write(S, '    ~r/```(?:json)?\\s*\\n(.*?)```/s\n'),
    write(S, '    |> Regex.scan(text)\n'),
    write(S, '    |> Enum.flat_map(fn [_, body] ->\n'),
    write(S, '      case Jason.decode(body) do\n'),
    write(S, '        {:ok, parsed} when is_map(parsed) -> [parsed]\n'),
    write(S, '        _ -> []\n'),
    write(S, '      end\n'),
    write(S, '    end)\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Extract bare JSON objects from text using bracket depth tracking"\n'),
    write(S, '  @spec extract_bare(String.t()) :: [map()]\n'),
    write(S, '  def extract_bare(text) do\n'),
    write(S, '    text\n'),
    write(S, '    |> String.graphemes()\n'),
    write(S, '    |> Enum.reduce({0, "", []}, fn\n'),
    write(S, '      "{", {0, _acc, results} -> {1, "{", results}\n'),
    write(S, '      "{", {depth, acc, results} -> {depth + 1, acc <> "{", results}\n'),
    write(S, '      "}", {1, acc, results} ->\n'),
    write(S, '        candidate = acc <> "}"\n'),
    write(S, '        case Jason.decode(candidate) do\n'),
    write(S, '          {:ok, parsed} when is_map(parsed) -> {0, "", results ++ [parsed]}\n'),
    write(S, '          _ -> {0, "", results}\n'),
    write(S, '        end\n'),
    write(S, '      "}", {depth, acc, results} when depth > 1 -> {depth - 1, acc <> "}", results}\n'),
    write(S, '      char, {depth, acc, results} when depth > 0 -> {depth, acc <> char, results}\n'),
    write(S, '      _, state -> state\n'),
    write(S, '    end)\n'),
    write(S, '    |> elem(2)\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Parse model response text and extract structured JSON blocks"\n'),
    write(S, '  @spec parse_response(String.t(), [atom()]) :: {[map()], String.t(), [String.t()]}\n'),
    write(S, '  def parse_response(text, _expected_keys \\\\ []) do\n'),
    write(S, '    fenced = extract_fenced(text)\n'),
    write(S, '    blocks = if fenced != [], do: fenced, else: extract_bare(text)\n'),
    write(S, '    {blocks, text, []}\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/lib/agent_loop/output_parser.ex~n', []).

%% --- Sessions ---
generate_elixir_sessions :-
    output_path(elixir, 'sessions.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.Sessions', 'Session persistence — save/load/list/delete conversation sessions'),
    write(S, 'defmodule AgentLoop.Sessions do\n'),
    write(S, '  @moduledoc "Session persistence for agent conversations."\n\n'),
    write(S, '  defstruct sessions_dir: "sessions"\n\n'),
    write(S, '  @type t :: %__MODULE__{sessions_dir: String.t()}\n\n'),
    ...
    write(S, '    data = %{\n'),
    write(S, '      "id" => session_id,\n'),
    write(S, '      "name" => name,\n'),
    write(S, '      "messages" => messages,\n'),
    write(S, '      "saved_at" => DateTime.utc_now() |> DateTime.to_iso8601()\n'),
    write(S, '    }\n'),
    write(S, '    File.write!(path, Jason.encode!(data, pretty: true))\n'),
    write(S, '    :ok\n'),
    write(S, '  end\n\n'),
    ...
    write(S, '      {:error, reason} -> {:error, "Failed to load session: #{inspect(reason)}"}\n'),
    write(S, '    end\n'),
    write(S, '  end\n\n'),
    ...
    write(S, '      {:ok, files} ->\n'),
    write(S, '        files\n'),
    write(S, '        |> Enum.filter(&String.ends_with?(&1, ".json"))\n'),
    write(S, '        |> Enum.map(fn file ->\n'),
    write(S, '          path = Path.join(dir, file)\n'),
    write(S, '          case File.read(path) do\n'),
    write(S, '            {:ok, content} ->\n'),
    write(S, '              case Jason.decode(content) do\n'),
    write(S, '                {:ok, data} -> Map.take(data, ["id", "name", "saved_at"])\n'),
    write(S, '                _ -> nil\n'),
    write(S, '              end\n'),
    write(S, '            _ -> nil\n'),
    write(S, '          end\n'),
    write(S, '        end)\n'),
    write(S, '        |> Enum.reject(&is_nil/1)\n'),
    write(S, '      {:error, _} -> []\n'),
    write(S, '    end\n'),
    write(S, '  end\n\n'),
    ...
    write(S, '      :ok -> :ok\n'),
    write(S, '      {:error, reason} -> {:error, "Failed to delete session: #{inspect(reason)}"}\n'),
    write(S, '    end\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/lib/agent_loop/sessions.ex~n', []).

%% --- Retry ---
generate_elixir_retry :-
    output_path(elixir, 'retry.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.Retry', 'Retry logic — exponential backoff and retryable status detection'),
    write(S, 'defmodule AgentLoop.Retry do\n'),
    write(S, '  @moduledoc "Exponential backoff and HTTP status retry logic."\n\n'),
    write_shared_block_elixir(S, retry),
    write(S, 'end\n'),
    close(S),
...
%% --- Pricing module attribute table ---
generate_elixir_pricing :-
    output_path(elixir, 'pricing.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.Pricing', 'Model pricing table'),
    write(S, 'defmodule AgentLoop.Pricing do\n'),
    write(S, '  @moduledoc "Model pricing data — cost per million tokens (input/output)."\n\n'),
    write(S, '  @type entry :: {String.t(), float(), float()}\n\n'),
    write(S, '  @pricing [\n'),
    forall(model_pricing(Model, Input, Output), (
        format(S, '    {"~w", ~w, ~w},~n', [Model, Input, Output])
    )),
    write(S, '  ]\n\n'),
    write(S, '  @doc "Return the full pricing table."\n'),
    write(S, '  @spec all() :: [entry()]\n'),
    write(S, '  def all, do: @pricing\n\n'),
    write(S, '  @doc "Look up pricing for a model name. Returns {input, output} or :unknown."\n'),
    write(S, '  @spec lookup(String.t()) :: {float(), float()} | :unknown\n'),
    write(S, '  def lookup(model) do\n'),
    write(S, '    case Enum.find(@pricing, fn {name, _, _} -> name == model end) do\n'),
    write(S, '      {_, input, output} -> {input, output}\n'),
    write(S, '      nil -> :unknown\n'),
    write(S, '    end\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/lib/agent_loop/pricing.ex~n', []).

%% --- Tool specs module ---
generate_elixir_tools :-
    output_path(elixir, 'tools.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.Tools', 'Tool specifications'),
    write(S, 'defmodule AgentLoop.Tools do\n'),
    write(S, '  @moduledoc "Tool specification registry — parameters, confirmation, timeouts."\n\n'),
    write(S, '  @type param :: %{name: atom(), type: atom(), required: boolean(), description: String.t()}\n'),
    write(S, '  @type spec :: %{name: atom(), description: String.t(), params: [param()], confirm: boolean(), timeout: non_neg_integer()}\n\n'),
    write(S, '  @tools [\n'),
    forall(tool_spec(Name, Props), (
        member(description(Desc), Props),
        (member(parameters(Params), Props) -> true ; Params = []),
        (member(confirmation_required(Confirm), Props) -> true ; Confirm = false),
        (member(timeout(Timeout), Props) -> true ; Timeout = 120),
        format(S, '    %{name: :~w, description: "~w", confirm: ~w, timeout: ~w, params: [', [Name, Desc, Confirm, Timeout]),
        forall(member(param(PName, PType, PReq, PDesc), Params), (
            (PReq == required -> IsReq = true ; IsReq = false),
            format(S, '%{name: :~w, type: :~w, required: ~w, description: "~w"}, ', [PName, PType, IsReq, PDesc])
        )),
        write(S, ']},\n')
    )),
    write(S, '  ]\n\n'),
    %% Destructive tools
    write(S, '  @destructive_tools MapSet.new([\n'),
    forall(destructive_tool(DT), (
        format(S, '    "~w",~n', [DT])
    )),
    write(S, '  ])\n\n'),
    write(S, '  @doc "Return all tool specifications."\n'),
    write(S, '  @spec all() :: [spec()]\n'),
    write(S, '  def all, do: @tools\n\n'),
    write(S, '  @doc "Look up a tool spec by name."\n'),
    write(S, '  @spec lookup(atom()) :: spec() | nil\n'),
    write(S, '  def lookup(name) do\n'),
    write(S, '    Enum.find(@tools, fn t -> t.name == name end)\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Check if a tool is destructive."\n'),
    write(S, '  @spec destructive?(String.t()) :: boolean()\n'),
    write(S, '  def destructive?(name), do: MapSet.member?(@destructive_tools, name)\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/lib/agent_loop/tools.ex~n', []).

%% --- Backend config module ---
generate_elixir_backends :-
    output_path(elixir, 'backends.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.Backends', 'Backend configuration registry'),
    write(S, 'defmodule AgentLoop.Backends do\n'),
    write(S, '  @moduledoc "Agent backend configuration — CLI and API backends."\n\n'),
    write(S, '  @type backend :: %{\n'),
    write(S, '    name: atom(),\n'),
    write(S, '    type: atom(),\n'),
    write(S, '    class_name: String.t(),\n'),
    write(S, '    command: String.t(),\n'),
    write(S, '    description: String.t(),\n'),
    write(S, '    supports_streaming: boolean()\n'),
    write(S, '  }\n\n'),
    write(S, '  @backends [\n'),
    forall(agent_backend(Name, Props), (
        ...
        (member(default_model(DefModel0), Props) ->
            format(atom(DefModel), '"~w"', [DefModel0])
        ;
            DefModel = nil
        ),
        format(S, '    %{name: :~w, type: :~w, class_name: "~w", command: "~w", description: "~w", supports_streaming: ~w, default_model: ~w},~n',
            [Name, Type, ClassName, Cmd, Desc, Stream, DefModel])
    )),
    write(S, '  ]\n\n'),
    write(S, '  @doc "Return all backend configurations."\n'),
    write(S, '  @spec all() :: [backend()]\n'),
    write(S, '  def all, do: @backends\n\n'),
    write(S, '  @doc "Look up a backend by name."\n'),
    write(S, '  @spec lookup(atom()) :: backend() | nil\n'),
    write(S, '  def lookup(name) do\n'),
    write(S, '    Enum.find(@backends, fn b -> b.name == name end)\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/lib/agent_loop/backends.ex~n', []).

%% =============================================================================
%% Elixir GenServer Wrappers
%% =============================================================================

%% --- CostTracker GenServer ---
generate_elixir_cost_server :-
    output_path(elixir, 'cost_server.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.CostServer', 'GenServer wrapper for CostTracker'),
    write(S, 'defmodule AgentLoop.CostServer do\n'),
    write(S, '  @moduledoc """\n'),
    write(S, '  GenServer wrapping AgentLoop.CostTracker for OTP integration.\n'),
    write(S, '  Manages cost tracking state across concurrent API calls.\n'),
    write(S, '  """\n\n'),
    write(S, '  use GenServer\n\n'),
    write(S, '  alias AgentLoop.CostTracker\n\n'),
    write(S, '  # --- Client API ---\n\n'),
    write(S, '  def start_link(opts \\\\ []) do\n'),
    write(S, '    GenServer.start_link(__MODULE__, %CostTracker{}, opts)\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Check if total cost exceeds budget."\n'),
    write(S, '  def over_budget?(server, budget) do\n'),
    write(S, '    GenServer.call(server, {:is_over_budget, budget})\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Return remaining budget."\n'),
    write(S, '  def budget_remaining(server, budget) do\n'),
    write(S, '    GenServer.call(server, {:budget_remaining, budget})\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Reset all cost tracking state."\n'),
    ...
    write(S, '  @impl true\n'),
    write(S, '  def handle_call({:is_over_budget, budget}, _from, state) do\n'),
    write(S, '    {:reply, CostTracker.is_over_budget(state, budget), state}\n'),
    write(S, '  end\n\n'),
    write(S, '  @impl true\n'),
    write(S, '  def handle_call({:budget_remaining, budget}, _from, state) do\n'),
    write(S, '    {:reply, CostTracker.budget_remaining(state, budget), state}\n'),
    write(S, '  end\n\n'),
    write(S, '  @impl true\n'),
    write(S, '  def handle_call(:reset, _from, _state) do\n'),
    write(S, '    new_state = CostTracker.reset(%CostTracker{})\n'),
    ...
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/lib/agent_loop/cost_server.ex~n', []).

%% --- ContextManager GenServer ---
generate_elixir_context_server :-
    output_path(elixir, 'context_server.ex', Path),
    open(Path, write, S),
    write_elixir_header(S, 'AgentLoop.ContextServer', 'GenServer wrapper for ContextManager'),
    write(S, 'defmodule AgentLoop.ContextServer do\n'),
    write(S, '  @moduledoc """\n'),
    write(S, '  GenServer wrapping AgentLoop.ContextManager for OTP integration.\n'),
    write(S, '  Manages conversation context state with message history.\n'),
    write(S, '  """\n\n'),
    write(S, '  use GenServer\n\n'),
    write(S, '  alias AgentLoop.ContextManager\n\n'),
    write(S, '  # --- Client API ---\n\n'),
    write(S, '  def start_link(opts \\\\ []) do\n'),
    write(S, '    GenServer.start_link(__MODULE__, %ContextManager{}, opts)\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Add a message to the context."\n'),
    write(S, '  def add_message(server, message) do\n'),
    write(S, '    GenServer.call(server, {:add_message, message})\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Clear all messages."\n'),
    write(S, '  def clear(server) do\n'),
    write(S, '    GenServer.call(server, :clear)\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Return number of messages."\n'),
    write(S, '  def len(server) do\n'),
    write(S, '    GenServer.call(server, :len)\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Check if context is empty."\n'),
    write(S, '  def empty?(server) do\n'),
    write(S, '    GenServer.call(server, :is_empty)\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Estimate token count."\n'),
    write(S, '  def estimate_tokens(server) do\n'),
    write(S, '    GenServer.call(server, :estimate_tokens)\n'),
    write(S, '  end\n\n'),
    write(S, '  @doc "Get current state snapshot."\n'),
    write(S, '  def get_state(server) do\n'),
    write(S, '    GenServer.call(server, :get_state)\n'),
    ...
    write(S, '  def init(state), do: {:ok, state}\n\n'),
    write(S, '  @impl true\n'),
    write(S, '  def handle_call({:add_message, message}, _from, state) do\n'),
    write(S, '    new_state = %{state | messages: state.messages ++ [message]}\n'),
    write(S, '    {:reply, :ok, new_state}\n'),
    write(S, '  end\n\n'),
    write(S, '  @impl true\n'),
    write(S, '  def handle_call(:clear, _from, _state) do\n'),
    write(S, '    {:reply, :ok, ContextManager.clear(%ContextManager{})}\n'),
    write(S, '  end\n\n'),
    write(S, '  @impl true\n'),
    write(S, '  def handle_call(:len, _from, state) do\n'),
    write(S, '    {:reply, ContextManager.len(state), state}\n'),
    write(S, '  end\n\n'),
    write(S, '  @impl true\n'),
    write(S, '  def handle_call(:is_empty, _from, state) do\n'),
    write(S, '    {:reply, ContextManager.is_empty(state), state}\n'),
    write(S, '  end\n\n'),
    write(S, '  @impl true\n'),
    write(S, '  def handle_call(:estimate_tokens, _from, state) do\n'),
    write(S, '    {:reply, ContextManager.estimate_tokens(state), state}\n'),
    write(S, '  end\n\n'),
    write(S, '  @impl true\n'),
    write(S, '  def handle_call(:get_state, _from, state) do\n'),
    write(S, '    {:reply, state, state}\n'),
...
generate_elixir_tests :-
    target_dir(elixir, LibDir),
    atom_concat(LibDir, '../../test/', TestDir),
    make_directory_path(TestDir),
    generate_elixir_test_helper(TestDir),
    generate_elixir_cost_test(TestDir),
    generate_elixir_context_test(TestDir),
    generate_elixir_streaming_test(TestDir),
...
generate_elixir_test_helper(Dir) :-
    ...
    open(Path, write, S),
    write(S, 'ExUnit.start()\n'),
    close(S),
    format('  Generated elixir/test/test_helper.exs~n', []).

%% --- CostTracker tests ---
generate_elixir_cost_test(Dir) :-
    atom_concat(Dir, 'cost_tracker_test.exs', Path),
    open(Path, write, S),
    write(S, 'defmodule AgentLoop.CostTrackerTest do\n'),
    write(S, '  use ExUnit.Case, async: true\n\n'),
    write(S, '  alias AgentLoop.CostTracker\n\n'),
    write(S, '  test "new struct has zero costs" do\n'),
    write(S, '    state = %CostTracker{}\n'),
    write(S, '    assert state.total_cost == 0.0\n'),
    write(S, '    assert state.total_input_tokens == 0\n'),
    write(S, '    assert state.total_output_tokens == 0\n'),
    write(S, '    assert state.records == []\n'),
    write(S, '  end\n\n'),
    write(S, '  test "is_over_budget returns false for zero budget (unlimited)" do\n'),
    write(S, '    state = %CostTracker{}\n'),
    write(S, '    refute CostTracker.is_over_budget(state, 0)\n'),
    write(S, '  end\n\n'),
    write(S, '  test "is_over_budget returns false when under budget" do\n'),
    write(S, '    state = %CostTracker{total_cost: 5.0}\n'),
    write(S, '    refute CostTracker.is_over_budget(state, 10.0)\n'),
    write(S, '  end\n\n'),
    write(S, '  test "is_over_budget returns true when over budget" do\n'),
    write(S, '    state = %CostTracker{total_cost: 15.0}\n'),
    write(S, '    assert CostTracker.is_over_budget(state, 10.0)\n'),
    write(S, '  end\n\n'),
    write(S, '  test "budget_remaining returns -1 for unlimited" do\n'),
    write(S, '    state = %CostTracker{}\n'),
    write(S, '    assert CostTracker.budget_remaining(state, 0) == -1.0\n'),
    write(S, '  end\n\n'),
    write(S, '  test "budget_remaining calculates correctly" do\n'),
    write(S, '    state = %CostTracker{total_cost: 3.0}\n'),
    write(S, '    assert CostTracker.budget_remaining(state, 10.0) == 7.0\n'),
    write(S, '  end\n\n'),
    write(S, '  test "reset clears all fields" do\n'),
    write(S, '    state = %CostTracker{total_cost: 5.0, total_input_tokens: 100, records: [%{}]}\n'),
    write(S, '    reset = CostTracker.reset(state)\n'),
    write(S, '    assert reset.total_cost == 0.0\n'),
    write(S, '    assert reset.total_input_tokens == 0\n'),
    write(S, '    assert reset.total_output_tokens == 0\n'),
    write(S, '    assert reset.records == []\n'),
    write(S, '  end\n\n'),
    write(S, '  test "cost_compute calculates per-million pricing" do\n'),
    write(S, '    # 1000 tokens at $15/million = $0.015\n'),
    write(S, '    cost = CostTracker.cost_compute(1000, 15.0)\n'),
    write(S, '    assert_in_delta cost, 0.015, 0.0001\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/test/cost_tracker_test.exs~n', []).

%% --- ContextManager tests ---
generate_elixir_context_test(Dir) :-
    atom_concat(Dir, 'context_manager_test.exs', Path),
    open(Path, write, S),
    write(S, 'defmodule AgentLoop.ContextManagerTest do\n'),
    write(S, '  use ExUnit.Case, async: true\n\n'),
    write(S, '  alias AgentLoop.ContextManager\n\n'),
    write(S, '  test "new context is empty" do\n'),
    write(S, '    state = %ContextManager{}\n'),
    write(S, '    assert ContextManager.is_empty(state)\n'),
    write(S, '    assert ContextManager.len(state) == 0\n'),
    write(S, '  end\n\n'),
    write(S, '  test "clear resets messages" do\n'),
    write(S, '    state = %ContextManager{messages: [%{"role" => "user", "content" => "hi"}]}\n'),
    write(S, '    cleared = ContextManager.clear(state)\n'),
    write(S, '    assert cleared.messages == []\n'),
    write(S, '    assert ContextManager.is_empty(cleared)\n'),
    write(S, '  end\n\n'),
    write(S, '  test "len counts messages" do\n'),
    write(S, '    state = %ContextManager{messages: [%{}, %{}, %{}]}\n'),
    write(S, '    assert ContextManager.len(state) == 3\n'),
    write(S, '  end\n\n'),
    write(S, '  test "estimate_tokens uses char/4 heuristic" do\n'),
    write(S, '    msg = %{"content" => String.duplicate("a", 400)}\n'),
    write(S, '    state = %ContextManager{messages: [msg]}\n'),
    write(S, '    assert ContextManager.estimate_tokens(state) == 100\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/test/context_manager_test.exs~n', []).

%% --- StreamingTokenCounter tests ---
generate_elixir_streaming_test(Dir) :-
    atom_concat(Dir, 'streaming_token_counter_test.exs', Path),
    open(Path, write, S),
    write(S, 'defmodule AgentLoop.StreamingTokenCounterTest do\n'),
    write(S, '  use ExUnit.Case, async: true\n\n'),
    write(S, '  alias AgentLoop.StreamingTokenCounter\n\n'),
    write(S, '  test "new counter starts at zero" do\n'),
    write(S, '    state = %StreamingTokenCounter{}\n'),
    write(S, '    assert state.token_count == 0\n'),
    write(S, '    assert state.char_count == 0\n'),
    write(S, '  end\n\n'),
    write(S, '  test "format_summary shows counts" do\n'),
    write(S, '    state = %StreamingTokenCounter{token_count: 42, char_count: 168}\n'),
    write(S, '    assert StreamingTokenCounter.format_summary(state) == "42 tokens, 168 chars"\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/test/streaming_token_counter_test.exs~n', []).

%% --- ToolResultCache tests ---
generate_elixir_tool_cache_test(Dir) :-
    atom_concat(Dir, 'tool_result_cache_test.exs', Path),
    open(Path, write, S),
    write(S, 'defmodule AgentLoop.ToolResultCacheTest do\n'),
    write(S, '  use ExUnit.Case, async: true\n\n'),
    write(S, '  alias AgentLoop.ToolResultCache\n\n'),
    write(S, '  test "new cache is empty" do\n'),
    write(S, '    state = %ToolResultCache{}\n'),
    write(S, '    assert Enum.count(state.cache) == 0\n'),
    write(S, '  end\n\n'),
    write(S, '  test "clear resets cache" do\n'),
    write(S, '    state = %ToolResultCache{cache: %{"key" => "val"}}\n'),
    write(S, '    cleared = ToolResultCache.clear(state)\n'),
    write(S, '    assert cleared.cache == %{}\n'),
    write(S, '  end\n\n'),
    write(S, '  test "len counts entries" do\n'),
    write(S, '    state = %ToolResultCache{cache: %{"a" => 1, "b" => 2}}\n'),
    write(S, '    assert ToolResultCache.len(state) == 2\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/test/tool_result_cache_test.exs~n', []).

%% --- Retry tests ---
generate_elixir_retry_test(Dir) :-
    atom_concat(Dir, 'retry_test.exs', Path),
    open(Path, write, S),
    write(S, 'defmodule AgentLoop.RetryTest do\n'),
    write(S, '  use ExUnit.Case, async: true\n\n'),
    write(S, '  alias AgentLoop.Retry\n\n'),
    write(S, '  test "is_retryable_status for 429" do\n'),
    write(S, '    assert Retry.is_retryable_status(429)\n'),
    write(S, '  end\n\n'),
    write(S, '  test "is_retryable_status for 500" do\n'),
    write(S, '    assert Retry.is_retryable_status(500)\n'),
    write(S, '  end\n\n'),
    write(S, '  test "is_retryable_status false for 200" do\n'),
    write(S, '    refute Retry.is_retryable_status(200)\n'),
    write(S, '  end\n\n'),
    write(S, '  test "is_retryable_status false for 404" do\n'),
    write(S, '    refute Retry.is_retryable_status(404)\n'),
    write(S, '  end\n\n'),
    write(S, '  test "compute_delay first attempt" do\n'),
    write(S, '    delay = Retry.compute_delay(1.0, 2.0, 1, 60.0)\n'),
    write(S, '    assert_in_delta delay, 1.0, 0.001\n'),
    write(S, '  end\n\n'),
    write(S, '  test "compute_delay exponential growth" do\n'),
    write(S, '    delay = Retry.compute_delay(1.0, 2.0, 4, 60.0)\n'),
    write(S, '    assert_in_delta delay, 8.0, 0.001\n'),
    write(S, '  end\n\n'),
    write(S, '  test "compute_delay caps at max_delay" do\n'),
    write(S, '    delay = Retry.compute_delay(1.0, 2.0, 10, 30.0)\n'),
    write(S, '    assert delay == 30.0\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/test/retry_test.exs~n', []).

%% --- Pricing tests ---
generate_elixir_pricing_test(Dir) :-
    atom_concat(Dir, 'pricing_test.exs', Path),
    open(Path, write, S),
    write(S, 'defmodule AgentLoop.PricingTest do\n'),
    write(S, '  use ExUnit.Case, async: true\n\n'),
    write(S, '  alias AgentLoop.Pricing\n\n'),
    write(S, '  test "all returns non-empty list" do\n'),
    write(S, '    assert length(Pricing.all()) > 0\n'),
    write(S, '  end\n\n'),
    write(S, '  test "lookup known model" do\n'),
    write(S, '    assert {15.0, 75.0} = Pricing.lookup("opus")\n'),
    write(S, '  end\n\n'),
    write(S, '  test "lookup unknown model" do\n'),
    write(S, '    assert :unknown = Pricing.lookup("nonexistent-model")\n'),
    write(S, '  end\n\n'),
    %% Count pricing entries from facts
    aggregate_all(count, model_pricing(_, _, _), PricingCount),
    format(S, '  test "pricing table has ~w entries" do~n', [PricingCount]),
    format(S, '    assert length(Pricing.all()) == ~w~n', [PricingCount]),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/test/pricing_test.exs~n', []).

%% --- Tools tests ---
generate_elixir_tools_test(Dir) :-
    atom_concat(Dir, 'tools_test.exs', Path),
    open(Path, write, S),
    write(S, 'defmodule AgentLoop.ToolsTest do\n'),
    write(S, '  use ExUnit.Case, async: true\n\n'),
    write(S, '  alias AgentLoop.Tools\n\n'),
    write(S, '  test "all returns non-empty list" do\n'),
    write(S, '    assert length(Tools.all()) > 0\n'),
    write(S, '  end\n\n'),
    write(S, '  test "lookup bash tool" do\n'),
    write(S, '    tool = Tools.lookup(:bash)\n'),
    write(S, '    assert tool != nil\n'),
    write(S, '    assert tool.name == :bash\n'),
    write(S, '    assert tool.confirm == true\n'),
    write(S, '  end\n\n'),
    write(S, '  test "lookup read tool is non-destructive" do\n'),
    write(S, '    tool = Tools.lookup(:read)\n'),
    write(S, '    assert tool != nil\n'),
    write(S, '    assert tool.confirm == false\n'),
    write(S, '  end\n\n'),
    write(S, '  test "destructive? checks destructive tools" do\n'),
    (destructive_tool(DT) ->
        format(S, '    assert Tools.destructive?("~w")~n', [DT])
    ; true),
    write(S, '    refute Tools.destructive?("read")\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
...
%% --- OutputParser tests ---
generate_elixir_output_parser_test(Dir) :-
    atom_concat(Dir, 'output_parser_test.exs', Path),
    open(Path, write, S),
    write(S, 'defmodule AgentLoop.OutputParserTest do\n'),
    write(S, '  use ExUnit.Case, async: true\n\n'),
    write(S, '  alias AgentLoop.OutputParser\n\n'),
    write(S, '  test "extract_fenced parses fenced JSON blocks" do\n'),
    write(S, '    text = "Some text\\n```json\\n{\\\"key\\\": \\\"value\\\"}\\n```\\nMore text"\n'),
    write(S, '    result = OutputParser.extract_fenced(text)\n'),
    write(S, '    assert [%{"key" => "value"}] = result\n'),
    write(S, '  end\n\n'),
    write(S, '  test "extract_bare parses bare JSON objects" do\n'),
    write(S, '    text = "Here is {\\\"a\\\": 1} inline"\n'),
    write(S, '    result = OutputParser.extract_bare(text)\n'),
    write(S, '    assert [%{"a" => 1}] = result\n'),
    write(S, '  end\n\n'),
    write(S, '  test "parse_response prefers fenced blocks" do\n'),
    write(S, '    text = "```json\\n{\\\"tool\\\": \\\"read\\\"}\\n```"\n'),
    write(S, '    {blocks, _raw, errors} = OutputParser.parse_response(text)\n'),
    write(S, '    assert length(blocks) == 1\n'),
    write(S, '    assert errors == []\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
    format('  Generated elixir/test/output_parser_test.exs~n', []).

%% --- Sessions tests ---
generate_elixir_sessions_test(Dir) :-
    atom_concat(Dir, 'sessions_test.exs', Path),
    open(Path, write, S),
    write(S, 'defmodule AgentLoop.SessionsTest do\n'),
    write(S, '  use ExUnit.Case, async: true\n\n'),
    write(S, '  alias AgentLoop.Sessions\n\n'),
    write(S, '  setup do\n'),
    write(S, '    dir = Path.join(System.tmp_dir!(), "agent_loop_test_sessions_#{:rand.uniform(999999)}")\n'),
    write(S, '    File.mkdir_p!(dir)\n'),
    write(S, '    on_exit(fn -> File.rm_rf!(dir) end)\n'),
    write(S, '    %{state: %Sessions{sessions_dir: dir}}\n'),
    write(S, '  end\n\n'),
    write(S, '  test "session_path builds correct path", %{state: state} do\n'),
    write(S, '    path = Sessions.session_path(state, "abc123")\n'),
    write(S, '    assert String.ends_with?(path, "abc123.json")\n'),
    write(S, '  end\n\n'),
    write(S, '  test "session_exists returns false for missing session", %{state: state} do\n'),
    write(S, '    refute Sessions.session_exists(state, "nonexistent")\n'),
    write(S, '  end\n\n'),
    write(S, '  test "save and load round-trip", %{state: state} do\n'),
    write(S, '    :ok = Sessions.save_session(state, "test1", "Test Session", [%{"role" => "user", "content" => "hello"}])\n'),
    write(S, '    assert Sessions.session_exists(state, "test1")\n'),
    write(S, '    {:ok, data} = Sessions.load_session(state, "test1")\n'),
    write(S, '    assert data["name"] == "Test Session"\n'),
    write(S, '    assert length(data["messages"]) == 1\n'),
    write(S, '  end\n\n'),
    write(S, '  test "list_sessions returns saved sessions", %{state: state} do\n'),
    write(S, '    :ok = Sessions.save_session(state, "s1", "Session 1", [])\n'),
    write(S, '    :ok = Sessions.save_session(state, "s2", "Session 2", [])\n'),
    write(S, '    sessions = Sessions.list_sessions(state)\n'),
    write(S, '    assert length(sessions) == 2\n'),
    write(S, '  end\n\n'),
    write(S, '  test "delete_session removes file", %{state: state} do\n'),
    write(S, '    :ok = Sessions.save_session(state, "del1", "To Delete", [])\n'),
    write(S, '    assert Sessions.session_exists(state, "del1")\n'),
    write(S, '    :ok = Sessions.delete_session(state, "del1")\n'),
    write(S, '    refute Sessions.session_exists(state, "del1")\n'),
    write(S, '  end\n'),
    write(S, 'end\n'),
    close(S),
...
generate_backends_init :-
    ...
    write(S, '# Auto-generated by agent_loop_module.pl\n'),
    write(S, 'from .base import AgentBackend, AgentResponse, ToolCall, AsyncApiBackend\n'),
...
generate_tools_module :-
    ...
    write(S, '\n\n'),
    %% ToolHandler class (header + dispatch dict + cache/mcp + plugins init + methods + body)
    ...
    write(S, '\n        # Tool result cache\n'),
    write(S, '        self.cache = ToolResultCache()\n\n'),
    write(S, '        # MCP manager (initialized later from config)\n'),
    write(S, '        self.mcp_manager = None\n\n'),
    write(S, '        self._init_plugins()\n'),
    write(S, '\n'),
    write_py(S, tools_handler_class_methods),
...
generate_alias_group_other(S) :-
    ...
    write(S, '\n'),
    write(S, '    # Cache\n'),
    command_alias("cc", CcV), format(S, '    "cc": "~w",~n', [CcV]).
...
%% --- Fragment: async_api_backend (Python parity with Rust AsyncApiBackend) ---

py_fragment(async_api_backend, '

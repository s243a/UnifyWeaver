...
generate_prolog_backends :-
    ...
    write(S, '            format("[Retry ~w/~w in ~2fs]~n", [Attempt, Max, ActualDelay]),\n'),
    ...
    write(S, '        close(In)),\n'),
    write(S, '    Response = _{content: Content, tool_calls: [], usage: _{input_tokens: 0, output_tokens: 0}}.\n\n'),
    ...
    write(S, '    (member(auth_prefix(AuthP), Spec) -> true ; AuthP = "Bearer "),\n'),
    write(S, '    atom_concat(AuthP, Key, AuthVal),\n'),
    write(S, '    Body = json(_{model: Model, messages: Messages, tools: Tools, stream: true}),\n'),
    write(S, '    setup_call_cleanup(\n'),
    write(S, '        http_open(URL, In, [\n'),
    write(S, '            method(post),\n'),
    write(S, '            request_header(AuthH=AuthVal),\n'),
    ...
    write(S, '            post(Body)\n'),
    write(S, '        ]),\n'),
    write(S, '        read_sse_stream(In, Content, ToolCalls),\n'),
    write(S, '        close(In)),\n'),
    write(S, '    Response = _{content: Content, tool_calls: ToolCalls, usage: _{input_tokens: 0, output_tokens: 0}}.\n\n'),
    ...
    write(S, '            AT = _{name: TN, description: TD, input_schema: _{type: "object", properties: _{}}}\n'),
    ...
    write(S, 'read_anthropic_sse_stream(In, TokAcc, TCAcc, InToks, OutToks, _CurTool,\n'),
    ...
    write(S, '                _, (Evt = null)),\n'),
    write(S, '            (Evt \\= null ->\n'),
    write(S, '                handle_anthropic_event(Evt, In, TokAcc, TCAcc, InToks, OutToks,\n'),
    write(S, '                                       Content, ToolCalls, Usage)\n'),
    write(S, '            ;\n'),
    ...
    write(S, '        %% Extract input token count from message.usage\n'),
    ...
    write(S, '        read_anthropic_sse_stream(In, TokAcc, TCAcc, NewInToks, OutToks, [],\n'),
    write(S, '                                  Content, ToolCalls, Usage)\n'),
    ...
    write(S, '            write(Token), flush_output,\n'),
    write(S, '            read_anthropic_sse_stream(In, [Token|TokAcc], TCAcc, InToks, OutToks, [],\n'),
    write(S, '                                      Content, ToolCalls, Usage)\n'),
    write(S, '        ;\n'),
    write(S, '            %% input_json_delta or other — skip\n'),
    write(S, '            read_anthropic_sse_stream(In, TokAcc, TCAcc, InToks, OutToks, [],\n'),
    write(S, '                                      Content, ToolCalls, Usage)\n'),
    write(S, '        )\n'),
    write(S, '    ; Type = "message_delta" ->\n'),
    write(S, '        %% Extract output token count\n'),
    write(S, '        (get_dict(usage, Evt, DUsage),\n'),
    write(S, '         get_dict(output_tokens, DUsage, NewOutToks) -> true\n'),
    write(S, '        ; NewOutToks = OutToks),\n'),
    write(S, '        read_anthropic_sse_stream(In, TokAcc, TCAcc, InToks, NewOutToks, [],\n'),
    write(S, '                                  Content, ToolCalls, Usage)\n'),
    ...
    write(S, '        Usage = _{input_tokens: InToks, output_tokens: OutToks}\n'),
    write(S, '    ;\n'),
    write(S, '        %% ping, content_block_start, content_block_stop, etc. — skip\n'),
    write(S, '        read_anthropic_sse_stream(In, TokAcc, TCAcc, InToks, OutToks, [],\n'),
    write(S, '                                  Content, ToolCalls, Usage)\n'),
    write(S, '    ).\n\n'),
    ...
    write(S, 'read_sse_stream(In, Content, ToolCalls) :-\n'),
    write(S, '    read_sse_stream(In, [], [], Content, ToolCalls).\n\n'),
    write(S, 'read_sse_stream(In, TokenAcc, TCAcc, Content, ToolCalls) :-\n'),
    write(S, '    (at_end_of_stream(In) ->\n'),
    ...
    write(S, '                        write(Token), flush_output,\n'),
    write(S, '                        read_sse_stream(In, [Token|TokenAcc], TCAcc, Content, ToolCalls)\n'),
    write(S, '                    ;\n'),
    write(S, '                        read_sse_stream(In, TokenAcc, TCAcc, Content, ToolCalls)\n'),
    write(S, '                    )\n'),
    write(S, '                ;\n'),
    ...
    write(S, '            )\n'),
    write(S, '        ;\n'),
    write(S, '            %% Skip non-data lines (empty lines, comments, event types)\n'),
    write(S, '            read_sse_stream(In, TokenAcc, TCAcc, Content, ToolCalls)\n'),
    write(S, '        )\n'),
    write(S, '    ).\n'),
    close(S),
...
generate_prolog_agent_loop :-
    ...
    write(S, '    length(Messages, Len),\n'),
    write(S, '    (Len > MaxMsgs ->\n'),
    write(S, '        Drop is Len - MaxMsgs,\n'),
    write(S, '        length(Prefix, Drop),\n'),
    write(S, '        append(Prefix, Trimmed0, Messages)\n'),
    write(S, '    ; Trimmed0 = Messages),\n'),
    write(S, '    context_max_tokens(MaxTokens),\n'),
    write(S, '    trim_by_tokens(Trimmed0, MaxTokens, Trimmed).\n\n'),

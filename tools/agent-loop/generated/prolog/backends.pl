...
backend_factory('claude-code', [resolve_type(cli), class_name('ClaudeCodeBackend'), import_from(backends), default_command(claude), default_model(sonnet), cli_args(["--print","--model"]), constructor_args([arg(command,cmd),arg_model])]).
backend_factory(gemini, [resolve_type(cli), class_name('GeminiBackend'), import_from(backends), default_command(gemini), default_model('gemini-3-flash-preview'), model_validator(validate_gemini_model), cli_args(["--model"]), constructor_args([arg(command,cmd),arg_model,arg(sandbox,sandbox),arg(approval_mode,approval_mode),arg_expr(allowed_tools,'allowed_tools or []')])]).
backend_factory(claude, [optional_import(true), resolve_type(api), class_name('ClaudeAPIBackend'), import_from(backends), default_model('claude-sonnet-4-20250514'), constructor_args([arg(api_key,api_key),arg_model,arg(system_prompt,system_prompt)])]).
backend_factory(openai, [optional_import(true), resolve_type(api), class_name('OpenAIBackend'), import_from(backends), default_model('gpt-4o'), constructor_args([arg(api_key,api_key),arg_model,arg(system_prompt,system_prompt),arg_expr(base_url,'agent_config.extra.get(\'base_url\')')])]).

...
def process_query(query, shards=None, reasoning_mode="api", synthesis_mode="api"):
    """Send query to orchestrator with optional mode overrides"""
    try:
        payload = {
            "query": query,
            "shards": shards or [],
        ...
        # Note: synthesis_mode is set via environment variable
        # If manual synthesis is needed, set SYNTH_PROVIDER=manual before starting
...
with st.sidebar:
    ...
    # Manual mode toggles
    manual_synthesis = st.checkbox(
        "Manual Synthesis Mode",
        value=False,
        help="Generate retrieval outputs with instructions for manual Master Prompt creation"
    )
    
    manual_reasoning = st.checkbox(
        "Manual Reasoning Mode",
        value=False,
        help="Generate Master Prompt only - paste into Claude/ChatGPT for final answer"
    )
    
    if manual_synthesis:
        st.info("📝 **Manual Synthesis**: Retrieval outputs will include instructions for generating the Master Prompt in your chat interface")
...
with tab1:
        ...
        if st.button("🚀 Submit", type="primary", use_container_width=True):
            if query:
                with st.spinner("Processing query..."):
                    # Include manual reasoning flag if set
                    reasoning_mode = "manual" if manual_reasoning else "api"
    ...
    if st.session_state.current_result:
        ...
        if "error" in result:
            ...
            # Show special message for manual modes
            metadata = result.get("metadata", {})
            if metadata.get("synthesis_mode") == "manual":
                st.warning("""
                **🔧 Manual Synthesis Mode**
                
                1. Copy the synthesis instructions below
                2. Paste them into your preferred chat interface (Claude, ChatGPT, etc.)
                3. Generate the Master Prompt
                4. Use the generated Master Prompt for final reasoning
                """)
            elif metadata.get("model") == "manual":
                st.info("🔍 **Manual Reasoning Mode**: Copy the Master Prompt below and paste it into your preferred chat interface for final reasoning.")
            ...
            with st.expander("📋 Metadata"):
                ...
                with col3:
                    st.metric("Mode", metadata.get("synthesis_mode", "api"))
            
            # Master Prompt or Synthesis Instructions
            is_manual_synthesis = metadata.get("synthesis_mode") == "manual"
            prompt_title = "📝 Synthesis Instructions" if is_manual_synthesis else "🔧 Master Prompt (Synthesized Context)"
            
            with st.expander(prompt_title, expanded=(metadata.get("model") == "manual" or is_manual_synthesis)):
                content = result.get("master_prompt", "Not available")
                
                if is_manual_synthesis:
                    st.markdown("**Copy these instructions to generate the Master Prompt:**")
                
                st.text_area(
                    "Content",
                    value=content,
                    height=400 if is_manual_synthesis else 300,
                    disabled=True,
                    key="master_prompt_display"
                )
                
                # Copy button
                col1, col2 = st.columns([3, 1])
                with col2:
                    if st.button("📋 Copy to Clipboard", key="copy_master"):
                        st.success("Select all text (Ctrl+A) in the box above and copy (Ctrl+C)")

...
%% generate_wam_rust_optimized_benchmark.pl
%%
%% Generates a Rust WAM benchmark from OPTIMIZED Prolog.
%%
%% Pipeline:
...
%%   4. Compile ALL predicates (base + generated) to WAM -> Rust
%%
%% This bridges the Prolog optimization passes with the WAM Rust target,
%% ensuring the WAM compiler sees the same optimized predicates that make
%% the SWI-Prolog benchmarks fast.
%%
%% Usage:
%%   swipl -q -s generate_wam_rust_optimized_benchmark.pl -- \
...
generate(VariantAtom, OutputDir) :-
    ...
    format(user_error, '[WAM-Rust-Optimized] variant=~w predicates=~w~n',
           [VariantAtom, Predicates]),

    % Step 5: Generate Rust WAM project with lowered emitter and parallel support
    Options = [module_name('wam-rust-optimized-bench'),
               wam_fallback(true),
               emit_mode(functions),
               parallel(true)],
    write_wam_rust_project(Predicates, Options, OutputDir),
    format(user_error, '[WAM-Rust-Optimized] Generated project at ~w~n', [OutputDir]).

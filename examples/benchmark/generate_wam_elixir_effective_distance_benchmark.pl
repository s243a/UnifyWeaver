...
write_bench_driver(Path, Predicates, Options) :-
    ...
    format(S, '      {:ok, state} ->~n', []),
    format(S, '        hops = WamRuntime.deref_var(state, Map.get(state.regs, 3))~n', []),
    format(S, '        h_val = if is_integer(hops), do: hops, else: String.to_integer(to_string(hops))~n', []),
    ...
    format(S, '        hops = WamRuntime.deref_var(next_state, Map.get(next_state.regs, 3))~n', []),
    format(S, '        h_val = if is_integer(hops), do: hops, else: String.to_integer(to_string(hops))~n', []),
    format(S, '        weight = :math.pow(h_val + 1, -5)~n', []),

...
python examples/benchmark/benchmark_csharp_query_lmdb_source_mode_sweep.py \
  --rows 1000 \
  --keys 100 \
  --lookup-repetitions 10 \
  --format markdown
```

`benchmark_csharp_query_effective_distance_artifact_backends.py` repeats the
same backend comparison on real effective-distance support relations. By
default it uses `category_parent/2`; pass `--relation article_category` to
measure the page-to-category membership side of the workload. It interns TSV
values to integer IDs for the artifact formats, then reports open, lookup,
bucket, and scan timings:
...
The same script can also run the SimpleWiki category-only larger fixtures used
by the Haskell scaling notes. These fixtures are intentionally not generated by
default; prepare them explicitly and keep the memory/idle guardrails enabled for
...
For English Wikipedia page-inclusive backend tests, prepare `article_category/2`
from `categorylinks` rows whose `cl_type` is `page`. This is still a
page-to-category relation, not page-to-page links from `pagelinks`:
...
Then run the same backend comparison with the matching relation:

```bash
python examples/benchmark/benchmark_csharp_query_effective_distance_artifact_backends.py \
  --scales 1m_pages \
  --relation article_category \
  --benchmark-root /tmp/uw-csharp-query-page-relations \
  --artifact-root /tmp/uw-csharp-query-page-relation-artifacts \
...
Wikipedia page IDs from the parser output. In Rust mode, the TSV sidecar is only
for non-LMDB backend comparisons, not the LMDB transport. Existing LMDB
artifacts are reused by default; pass `--refresh-lmdb` to rebuild them.
...
pass `--lmdb-only`; that mode does not require the relation TSV sidecar.
...
On the first 1M English page-inclusive probe (`article_category/2`, sourced
from `categorylinks` rows with `cl_type=page`), `mmap-array` also won both
lookup directions and both bucket directions. The page-inclusive relation is
larger and less scan-friendly than the category-only smoke, but the same
high-level policy conclusion held:
...
| `benchmark_csharp_query_effective_distance_artifact_backends.py` | Compare preload, binary artifact, delimited artifact, LMDB, and mmap-array providers on real effective-distance support relation data |

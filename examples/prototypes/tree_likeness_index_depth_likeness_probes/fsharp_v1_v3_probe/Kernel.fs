...
///   upward: category_parent (category_parent), downward: category_child CSR.
///
...
let mutable maxPaths = 200000   // global path cap for the explore loop
let mutable capHit = false       // flips true if the cap was reached on the most recent call

let nativeKernel_bidirectional_ancestor_withMinDist
    (lookupParents: int -> int list)
    (lookupChildren: int -> int list)
    (minDist: System.Collections.Generic.Dictionary<int, int>)
    (cat: int) (root: int)
    ...
    let rec explore
        ...
        | (node, cost, hops, pHops, cHops, visited) :: rest ->
            let acc' =
                if node = root && hops > 0 then
                    pathCount <- pathCount + 1
                    (hops, pHops, cHops) :: acc
                else acc
            let parentNext =
...
let nativeKernel_bidirectional_ancestor
    ...
    nativeKernel_bidirectional_ancestor_withMinDist
        lookupParents lookupChildren cal.MinDist
        cat root parentCost childCost budget

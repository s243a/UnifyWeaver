...
;; Generated at: 2026-09-03 02:35:13
...
(defn push-choice-point
  ...
  ([state target-pc ite?]
   (update state :choice-points conj
           (cond-> (choice-snapshot state target-pc)
             ite? (assoc :ite true)))))
...
(defn pop-unify-item [state]
  (loop [s state]
    (let [q (:unify-queue s)]
      (if (seq q)
        [(first q) (assoc s :unify-queue (vec (rest q)))]

...
type WamState struct {
	...
	forceHeapScan bool
	// forceFullYSave (D116) makes pushYSave store the full 100-slot Y range
	// instead of sizing to MaxYReg-200, for a clean A/B of the slot-sizing win
	// (and a fallback). Set once from UW_GO_YSAVE_FULL=1. Output is identical
	// either way -- popYSave/invokeAtPC restore len(entry) slots and clear the
	// tail to nil regardless (mirroring restoreSavedRegs).
	forceFullYSave bool
}
...
func NewWamState(code []Instruction, labels map[string]int) *WamState {
	...
	return &WamState{Ctx: ctx, Bindings: make([]Value, 4096), E: -1, Input: bufio.NewReader(os.Stdin), NextStreamID: 1, heapVarPos: make(map[*Unbound][]int), forceHeapScan: os.Getenv("UW_GO_HEAPSCAN") == "1", forceFullYSave: os.Getenv("UW_GO_YSAVE_FULL") == "1"}
}
...
// copyYSaveStack (D116): copy only the outer slice of headers. Each entry is a
// write-once []Value (created by copy in pushYSave, thereafter only read), so
// sharing the backing arrays between vm.YSaves and a choice point is
// observationally identical to the old deep value-copy of [100]Value arrays --
// but costs depth*header instead of depth*1600 bytes. The outer slice is a
// fresh make() so a later append to vm.YSaves never perturbs the CP's snapshot.
func (vm *WamState) copyYSaveStack() [][]Value {
	if len(vm.YSaves) == 0 {
		return nil
	}
	out := make([][]Value, len(vm.YSaves))
	copy(out, vm.YSaves)
	return out
}
...
func (vm *WamState) restoreBarrier(cp *ChoicePoint) {
	...
	if cp.YSaves != nil {
		// D116: fresh outer slice (so future appends to vm.YSaves don't touch
		// the CP), sharing the write-once entry backings via header copy.
		vm.YSaves = make([][]Value, len(cp.YSaves))
...
// pushYSave (D116): snapshot only the Y-regs the program actually uses
// (Regs[200:MaxYReg]) instead of the full fixed 100-slot range. MaxYReg is a
// monotonic high-water, so slots >= MaxYReg were never written and are nil --
// the same value the old full-100 copy captured for them. popYSave restores the
// captured slots and clears the tail to nil, so output is byte-identical. Entry
// is a write-once []Value shared by choice-point snapshots (see copyYSaveStack).
func (vm *WamState) pushYSave() {
	n := vm.MaxYReg - 200
	if n < 0 {
		n = 0
	}
	if vm.forceFullYSave {
		n = 100
	}
	s := make([]Value, n)
	copy(s, vm.Regs[200:200+n])
...
func (vm *WamState) popYSave() {
	...
	// Restore the captured slots, then clear any Y-regs the callee wrote above
	// the snapshot (nil at save time) back to nil -- the "wipe callee's higher Y
	// writes" guarantee the old full-100 restore provided. Mirrors
	// restoreSavedRegs' tail-clear.
	copy(vm.Regs[200:], s)
	for i := 200 + len(s); i < vm.MaxYReg; i++ {
		vm.Regs[i] = nil
	}
}
...
func (vm *WamState) Clone() *WamState {
	newState := &WamState{
		...
		forceFullYSave: vm.forceFullYSave,
	}
	...
	if len(vm.YSaves) > 0 {
		newState.YSaves = append([][]Value(nil), vm.YSaves...)
	}
	if len(vm.CutB0Stack) > 0 {
...
func (vm *WamState) invokeAtPC(pc int, args []Value, cutFloor int) bool {
	...
	if len(vm.YSaves) > yStart {
		// D116: restore the sized entry + clear the callee-dirtied tail to nil
		// (same discipline as popYSave), then drop frames back to yStart.
		s := vm.YSaves[yStart]
		copy(vm.Regs[200:], s)
		for i := 200 + len(s); i < vm.MaxYReg; i++ {
			vm.Regs[i] = nil

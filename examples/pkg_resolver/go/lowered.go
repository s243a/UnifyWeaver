...
// PredActive_member4 — lowered from active_member/4
func (vm *WamState) PredActive_member4() bool {
    // allocate
    ...
    // get_structure a/3, X8
    if !vm.Step(&GetStructure{Functor: "a/3", Ai: 107}) {
        return false
    }
    // unify_variable Y2
    ...
    // if-then-else (lowered from try_me_else/cut_ite/jump/trust_me)
    {
        ...
        // if-then-else (lowered from try_me_else/cut_ite/jump/trust_me)
        {
            ...
            // call active_member/4
            {
                savedCP := vm.CP
                if !func() bool { if pc, ok := vm.Ctx.Labels["active_member/4"]; ok { vm.PC = pc; return vm.Run() }; if vm.executeBuiltin("active_member/4", 4) { return true }; vm.warnUnresolved("call", "active_member/4"); return false }() {
                    return false
                }
                vm.CP = savedCP
            }
...
func (vm *WamState) PredBlocked_acc5() bool {
    ...
    // get_structure alternatives/1, X7
    if !vm.Step(&GetStructure{Functor: "alternatives/1", Ai: 106}) {
        return false
    }
    // unify_variable Y2
    if !vm.Step(&UnifyVariable{Xn: 201}) {
    ...
    // put_structure blocked/1, X10
    if !vm.Step(&PutStructure{Functor: "blocked/1", Ai: 109}) {
        return false
    }
    // set_variable X11
    ...
    // put_structure alternatives/1, X11
    if !vm.Step(&PutStructure{Functor: "alternatives/1", Ai: 110}) {
        return false
    }
    // set_value Y5
...
// PredCandidate_versions4 — lowered from candidate_versions/4
func (vm *WamState) PredCandidate_versions4() bool {
    // allocate
    vm.Stack = append(vm.Stack, &EnvFrame{CP: vm.CP, B0: len(vm.ChoicePoints)})
    // get_variable Y1, A1
    vm.Regs[200] = vm.Regs[0]
...
func (vm *WamState) PredCandidates_high_first4() bool {
    ...
    // call candidate_versions/4
    {
        savedCP := vm.CP
        if !func() bool { if pc, ok := vm.Ctx.Labels["candidate_versions/4"]; ok { vm.PC = pc; return vm.Run() }; if vm.executeBuiltin("candidate_versions/4", 4) { return true }; vm.warnUnresolved("call", "candidate_versions/4"); return false }() {
            return false
        }
        vm.CP = savedCP
    }
...
// PredResolve_alternatives7 — lowered from resolve_alternatives/7
func (vm *WamState) PredResolve_alternatives7() bool {
    // allocate
    vm.Stack = append(vm.Stack, &EnvFrame{CP: vm.CP, B0: len(vm.ChoicePoints)})
    // get_variable Y2, A1
    vm.Regs[201] = vm.Regs[0]
    ...
    // if-then-else (lowered from try_me_else/cut_ite/jump/trust_me)
    {
        ...
        vm.Regs[4] = vm.getReg(207)
        // put_value Y9, A6
        vm.Regs[5] = vm.getReg(208)
        // call resolve_pending/6
        {
            savedCP := vm.CP
            if !func() bool { if pc, ok := vm.Ctx.Labels["resolve_pending/6"]; ok { vm.PC = pc; return vm.Run() }; if vm.executeBuiltin("resolve_pending/6", 6) { return true }; vm.warnUnresolved("call", "resolve_pending/6"); return false }() {
                return false
            }
            vm.CP = savedCP
        }
        ...
        // put_structure req/2, X12
        if !vm.Step(&PutStructure{Functor: "req/2", Ai: 111}) {
            return false
        }
        // set_value Y4
        if !vm.Step(&SetValue{Xn: 203}) {
...
func (vm *WamState) PredResolve_pending5() bool {
    ...
    // put_value X4, A4
    vm.Regs[3] = vm.getReg(103)
    // put_structure st/2, A5
    if !vm.Step(&PutStructure{Functor: "st/2", Ai: 4}) {
        return false
    }
    // set_constant 0
    if !vm.Step(&SetConstant{C: &Integer{Val: 0}}) {
        return false
    }
    // set_constant []
    if !vm.Step(&SetConstant{C: wamAtom____0}) {
    ...
    // execute resolve_pending/6 (tail call)
    return func() bool { if pc, ok := vm.Ctx.Labels["resolve_pending/6"]; ok { vm.PC = pc; return vm.Run() }; if vm.executeBuiltin("resolve_pending/6", 6) { return true }; vm.warnUnresolved("call", "resolve_pending/6"); return false }()

}

// PredResolve_pending6 — lowered from resolve_pending/6
func (vm *WamState) PredResolve_pending6() bool {
    // get_variable X1, A1
    vm.Regs[100] = vm.Regs[0]
    // get_variable X2, A2
    vm.Regs[101] = vm.Regs[1]
    ...
    vm.Regs[103] = vm.Regs[4]
    // get_value X3, A6
    if !vm.Unify(vm.Regs[5], vm.Regs[102]) {
        return false
    }
    return true

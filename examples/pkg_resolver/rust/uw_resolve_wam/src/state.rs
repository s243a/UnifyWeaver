...
impl WamState {
        ...
        match dd {
            ...
            Value::Str(ref f, ref a)
                ...
                Some(Value::strv("req".to_string(),
                                 vec![dd.clone(), Value::Atom("any".to_string())])),
            _ =>
    ...
    fn region_build_tree_rec(elems: &[Value], start: usize, n: i64) -> (Value, usize) {
        if n == 0 {
            return (Value::Atom("t".to_string()), start);
        }
        ...
        let (k, v) = match &elems[mid] {
            ...
            other => (other.clone(), Value::Atom("[]".to_string())),
        };
        ...
        let mut cur: Value = {
            ...
            match c {
                Value::Str(full, args) => {
                    ...
                    return Value::Str(f.to_string(), args.clone());
                }
                Value::Ref(_) => c.clone(),
                ...
                match derefed {
                    None => {
                        ...
                        if functor == full_str.as_str() {
                            ...
                            Value::Str(functor.to_string(), args.clone())
                        }
                    }
                    Some(acc) => Value::strv(functor.to_string(), acc),
                }
            }
            Value::List(items) => {
                let mut derefed: Option<Vec<Value>> = None;
...
mod region1_matching_deps_tests {
    ...
    fn atom(s: &str) -> Value { Value::Atom(s.to_string()) }
    ...
    fn fusion_walk_and_dep_to_req_cut_decision() {
        ...
        let list = Value::list(vec![
            depends(atom("foo"), Value::Integer(1), Value::strv("pkg".into(), vec![atom("bar")]), atom("gt")),
            ...
            depends(atom("foo"), Value::Integer(1), Value::strv("alternatives".into(), vec![Value::list(vec![atom("a"), atom("b")])]), atom("cc")),
        ]);
        ...
        let expected = Value::list(vec![
            req(Value::strv("pkg".into(), vec![atom("bar")]), atom("gt")),
            req(Value::strv("alternatives".into(), vec![Value::list(vec![atom("a"), atom("b")])]), atom("any")),
        ]);
    ...
    fn compound_version_structural_equality() {
        ...
        let ver = Value::strv("v".into(), vec![Value::Integer(1), Value::Integer(2), Value::Integer(3)]);
        let list = Value::list(vec![
            depends(atom("foo"), ver.clone(), atom("dep1"), atom("c1")),
            depends(atom("foo"), Value::strv("v".into(), vec![Value::Integer(1), Value::Integer(2), Value::Integer(4)]), atom("dep2"), atom("c2")), // differs -> skip
        ]);
    ...
    fn decline_open_tail_restores_minimal_snapshot() {
        ...
        let open = Value::strv("[|]/2".into(), vec![
            depends(atom("foo"), Value::Integer(1), atom("d"), atom("c")),
...
mod region2_matching_versions_tests {
    ...
    fn satisfies_all_seven_clauses() {
        ...
        assert_eq!(m.region_satisfies(&ver, &Value::strv("eq".into(), vec![v3(1,2,3)])), Some(true));
        assert_eq!(m.region_satisfies(&ver, &Value::strv("eq".into(), vec![v3(1,2,4)])), Some(false));
        // gte : \+ version_lt(Ver, G)
        assert_eq!(m.region_satisfies(&ver, &Value::strv("gte".into(), vec![v3(1,2,3)])), Some(true));  // equal -> !false
        assert_eq!(m.region_satisfies(&ver, &Value::strv("gte".into(), vec![v3(1,2,4)])), Some(false)); // ver<g -> !true
        assert_eq!(m.region_satisfies(&ver, &Value::strv("gte".into(), vec![v3(1,2,2)])), Some(true));
        // lte : \+ version_lt(G, Ver)
        assert_eq!(m.region_satisfies(&ver, &Value::strv("lte".into(), vec![v3(1,2,3)])), Some(true));
        assert_eq!(m.region_satisfies(&ver, &Value::strv("lte".into(), vec![v3(1,2,2)])), Some(false));
        // lt : version_lt(Ver, H)
        assert_eq!(m.region_satisfies(&ver, &Value::strv("lt".into(), vec![v3(1,2,4)])), Some(true));
        assert_eq!(m.region_satisfies(&ver, &Value::strv("lt".into(), vec![v3(1,2,3)])), Some(false));
        // gt : version_lt(H, Ver)
        assert_eq!(m.region_satisfies(&ver, &Value::strv("gt".into(), vec![v3(1,2,2)])), Some(true));
        assert_eq!(m.region_satisfies(&ver, &Value::strv("gt".into(), vec![v3(1,2,3)])), Some(false));
        // range(Lo,Hi) : \+ version_lt(Ver,Lo), version_lt(Ver,Hi)
        let range = |lo: Value, hi: Value| Value::strv("range".into(), vec![lo, hi]);
    ...
    fn fused_walk_keeps_only_matching_and_satisfying() {
        ...
        let c = Value::strv("gte".into(), vec![v3(1,0,0)]);
    ...
    fn shared_satisfies_callee_is_pure_no_state_mutation() {
        ...
        let probes = vec![
            atom("any"),
            Value::strv("eq".into(), vec![v3(1,2,3)]),
            Value::strv("gte".into(), vec![v3(1,0,0)]),
            Value::strv("lte".into(), vec![v3(2,0,0)]),
            Value::strv("lt".into(), vec![v3(2,0,0)]),
            Value::strv("gt".into(), vec![v3(1,0,0)]),
            Value::strv("range".into(), vec![v3(1,0,0), v3(2,0,0)]),
        ];
...
mod region3a_key_dep_rows_tests {
    ...
    fn fusion_walk_index_and_dep_to_req_cut() {
        ...
        // alternatives(_) to req(_,any) (its cut), else req(D,C).
        let alts = Value::strv("alternatives".into(), vec![Value::list(vec![atom("a"), atom("b")])]);
        let list = Value::list(vec![
            depends(atom("foo"), int(1), Value::strv("pkg".into(), vec![atom("bar")]), atom("gt")),
        ...
        assert_eq!(m.pc, 42, "pc advanced to cont_pc");
        let expected = Value::list(vec![
            row(atom("foo"), int(1), 0, req(Value::strv("pkg".into(), vec![atom("bar")]), atom("gt"))),
...
mod region5_dep_breaks_tests {
    ...
    fn ver(a: i64, b: i64, c: i64) -> Value {
        Value::strv("v".into(), vec![Value::Integer(a), Value::Integer(b), Value::Integer(c)])
    }
    // Acc entry: Name-Ver.
    fn pair(name: &str, v: Value) -> Value {
        Value::strv("-".into(), vec![atom(name), v])
    }
    fn depends(n: &str, v: Value, d: Value, c: Value) -> Value {
        Value::strv("depends".into(), vec![atom(n), v, d, c])
    }
    fn dep(name: &str, c: Value) -> Value {
        Value::strv("dep".into(), vec![atom(name), c])
    }
    fn alternatives(alts: Vec<Value>) -> Value {
        Value::strv("alternatives".into(), vec![Value::list(alts)])
    }
    fn eqc(v: Value) -> Value { Value::strv("eq".into(), vec![v]) }
    fn gtec(v: Value) -> Value { Value::strv("gte".into(), vec![v]) }
...
mod genrec_sibling_gap_tests {
    ...
    fn filter_declines_open_tail() {
        let open = Value::strv("[|]/2".into(), vec![v3(1,0,0), Value::new_unbound("Tail")]);
    ...
    fn keypkg_declines_non_package_element() {
        let list = Value::list(vec![Value::strv("depends".into(),

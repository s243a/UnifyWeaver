...
// Date: 2026-09-18 14:00:44

// D110: mimalloc as this binary's global allocator (default ON via the
// `mimalloc` Cargo feature). Drop-in swap -- changes HOW memory is
// allocated, not WHAT is computed -- so output is byte-identical to the
// OFF (system/glibc allocator) build. See docs/reports/wam_rust_mimalloc_ab.md.
...
use uw_resolve_wam::{shared_wam_program, setup_foreign_predicates};
...
fn main() {
    ...
    let n: f64 = if let Some(&pc) = labels.get("dimension_n/1") {
        vm.reset_query();
        vm.pc = pc;
        vm.set_reg("A1", Value::Unbound("_N".to_string().into()));
        if vm.run() {
    ...
    if let Some(attr) = uw_resolve_wam::state::cache_attribution() {

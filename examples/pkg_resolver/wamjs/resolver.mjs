...
function depTerm(row) {
  const third = row[2];
  let depArg;
  if (third && typeof third === "object" && third.alternatives) {
    depArg = V.Struct(functorId("alternatives"), [
      jsListToTerm(third.alternatives, (a) =>
        V.Struct(functorId("dep"), [internAtom(a.dep), constraintTerm(a.constraint)]))
    ]);
  } else {
    depArg = internAtom(third);
  }
  return V.Struct(functorId("depends"), [
    internAtom(row[0]), verTerm(row[1]), depArg, constraintTerm(row[3])
  ]);
}

function provideTerm(row) {
  if (row.length >= 4) {
    return V.Struct(functorId("provides"), [
      internAtom(row[0]), verTerm(row[1]), internAtom(row[2]), verTerm(row[3])
    ]);
  }
  return V.Struct(functorId("provides"), [
    internAtom(row[0]), verTerm(row[1]), internAtom(row[2])
  ]);
}
...
export function catalogToTerm(cat) {
  ...
  if (layers.length === 0 && excluded.length === 0 && aliases.length === 0 && provides.length === 0) {
  ...
  if (provides.length === 0) return V.Struct(functorId("catalog"), nine);
  return V.Struct(functorId("catalog"), [
    ...nine,
    jsListToTerm(provides, provideTerm)
  ]);
}

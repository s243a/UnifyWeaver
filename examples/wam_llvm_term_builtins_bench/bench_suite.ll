...
; uniform pushes/pops).
%StackEntry = type { i32, i64, %Value*, [16 x %Value] }  ; type, aux, data, y_save
...
define %WamState* @wam_state_new(%Instruction* %code, i32 %code_len, i32* %labels, i32 %label_count) {
  ...
  ; Allocate stack (initial capacity 256). StackEntry grew from 24 to
  ; 280 bytes after the Y-save/restore refactor (16 %Value slots per
  ; entry). Size: 256 * 280 = 71 680 B.
  %stack_mem = call i8* @malloc(i64 71680)  ; 256 * sizeof(StackEntry)
  ...
  ; Allocate heap (initial capacity 65536). Bumped from 4096 to give
  ; the Ref-based put_variable headroom: each fresh variable consumes a
  ; heap cell, and recursive predicates can push thousands per
  ; invocation. No bounds check in wam_heap_push yet, so growing the
  ; cap pre-emptively beats a silent overrun.
  %heap_mem = call i8* @malloc(i64 1048576)  ; 65536 * sizeof(Value)
  ...
  ; Allocate trail (initial capacity 16384 entries, ~384 KB).
  ; Previously 256; increased because the Ref-based put_variable
  ; landing will cause each binding to produce two trail entries (one
  ; reg, one heap) under the current trailing discipline, and deep
  ; recursion (e.g. bench_sum_big) comfortably exceeded 256. Headroom
  ; here removes trail overflow as a blocker for that work. Still no
  ; bounds check in wam_trail_binding/wam_trail_heap_binding — if this
  ; turns out to be insufficient, add a realloc path or a bounds
  ; check that aborts with a clear diagnostic.
  %trail_mem = call i8* @malloc(i64 393216)
...
@module_code = private constant [363 x %Instruction] [
  ...
  %Instruction { i32 8, i64 0, i64 65537 },
  %Instruction { i32 8, i64 55, i64 65538 },
  %Instruction { i32 17, i64 0, i64 0 },
  %Instruction { i32 19, i64 13, i64 0 },
  %Instruction { i32 16, i64 0, i64 0 },
  %Instruction { i32 11, i64 ptrtoint ([2 x i8]* @.fn_f to i64), i64 262144 },

...
:- module(mutual_recursion, [
    ...
    is_mutual_recursive_clause/2,   % +AllPredicates, +Clause (shared with target plugins)
    parse_recursive_body/5,         % +Body, +HeadVar, -Conditions, -Computations, -RecCall (shared)
...
%% compile_mutual_pattern/5 is multifile - targets register their own clauses.
%% compile_mutual_pattern(+Target, +Predicates, +MemoEnabled, +MemoStrategy, -Code)
:- multifile compile_mutual_pattern/5.
...
compile_mutual_recursion(Predicates, Options, Code) :-
    ...
    (   compile_mutual_pattern(Target, Predicates, MemoEnabled, MemoStrategy, Code) ->
        true
    ;   format('Error: No mutual recursion support for target ~w~n', [Target]),
        fail
    ).
...
compile_mutual_pattern(bash, Predicates, MemoEnabled, MemoStrategy, BashCode) :-
...
test_mutual_recursion :-
    ...
    (   can_compile_mutual_recursion(Predicates1) ->
        ...
        compile_mutual_recursion(Predicates1, [target(c)], Code1C),
        write_bash_file('output/advanced/even_odd.c', Code1C),
        writeln('  ✓ Compiled to output/advanced/even_odd.c (c)'),
        compile_mutual_recursion(Predicates1, [target(haskell)], Code1H),

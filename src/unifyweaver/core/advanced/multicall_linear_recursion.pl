...
:- module(multicall_linear_recursion, [
    ...
    compile_multicall_linear_recursion/3, % +Pred/Arity, +Options, -Code
    extract_goal_from_body/2,             % +Body, -Goal (shared with target plugins)
    find_aggregation_expr/2,              % +Body, -Expr (shared with target plugins)
...
:- multifile compile_multicall_pattern/6.
...
compile_multicall_linear_recursion(Pred/Arity, Options, Code) :-
    ...
    (   compile_multicall_pattern(Target, PredStr, BaseClauses, RecClauses, MemoEnabled, Code) ->
        true
    ;   format('Error: No multicall linear recursion support for target ~w~n', [Target]),
        fail
    ).
...
compile_multicall_pattern(bash, PredStr, BaseClauses, RecClauses, MemoEnabled, BashCode) :-

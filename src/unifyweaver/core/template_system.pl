...
%% The lmdb-zero dep is conditional on {{use_lmdb}} (R1 of the Rust-LMDB
%% arc; see context/wam_rust_lmdb_handoff.md).  The companion file at
%% templates/targets/rust_wam/Cargo.toml.mustache is currently a mirror
%% kept for future migration to file-based templates; the inline version
%% below is what `render_named_template(rust_wam_cargo, ...)` actually
%% renders.
...
[dependencies]
{{#use_lmdb}}lmdb-zero = "0.4"
{{/use_lmdb}}').

%% Rust WAM lib.rs template
%% lmdb_fact_source module is conditionally declared (R1 of the
%% Rust-LMDB arc; see context/wam_rust_lmdb_handoff.md).  Wiring its
%% callers into main.rs is R2/R4 work.
...
pub mod state;
{{#use_lmdb}}pub mod lmdb_fact_source;
{{/use_lmdb}}

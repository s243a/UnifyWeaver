...
%  Compile a rule with joins to pure PowerShell
compile_rule_to_powershell(Pred, Arity, _Head, Body, _Options, Code) :-
    ...
    % For now, handle simple joins (two binary predicates)
    (   Goals = [Goal1, Goal2],
        Goal1 =.. [Pred1|_Args1],
        Goal2 =.. [Pred2|_Args2]
    ->  % Simple join between two predicates
...
# Stream all results
~w
", [Pred, Arity, DateStr, PredStr, Pred1Str, Pred2Str, PredStr])

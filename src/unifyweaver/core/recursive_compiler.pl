...
%% Elixir transitive closure - generates BFS with Map/List
compile_transitive_closure(elixir, Pred, _Arity, BasePred, _Options, GeneratedCode) :-
    atom_string(Pred, PredStr),
    atom_string(BasePred, BaseStr),
    % Use proper CamelCase for Elixir module names
...
defmodule Generated.~wQuery do
  ...
  def main(args) do
    ...
    case args do
      [] ->
        IO.puts(:stderr, "Usage: elixir ~w_query.exs <start> [target]")
        System.halt(1)
      [start] ->
        find_all(start, rel)
...
case System.argv() do
  [] -> :ok
  args -> Generated.~wQuery.main(args)
end
', [PredStr, BaseStr, PredCap, PredStr, PredCap]),
    !.
...
', [PredStr, BaseStr, PredCap, PredCap, PredCap, BaseStr]),
    !.

%% Go transitive closure - generates BFS with map for adjacency
...
func New~wQuery() *~wQuery {
	return &~wQuery{baseRelation: make(map[string][]string)}
}

func (q *~wQuery) AddFact(from, to string) {
	q.baseRelation[from] = append(q.baseRelation[from], to)
}

func (q *~wQuery) FindAll(start string) []string {
	var results []string
...
func (q *~wQuery) Check(start, target string) bool {
	if start == target {
		return false
	}
	visited := make(map[string]bool)
...
', [PredStr, BaseStr, PredCap, PredCap, PredCap, PredCap, PredCap, PredCap, PredCap, PredCap, BaseStr]),
    !.

%% Python transitive closure - generates BFS with deque (CPython native)
...
%% PowerShell transitive closure - generates BFS with hashtable and ArrayList
...
%% Ruby transitive closure - generates BFS with Hash and Array
...
%% Perl transitive closure - generates BFS with hash and array
...
%% TypeScript transitive closure - generates BFS with Map and Array
...
%% F# transitive closure - generates BFS with Map and Set
...
let main argv =
    ...
    | _ ->
        eprintfn "Usage: ~w <start> [target]"
        1
', [PredStr, BaseStr, PredStr]),
    !.

%% Haskell transitive closure - generates BFS with Data.Map and Data.Set
...
main = do
    ...
    case args of
      ...
      _ -> do
        putStrLn "Usage: ~w <start> [target]"
        exitFailure
', [PredStr, BaseStr, PredStr]),
    !.

%% Lua transitive closure - generates BFS with tables
...
if #arg == 1 then
    local results = find_all(arg[1])
    for _, r in ipairs(results) do
        print(arg[1] .. ":" .. r)
    end
elseif #arg == 2 then
    if check(arg[1], arg[2]) then
        print(arg[1] .. ":" .. arg[2])
    else
        os.exit(1)
    end
else
    io.stderr:write("Usage: lua ~w.lua <start> [target]\\n")
    os.exit(1)
end
', [PredStr, BaseStr, BaseStr, PredStr]),
    !.

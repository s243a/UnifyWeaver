...
~w :: Int -> Int
~w ~w = ~w
~w n = foldl (\\acc current -> ~w) ~w [n, n-1 .. 1]
...
', [PredStr, PredStr, PredStr, BaseInput, BaseOutput, PredStr, HaskellOp, BaseOutput, PredStr, PredStr])
    ;   InputType = list ->
        format(string(Code),
...
main = do
    ...
    case args of
        (arg:_) -> print $ ~w (map read $ words $ map (\\c -> if c == '','' then '' '' else c) arg :: [Int])
        _       -> putStrLn "Usage: ~w <comma-separated-list>"
', [PredStr, PredStr, PredStr, BaseOutput, PredStr, PredStr, PredStr, PredStr])

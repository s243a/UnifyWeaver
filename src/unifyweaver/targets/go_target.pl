...
step_op_to_go(sub_element, "acc -= item").
...
generate_ternary_tail_recursion_go(PredStr, BaseArgs, _RecArgs, RecBody, GoCode) :-
    ...
    ternary_step_op_to_go(StepOp, GoStepCode),
...
%% ternary_step_op_to_go(+StepOp, -GoCode)
%  Convert Prolog step operation to Go code (used by generate_ternary_tail_recursion_go)
ternary_step_op_to_go(arithmetic(_Acc + Const), GoCode) :-
    integer(Const), !,
    format(atom(GoCode), 'currentAcc += ~w', [Const]).
ternary_step_op_to_go(arithmetic(_Acc + _N), 'currentAcc += currentN') :- !.
ternary_step_op_to_go(arithmetic(_Acc * _N), 'currentAcc *= currentN') :- !.
ternary_step_op_to_go(arithmetic(_Acc - _N), 'currentAcc -= currentN') :- !.
ternary_step_op_to_go(unknown, 'currentAcc += 1') :- !.
ternary_step_op_to_go(_, 'currentAcc += 1').  % Fallback
...
% ============================================================================
% MULTIFILE DISPATCH - Tail Recursion (Go target)
% ============================================================================

:- use_module('../core/advanced/tail_recursion').
:- multifile tail_recursion:compile_tail_pattern/9.

tail_recursion:compile_tail_pattern(go, PredStr, Arity, _BaseClauses, _RecClauses, _AccPos, StepOp, _ExitAfterResult, Code) :-
    step_op_to_go(StepOp, GoStepExpr),
    (   Arity =:= 3 ->
        % Use proper loop var syntax — Go doesn't allow unused vars
        (   sub_atom(GoStepExpr, _, _, _, item)
        ->  LoopHead = '_, item := range items'
        ;   LoopHead = 'range items'
        ),
        format(string(Code),
'// Generated by UnifyWeaver Go Target - Tail Recursion (slice accumulator, multifile dispatch)
// Predicate: ~w/~w
...
', [PredStr, Arity, PredStr, LoopHead, GoStepExpr, PredStr])
    ;   Arity =:= 2 ->
        format(string(Code),
'// Generated by UnifyWeaver Go Target - Tail Recursion (counter, multifile dispatch)
// Predicate: ~w/~w
...
step_op_to_go(arithmetic(Expr), GoExpr) :- tail_expr_to_go(Expr, GoExpr).
step_op_to_go(unknown, 'acc + 1').

tail_expr_to_go(_ + Const, GoExpr) :- integer(Const), !, format(atom(GoExpr), 'acc + ~w', [Const]).
tail_expr_to_go(_ + _, 'acc + item') :- !.
tail_expr_to_go(_ - _, 'acc - item') :- !.
tail_expr_to_go(_ * _, 'acc * item') :- !.
tail_expr_to_go(_, 'acc + 1').

% ============================================================================
% MULTIFILE DISPATCH - Linear Recursion (Go target)
% ============================================================================

:- use_module('../core/advanced/linear_recursion').
:- multifile linear_recursion:compile_linear_pattern/8.

linear_recursion:compile_linear_pattern(go, PredStr, Arity, BaseClauses, _RecClauses, MemoEnabled, _MemoStrategy, Code) :-
    (   Arity =:= 2 ->
        linear_fold_go(PredStr, BaseClauses, MemoEnabled, Code)
    ;   linear_generic_go(PredStr, Arity, MemoEnabled, Code)
    ).
...
linear_fold_go(PredStr, BaseClauses, MemoEnabled, Code) :-
    ...
    (   MemoEnabled = true ->
        MemoDecl = "var memo = map[int]int{}",
        format(string(MemoCheck), '    if v, ok := memo[n]; ok { return v }', []),
    ...
    (   ActualRec = [clause(RHead, RBody)|_] ->
        ...
        translate_fold_expr_go(FoldExpr, InputVar, AccVar, GoOp)
    ;   GoOp = "current * result"
    ),
    (   InputType = numeric ->
        format(string(Code),
'// Generated by UnifyWeaver Go Target - Linear Recursion (numeric, multifile dispatch)
// Predicate: ~w/2
...
', [PredStr, MemoDecl, PredStr, MemoCheck, BaseInput, BaseOutput, BaseOutput, GoOp, MemoStore, PredStr])
    ;   InputType = list ->
        ...
        (   ActualRec = [clause(LRHead, LRBody)|_] ->
            ...
            translate_list_fold_go(LFoldExpr, LHeadVar, LAccVar, ListGoOp)
        ;   ListGoOp = "result + item"
        ),
        format(string(Code),
'// Generated by UnifyWeaver Go Target - Linear Recursion (list, multifile dispatch)
// Predicate: ~w/2
...
', [PredStr, PredStr, BaseOutput, BaseOutput, ListGoOp, PredStr])
    ;   linear_generic_go(PredStr, 2, MemoEnabled, Code)
    ).

linear_generic_go(PredStr, Arity, MemoEnabled, Code) :-
    (   MemoEnabled = true ->
        format(string(Code),
'// Generated by UnifyWeaver Go Target - Linear Recursion (generic, multifile dispatch)
// Predicate: ~w/~w
...
func ~w(n int) int {
    ...
    var result int
    if n <= 0 { result = 0 } else if n == 1 { result = 1 } else { result = ~w(n - 1) + n }
    memo[n] = result
    return result
}
...
translate_fold_expr_go(A * B, IV, AV, E) :- translate_go_term(A, IV, AV, AT), translate_go_term(B, IV, AV, BT), format(string(E), '~w * ~w', [AT, BT]).
translate_fold_expr_go(A + B, IV, AV, E) :- translate_go_term(A, IV, AV, AT), translate_go_term(B, IV, AV, BT), format(string(E), '~w + ~w', [AT, BT]).
translate_fold_expr_go(A - B, IV, AV, E) :- translate_go_term(A, IV, AV, AT), translate_go_term(B, IV, AV, BT), format(string(E), '~w - ~w', [AT, BT]).
translate_fold_expr_go(T, IV, AV, E) :- translate_go_term(T, IV, AV, E).

translate_go_term(T, IV, _, 'current') :- T == IV, !.
translate_go_term(T, _, AV, 'result') :- T == AV, !.
translate_go_term(N, _, _, S) :- integer(N), !, format(string(S), '~w', [N]).
translate_go_term(A, _, _, S) :- format(string(S), '~w', [A]).

%% translate_list_fold_go(+PrologExpr, +HeadVar, +AccVar, -GoExpr)
%  Maps HeadVar -> 'item' (range variable) and AccVar -> 'result' (accumulator).
translate_list_fold_go(A * B, HV, AV, E) :- translate_list_term_go(A, HV, AV, AT), translate_list_term_go(B, HV, AV, BT), format(string(E), '~w * ~w', [AT, BT]).
translate_list_fold_go(A + B, HV, AV, E) :- translate_list_term_go(A, HV, AV, AT), translate_list_term_go(B, HV, AV, BT), format(string(E), '~w + ~w', [AT, BT]).
translate_list_fold_go(A - B, HV, AV, E) :- translate_list_term_go(A, HV, AV, AT), translate_list_term_go(B, HV, AV, BT), format(string(E), '~w - ~w', [AT, BT]).
translate_list_fold_go(T, HV, AV, E) :- translate_list_term_go(T, HV, AV, E).

translate_list_term_go(T, HV, _, 'item') :- T == HV, !.
translate_list_term_go(T, _, AV, 'result') :- T == AV, !.
translate_list_term_go(N, _, _, S) :- integer(N), !, format(string(S), '~w', [N]).
translate_list_term_go(A, _, _, S) :- format(string(S), '~w', [A]).

% ============================================================================
% MULTIFILE DISPATCH - Tree Recursion (Go target)
% ============================================================================

:- use_module('../core/advanced/tree_recursion').
:- multifile tree_recursion:compile_tree_pattern/6.

tree_recursion:compile_tree_pattern(go, _Pattern, Pred, _Arity, _UseMemo, GoCode) :-
    atom_string(Pred, PredStr),
    format(string(GoCode),
'// Generated by UnifyWeaver Go Target - Tree Recursion (fibonacci, multifile dispatch)
...
% ============================================================================
% MULTIFILE DISPATCH - Multicall Linear Recursion (Go target)
% ============================================================================

:- use_module('../core/advanced/multicall_linear_recursion').
:- multifile multicall_linear_recursion:compile_multicall_pattern/6.

multicall_linear_recursion:compile_multicall_pattern(go, PredStr, BaseClauses, _RecClauses, _MemoEnabled, GoCode) :-
    ...
    format(string(GoCode),
'// Generated by UnifyWeaver Go Target - Multicall Linear Recursion (multifile dispatch)
...
% ============================================================================
% MULTIFILE DISPATCH - Direct Multi-Call Recursion (Go target)
% ============================================================================
...
direct_multi_call_recursion:compile_direct_multicall_pattern(go, PredStr, BaseClauses, RecClause, GoCode) :-
    findall(BaseCaseCode, (
        member(clause(BHead, _), BaseClauses),
        BHead =.. [_P, BInput, BOutput],
    ...
    direct_computations_go(Computations, ComputationsCode),
    direct_recursive_calls_go(RecCalls, PredStr, RecCallsCode),
    direct_aggregation_go(Aggregation, AggCode),
    format(string(GoCode),
'// Generated by UnifyWeaver Go Target - Direct Multicall Recursion (multifile dispatch)
...
func ~w(input int) int {
    if v, ok := memo[input]; ok { return v }
~w
~w
~w
~w
...
direct_computations_go(Computations, Code) :-
    findall(Line, (
        member(Var is Expr, Computations),
        direct_var_to_go_name(Var, VarName),
        direct_translate_expr_go(Expr, GoExpr),
        format(string(Line), '    ~w := ~w', [VarName, GoExpr])
    ), Lines),
    atomic_list_concat(Lines, '\n', Code).

direct_recursive_calls_go(RecCalls, PredStr, Code) :-
    findall(Line, (
        member(RecCall, RecCalls),
        RecCall =.. [_Pred, ArgVar, ResultVar],
        direct_var_to_go_name(ArgVar, ArgName),
        direct_var_to_go_name(ResultVar, ResName),
        format(string(Line), '    ~w := ~w(~w)', [ResName, PredStr, ArgName])
    ), Lines),
    atomic_list_concat(Lines, '\n', Code).

direct_aggregation_go(_ is Expr, Code) :-
    direct_translate_agg_expr_go(Expr, GoExpr),
    format(string(Code), '    result := ~w', [GoExpr]).

direct_translate_agg_expr_go(A + B, Expr) :-
    direct_var_to_go_name(A, AName), direct_var_to_go_name(B, BName),
    format(string(Expr), '~w + ~w', [AName, BName]).
direct_translate_agg_expr_go(A * B, Expr) :-
    direct_var_to_go_name(A, AName), direct_var_to_go_name(B, BName),
    format(string(Expr), '~w * ~w', [AName, BName]).

%% direct_var_to_go_name(+Var, -GoName)
%  Convert Prolog variable to valid Go identifier.
direct_var_to_go_name(Var, GoName) :-
    (   var(Var) ->
        term_string(Var, VarStr),
        atom_string(VarAtom, VarStr),
        downcase_atom(VarAtom, LowerName),
        ensure_go_identifier(LowerName, GoName)
    ;   atom(Var) ->
...
ensure_go_identifier(Name, ValidName) :-
...
direct_translate_expr_go(N - K, Expr) :- var(N), integer(K), !,
    format(string(Expr), 'input - ~w', [K]).
direct_translate_expr_go(N + K, Expr) :- var(N), integer(K), K < 0, !,
    AbsK is abs(K), format(string(Expr), 'input - ~w', [AbsK]).
direct_translate_expr_go(N + K, Expr) :- var(N), integer(K), !,
    format(string(Expr), 'input + ~w', [K]).
direct_translate_expr_go(Expr, 'input') :- var(Expr), !.
direct_translate_expr_go(N, S) :- integer(N), !, format(string(S), '~w', [N]).
direct_translate_expr_go(A, S) :- format(string(S), '~w', [A]).

% ============================================================================
% MULTIFILE DISPATCH - Mutual Recursion (Go target)
% ============================================================================

:- use_module('../core/advanced/mutual_recursion').
:- multifile mutual_recursion:compile_mutual_pattern/5.

mutual_recursion:compile_mutual_pattern(go, Predicates, MemoEnabled, _MemoStrategy, GoCode) :-
    % Generate function definitions
    mutual_functions_go(Predicates, Predicates, MemoEnabled, FuncCodes),
    atomic_list_concat(FuncCodes, '\n\n', FunctionsCode),
    % Generate dispatch switch cases
    mutual_dispatch_go(Predicates, DispatchCode),
    (   MemoEnabled = true ->
        MemoDecl = "var memo = map[string]bool{}"
    ;   MemoDecl = "// Memoization disabled"
    ),
    format(string(GoCode),
'// Generated by UnifyWeaver Go Target - Mutual Recursion (multifile dispatch)
...
mutual_functions_go([], _AllPreds, _MemoEnabled, []).
mutual_functions_go([Pred/Arity|Rest], AllPreds, MemoEnabled, [FuncCode|RestCodes]) :-
    atom_string(Pred, PredStr),
    functor(Head, Pred, Arity),
    ...
    mutual_base_cases_go(BaseClauses, MemoEnabled, PredStr, BaseParts),
    atomic_list_concat(BaseParts, '\n', BaseCode),
    mutual_rec_cases_go(RecClauses, MemoEnabled, PredStr, RecParts),
    atomic_list_concat(RecParts, '\n', RecCode),
    (   MemoEnabled = true ->
        format(string(FuncCode),
'func ~w(n int) bool {
    key := fmt.Sprintf("~w_%d", n)
...
~w
    ...
    mutual_functions_go(Rest, AllPreds, MemoEnabled, RestCodes).

mutual_base_cases_go([], _MemoEnabled, _PredStr, []).
mutual_base_cases_go([clause(Head, true)|Rest], MemoEnabled, PredStr, [Part|RestParts]) :-
    Head =.. [_Pred, Value],
    (   MemoEnabled = true ->
        format(string(Part),
'    if n == ~w { memo[fmt.Sprintf("~w_%d", n)] = true; return true }',
            [Value, PredStr])
    ;   format(string(Part), '    if n == ~w { return true }', [Value])
    ),
    mutual_base_cases_go(Rest, MemoEnabled, PredStr, RestParts).
mutual_base_cases_go([_|Rest], MemoEnabled, PredStr, RestParts) :-
    mutual_base_cases_go(Rest, MemoEnabled, PredStr, RestParts).

mutual_rec_cases_go([], _MemoEnabled, _PredStr, []).
mutual_rec_cases_go([clause(_Head, Body)|Rest], MemoEnabled, PredStr, [Part|RestParts]) :-
    extract_mutual_rec_info_go(Body, Guard, CalledPred, Step),
...
'    if n > ~w {
        result := ~w(n ~w)
        memo[fmt.Sprintf("~w_%d", n)] = result
        return result
...
'    result := ~w(n ~w)
    memo[fmt.Sprintf("~w_%d", n)] = result
    return result', [CalledStr, Step, PredStr])
        ;   format(string(Part), '    return ~w(n ~w)', [CalledStr, Step])
        )
    ),
    mutual_rec_cases_go(Rest, MemoEnabled, PredStr, RestParts).
mutual_rec_cases_go([_|Rest], MemoEnabled, PredStr, RestParts) :-
    mutual_rec_cases_go(Rest, MemoEnabled, PredStr, RestParts).

extract_mutual_rec_info_go(Body, Guard, CalledPred, Step) :-
    extract_goals_from_body_go(Body, Goals),
...
extract_goals_from_body_go((A, B), Goals) :- !,
    extract_goals_from_body_go(A, GA),
    extract_goals_from_body_go(B, GB),
    append(GA, GB, Goals).
extract_goals_from_body_go(true, []) :- !.
extract_goals_from_body_go(Goal, [Goal]).

mutual_dispatch_go(Predicates, Code) :-
    findall(DispLine, (
        member(Pred/_Arity, Predicates),
        atom_string(Pred, PredStr),

...
emit_one(get_constant(CStr, AiStr), I) :-
    ...
    format("~w{~n", [I]),
    format("~w    let _a = vm.get_reg(\"~w\").unwrap_or(Value::Unbound(\"__uninit__\".to_string()));~n", [I, Ai]),
...
emit_one(get_value(XnStr, AiStr), I) :-
    ...
    format("~w{~n", [I]),
    format("~w    let va = vm.get_reg(\"~w\").unwrap_or(Value::Unbound(\"__uninit__\".to_string()));~n", [I, Ai]),
    format("~w    let vx = vm.get_reg(\"~w\").unwrap_or(Value::Unbound(\"__uninit__\".to_string()));~n", [I, Xn]),

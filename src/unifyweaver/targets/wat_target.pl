...
compile_linear_recursion_wat(Pred/Arity, _Options, Code) :-
    ...
    ->  translate_fold_expr_wat(FoldExpr, InputVar, AccVar, WATFoldOp)
    ;   WATFoldOp = "i64.add"
    ),
    format(string(Code),
'(module
...
', [PredStr, Arity, PredStr, PredStr, WATFoldOp, PredStr]).

%% translate_fold_expr_wat(+FoldExpr, +InputVar, +AccVar, -WATOp)
%  Maps Prolog fold expressions to WAT i64 instructions.
translate_fold_expr_wat(A * B, IV, AV, "i64.mul") :- (A == IV ; A == AV), (B == IV ; B == AV), !.
translate_fold_expr_wat(A + B, IV, AV, "i64.add") :- (A == IV ; A == AV), (B == IV ; B == AV), !.
translate_fold_expr_wat(A - B, IV, AV, "i64.sub") :- (A == IV ; A == AV), (B == IV ; B == AV), !.
translate_fold_expr_wat(A * _, IV, AV, "i64.mul") :- (A == IV ; A == AV), !.
translate_fold_expr_wat(_ * B, IV, AV, "i64.mul") :- (B == IV ; B == AV), !.
translate_fold_expr_wat(_, _, _, "i64.add").

...
~w    public static class ~w
    {
~w        private static readonly Lazy<QueryPlan> CachedPlan = new Lazy<QueryPlan>(() =>
            new QueryPlan(
                new PredicateId("~w", ~w),
                ~w,
                ~w,
...
'        public static (IRelationProvider Provider, QueryPlan Plan) Build() => ~w();
...
build_provider_method(ProviderSection, Method) :-
    format(atom(Method),
'        private static ConfiguredDelimitedRelationProvider BuildProvider()
        {
...
', [Cond, MethodName])
    ...
    format(atom(Method),
'        public static (IRelationProvider Provider, QueryPlan Plan) BuildForInputs(params int[] inputPositions)
        {
...
'        private static readonly Lazy<QueryPlan> ~w = new Lazy<QueryPlan>(() =>
        ...
        public static (IRelationProvider Provider, QueryPlan Plan) ~w()
        {
            var configuredProvider = BuildProvider();
            return (configuredProvider.Provider, ~w.Value);
        }
...
relation_blocks(Relations, ProviderStatements, UsesDynamic) :-
    findall(Line,
        (   member(relation{predicate:predicate{name:Name, arity:Arity}, facts:Rows}, Relations),
            ...
            format(atom(Line), '            memoryProvider.AddFact(new PredicateId("~w", ~w), ~w);', [NameStr, Arity, ArgList])
        ),
        FactLines),
    findall(Block,
...
'            foreach (var row in ~w.Read())
            {
                memoryProvider.AddFact(new PredicateId(\"~w\", ~w), row);
            }',
...
field_separator_char_literal(Value, Literal) :-
    literal_string(Value, Str),
    string_length(Str, 1),
    sub_string(Str, 0, 1, _, Char),
    escape_csharp_char(Char, Escaped),
...
escape_csharp_char(Input, Escaped) :-
    string_codes(Input, [Code]),
    escape_char_code(Code, Escaped).

escape_char_code(92, '\\\\') :- !.
escape_char_code(39, '\\\'') :- !.
escape_char_code(10, '\\n') :- !.
escape_char_code(13, '\\r') :- !.
escape_char_code(9, '\\t') :- !.
escape_char_code(Code, Atom) :-

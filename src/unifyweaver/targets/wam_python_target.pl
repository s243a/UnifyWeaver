...
clean_wam_constant_token(Token, Clean) :-
	...
	!,
	InnerLen is Len - 2,
	sub_string(S, 1, InnerLen, 1, Inner),
	atom_string(Clean, Inner).
clean_wam_constant_token(Token, Clean) :-
...
wam_line_to_python_literal(["get_constant", C, Ai], Py) :-
	python_text_constant_literal(C, PyVal), clean_comma(Ai, CAi),
	format(atom(Py), '("get_constant", ~w, ~w)', [PyVal, CAi]).
...
wam_line_to_python_literal(["unify_constant", C], Py) :-
	python_text_constant_literal(C, PyVal),
	format(atom(Py), '("unify_constant", ~w)', [PyVal]).
...
write_wam_python_project(Predicates, Options, ProjectDir) :-
	...
	python_project_predicates(Predicates, RuntimeParserMode, ProjectPredicates),
	...
	compile_all_predicates(ProjectPredicates, [runtime_parser_mode(RuntimeParserMode)|Options], PredicatesCode),
...
compile_all_predicates(Predicates, Options, Code) :-
	...
	atomic_list_concat([
		...
		% Structure read/write-ctx helpers (underscore-prefixed, so not pulled in
		% by `import *`) used by the lowered emitter to build/match compounds with
		% the same model as the bytecode interpreter.
		'from wam_runtime import _begin_write_ctx, _write_ctx_put, _begin_read_ctx, _read_ctx_get, _read_ctx_skip, _is_cons_functor\n',
		'# WAM register index constants (A1..A128 = 1..128, X1..X128 = 129..256, Y1..Y128 = 301..428)\n',
		'globals().update({f\"A{i}\": i for i in range(1, 129)})\n',
		'globals().update({f\"X{i}\": 128 + i for i in range(1, 129)})\n',
		'globals().update({f\"Y{i}\": 300 + i for i in range(1, 129)})\n',
		RuntimeParserLine
		| PredCodes
	], '\n\n', PredSection),
...
python_project_predicates(Predicates, compiled(prolog_term_parser), ProjectPredicates) :-
	!,
	python_runtime_parser_predicates(ParserPredicates),
	append(Predicates, ParserPredicates, Combined),
	sort(Combined, ProjectPredicates).
python_project_predicates(Predicates, _RuntimeParserMode, Predicates).

python_runtime_parser_predicates(Predicates) :-

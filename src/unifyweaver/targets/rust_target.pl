...
direct_multi_call_recursion:compile_direct_multicall_pattern(rust, PredStr, BaseClauses, RecClause, RustCode) :-
    ...
    direct_computations_rust(Computations, ComputationsCode),
    direct_recursive_calls_rust(RecCalls, PredStr, RecCallsCode),
    direct_aggregation_rust(Aggregation, AggCode),
    format(string(RustCode),
...
direct_computations_rust(Computations, Code) :-
    findall(Line, (
        member(Var is Expr, Computations),
        direct_var_to_rust_name(Var, VarName),
        direct_translate_expr_rust(Expr, RustExpr),
        format(string(Line), '    let ~w: i64 = ~w;', [VarName, RustExpr])
    ), Lines),
    atomic_list_concat(Lines, '\n', Code).

direct_recursive_calls_rust(RecCalls, PredStr, Code) :-
    findall(Line, (
        member(RecCall, RecCalls),
        RecCall =.. [_Pred, ArgVar, ResultVar],
        direct_var_to_rust_name(ArgVar, ArgName),
        direct_var_to_rust_name(ResultVar, ResName),
        format(string(Line), '    let ~w: i64 = ~w(~w, memo);', [ResName, PredStr, ArgName])
    ), Lines),
    atomic_list_concat(Lines, '\n', Code).

direct_aggregation_rust(_ is Expr, Code) :-
    direct_translate_agg_expr_rust(Expr, RustExpr),
    format(string(Code), '    let result: i64 = ~w;', [RustExpr]).

direct_translate_agg_expr_rust(A + B, Expr) :-
    direct_var_to_rust_name(A, AName), direct_var_to_rust_name(B, BName),

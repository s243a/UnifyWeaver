...
compile_predicate_to_awk(PredIndicator, _Options, AwkCode) :-
    PredIndicator = Pred/Arity,
    functor(Head, Pred, Arity),
    findall(Head-Body, user:clause(Head, Body), Clauses),
    Clauses \= [],
    \+ (member(_-Body, Clauses), Body == true),
    native_awk_clause_body(Pred/Arity, Clauses, FuncBody),
    !,
    atom_string(Pred, PredStr),
    Arity1 is Arity - 1,
    build_awk_native_arg_list(Arity1, ArgList),
    format(string(AwkCode),
'#!/usr/bin/awk -f
# Generated by UnifyWeaver AWK Target - Native Clause Lowering
# Predicate: ~w/~w
...
%% build_awk_native_arg_list(+N, -ArgList)
build_awk_native_arg_list(0, "") :- !.
build_awk_native_arg_list(N, ArgList) :-
    findall(Arg, (
        between(1, N, I),
...
%% native_awk_clause_body(+PredSpec, +Clauses, -Code)

% Single clause
native_awk_clause_body(PredSpec, [Head-Body], Code) :-
    native_awk_clause(PredSpec, Head, Body, Condition, ClauseCode),
    !,
    (   Condition == "1"
    ->  format(string(Code), '    return ~w', [ClauseCode])
    ;   format(string(Code),
'    if (~w) return ~w
    print "No matching clause for ~w" > "/dev/stderr"
...
native_awk_clause_body(PredSpec, Clauses, Code) :-
    Clauses = [_|[_|_]],
    maplist(native_awk_clause_pair(PredSpec), Clauses, Branches),
    Branches \= [],
    branches_to_awk_if_chain(Branches, PredSpec, Code).

native_awk_clause_pair(PredSpec, Head-Body, branch(Condition, ClauseCode)) :-
    native_awk_clause(PredSpec, Head, Body, Condition, ClauseCode),
    !.

%% native_awk_clause(+PredSpec, +Head, +Body, -Condition, -Code)
native_awk_clause(_PredSpec, Head, Body, Condition, Code) :-
    Head =.. [_Pred|HeadArgs],
    length(HeadArgs, Arity),
    build_head_varmap(HeadArgs, 1, VarMap),
    (   Arity > 1
    ->  append(_InputHeadArgs, [OutputHeadArg], HeadArgs),
        awk_head_conditions(HeadArgs, 1, Arity, HeadConditions)
    ;   OutputHeadArg = _,
        awk_head_conditions(HeadArgs, 1, Arity, HeadConditions)
    ),
    normalize_goals(Body, Goals),
    (   Goals == []
    ->  awk_resolve_value(VarMap, OutputHeadArg, Code),
        GoalConditions = []
    ;   (   Arity > 1, nonvar(OutputHeadArg)
        ->  clause_guard_output_split(Goals, VarMap, GuardGoals, OutputGoals),
            maplist(awk_guard_condition(VarMap), GuardGoals, GoalConditions),
            (   OutputGoals == []
            ->  awk_literal(OutputHeadArg, Code)
            ;   awk_output_goals(OutputGoals, VarMap, Code)
            )
        ;   native_awk_goal_sequence(Goals, VarMap, GoalConditions, Code)
        )
    ),
    append(HeadConditions, GoalConditions, AllConditions),
    combine_awk_conditions(AllConditions, Condition).

%% awk_head_conditions(+HeadArgs, +Index, +Arity, -Conditions)
awk_head_conditions([], _, _, []).
awk_head_conditions([_], _, Arity, []) :- Arity > 1, !.
awk_head_conditions([HeadArg|Rest], Index, Arity, Conditions) :-
    (   var(HeadArg)
    ->  Conditions = RestConditions
    ;   format(string(ArgName), 'arg~w', [Index]),
        awk_literal(HeadArg, Literal),
...
awk_guard_condition(VarMap, Goal, Condition) :-
    ...
    expr_op(Op, StdOp),
    !,
    awk_expr(Left, VarMap, ALeft),
    awk_expr(Right, VarMap, ARight),
    awk_op(StdOp, AOp),
    format(string(Condition), '~w ~w ~w', [ALeft, AOp, ARight]).

%% awk_output_goals(+Goals, +VarMap, -Code)
awk_output_goals([], _VarMap, '"error"') :- !.
awk_output_goals([Goal], VarMap, Code) :-
    !, awk_output_goal_last(Goal, VarMap, Code).
awk_output_goals([_Goal|Rest], VarMap, Code) :-
    awk_output_goals(Rest, VarMap, Code).

%% awk_output_goal_last — produce the return expression
awk_output_goal_last(_Module:Goal, VarMap, Code) :-
    !, awk_output_goal_last(Goal, VarMap, Code).
awk_output_goal_last(Goal, VarMap, Code) :-
    if_then_else_goal(Goal, IfGoal, ThenGoal, ElseGoal),
    !,
    awk_if_then_else_output(IfGoal, ThenGoal, ElseGoal, VarMap, Code).
awk_output_goal_last(=(Var, Expr), VarMap, Code) :-
    var(Var), !,
    awk_expr(Expr, VarMap, Code).
awk_output_goal_last(is(Var, Expr), VarMap, Code) :-
    var(Var), !,
    awk_expr(Expr, VarMap, Code).

%% awk_if_then_else_output — generate AWK ternary expressions
awk_if_then_else_output(IfGoal, ThenGoal, ElseGoal, VarMap, Code) :-
    flatten_awk_if_branches(IfGoal, ThenGoal, ElseGoal, Branches, DefaultGoal),
    awk_branches_to_ternary(Branches, DefaultGoal, VarMap, Code).

flatten_awk_if_branches(If, Then, Else, [branch(If, Then)|RestBranches], Default) :-
    if_then_else_goal(Else, If2, Then2, Else2),
    !,
    flatten_awk_if_branches(If2, Then2, Else2, RestBranches, Default).
flatten_awk_if_branches(If, Then, Else, [branch(If, Then)], Else).

awk_branches_to_ternary([branch(If, Then)], DefaultGoal, VarMap, Code) :-
    !,
    awk_guard_condition(VarMap, If, IfCond),
    awk_branch_value(Then, VarMap, ThenVal),
    awk_branch_value(DefaultGoal, VarMap, ElseVal),
    format(string(Code), '(~w) ? ~w : ~w', [IfCond, ThenVal, ElseVal]).
awk_branches_to_ternary([branch(If, Then)|Rest], DefaultGoal, VarMap, Code) :-
    awk_guard_condition(VarMap, If, IfCond),
    awk_branch_value(Then, VarMap, ThenVal),
    awk_branches_to_ternary(Rest, DefaultGoal, VarMap, ElseCode),
...
awk_branch_value((A, B), VarMap, Value) :-
    ...
    last(Goals, LastGoal),
    awk_branch_value(LastGoal, VarMap, Value).
awk_branch_value(=(_, Expr), VarMap, Value) :-
    !, awk_expr(Expr, VarMap, Value).
awk_branch_value(is(_, Expr), VarMap, Value) :-
    !, awk_expr(Expr, VarMap, Value).
awk_branch_value(Goal, VarMap, Value) :-
...
awk_expr(Var, VarMap, AExpr) :-
    ...
    ->  atom_string(Name, AExpr)
    ;   AExpr = "0"
    ).
awk_expr(Expr, VarMap, AExpr) :-
    compound(Expr),
    Expr =.. [Op, Left, Right],
    expr_op(Op, StdOp),
    !,
    ...
    awk_expr(Right, VarMap, ARight),
    awk_op(StdOp, AOp),
    format(string(AExpr), '(~w ~w ~w)', [ALeft, AOp, ARight]).
awk_expr(-Expr, VarMap, AExpr) :-
    !,
    awk_expr(Expr, VarMap, Inner),
    format(string(AExpr), '(-~w)', [Inner]).
awk_expr(Atom, _VarMap, AExpr) :-
    atom(Atom), !,
    awk_literal(Atom, AExpr).
awk_expr(Number, _VarMap, AExpr) :-
    number(Number), !,
    format(string(AExpr), '~w', [Number]).

%% awk_literal — convert Prolog value to AWK literal
awk_literal(Value, '""') :- var(Value), !.
awk_literal(Value, ALit) :-
    number(Value), !,
    format(string(ALit), '~w', [Value]).
awk_literal(Value, ALit) :-
    atom(Value), !,
    format(string(ALit), '"~w"', [Value]).
awk_literal(Value, ALit) :-
...
%% awk_resolve_value — resolve head arg to AWK expression
awk_resolve_value(VarMap, Var, Value) :-
    var(Var), !,
    (   lookup_var(Var, VarMap, Name)
    ->  atom_string(Name, Value)
    ;   Value = "''"
    ).
awk_resolve_value(_VarMap, Atom, Value) :-
    atom(Atom), !,
    format(string(Value), '"~w"', [Atom]).
awk_resolve_value(_VarMap, Number, Value) :-
    number(Number), !,
    format(string(Value), '~w', [Number]).
awk_resolve_value(_VarMap, _, '""').

%% awk_op — map comparison/arithmetic operator to AWK
awk_op('>', '>').
awk_op('<', '<').
awk_op('>=', '>=').
awk_op('<=', '<=').
...
%% combine_awk_conditions(+Conditions, -Combined)
combine_awk_conditions([], "1") :- !.
combine_awk_conditions([C], C) :- !.
combine_awk_conditions([C|Rest], Combined) :-
    combine_awk_conditions(Rest, RestCombined),
    format(string(Combined), '~w && ~w', [C, RestCombined]).

%% branches_to_awk_if_chain(+Branches, +PredSpec, -Code)
branches_to_awk_if_chain(Branches, PredSpec, Code) :-
    awk_if_chain_lines(Branches, PredSpec, true, Lines),
    atomic_list_concat(Lines, '\n', Code).

awk_if_chain_lines([branch(Cond, ClauseCode)], PredSpec, IsFirst, Lines) :-
    !,
    (   Cond == "1"
    ->  (   IsFirst == true
        ->  format(string(Line), '    return ~w', [ClauseCode]),
            Lines = [Line]
        ;   format(string(Line), '    else return ~w', [ClauseCode]),
            Lines = [Line]
        )
    ;   (   IsFirst == true
        ->  format(string(L1), '    if (~w) return ~w', [Cond, ClauseCode]),
            format(string(L2), '    print "No matching clause for ~w" > "/dev/stderr"', [PredSpec]),
            format(string(L3), '    exit 1', []),
            Lines = [L1, L2, L3]
        ;   format(string(L1), '    else if (~w) return ~w', [Cond, ClauseCode]),
            format(string(L2), '    print "No matching clause for ~w" > "/dev/stderr"', [PredSpec]),
            format(string(L3), '    exit 1', []),
            Lines = [L1, L2, L3]
        )
    ).
awk_if_chain_lines([branch(Cond, ClauseCode)|Rest], PredSpec, IsFirst, [Line|RestLines]) :-
    (   IsFirst == true
    ->  format(string(Line), '    if (~w) return ~w', [Cond, ClauseCode])
    ;   format(string(Line), '    else if (~w) return ~w', [Cond, ClauseCode])
    ),
    awk_if_chain_lines(Rest, PredSpec, false, RestLines).

...
:- use_module('r_target', [compile_predicate_to_r/3, init_r_target/0]).
...
compile_generic_typr(Module, Pred/Arity, _Options, TypedMode, Code) :-
    ...
    build_typr_function(Module:Pred/Arity, TypedMode, Clauses, Code).
...
build_typr_function(Module:Pred/Arity, TypedMode, Clauses, Code) :-
    ...
    typr_function_body(Module:Pred/Arity, TypedMode, Clauses, ReturnType, Body),
    format(string(Code),
...
typr_function_body(_PredSpec, _TypedMode, [], "bool", "result <- false;\n\tresult").
typr_function_body(PredSpec, _TypedMode, Clauses, "bool", Body) :-
...
wrapped_r_body_expression(Module:Pred/Arity, WrappedExpr) :-
    !,
    init_r_target,
    compile_predicate_to_r(Module:Pred/Arity, [], RCode),
    r_function_body(RCode, Body),
    raw_arg_list(Arity, ArgList),
    format(string(WrappedExpr), '(function(~w) {\n~w\n})(~w)', [ArgList, Body, ArgList]).
wrapped_r_body_expression(Pred/Arity, WrappedExpr) :-
    wrapped_r_body_expression(user:Pred/Arity, WrappedExpr).

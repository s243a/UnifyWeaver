...
compile_predicate_to_typr(PredIndicator, Options, Code) :-
    ...
    ->  compile_typr_transitive_closure(Module, Pred/Arity, BasePred, TypedMode, Options, Code)
    ;   compile_typr_per_path_count_aggregate(Module:Pred/Arity, TypedMode, Options, Code)
...
typr_mutual_wrapper_code(GroupName, Specs, true, Spec, Code) :-
    ...
    typr_return_annotation(explicit, ReturnType, ReturnAnnotation),
    format(string(Code),
'let ~w <- fn(~w)~w {
...
\n~w
\tresult
};', [PredStr, TypedArgList, ReturnAnnotation, GroupName, IndentedHelpersText, ResultLine]).
...
\n~w
\tresult
};', [PredStr, TypedArgList, ReturnAnnotation, IndentedHelpersText, ResultLine]).
...
compile_typr_tail_recursive_accumulator(_Module:Pred/Arity, TypedMode, Clauses, Code) :-
    ...
    typr_return_annotation(TypedMode, ReturnType, ReturnAnnotation),
    format(string(Code),
'# Generated by UnifyWeaver TypR Target
# Predicate: ~w/~w
...
', [PredStr, Arity, PredStr, TypedArgList, ReturnAnnotation, IndentedBody]).
...
compile_typr_per_path_count_aggregate(Module:Pred/Arity, TypedMode, Options, Code) :-
    findall(Head-Body, predicate_clause(Module, Pred, Arity, Head, Body), Clauses),
    Clauses = [Head-Body],
    classify_aggregate(Body, AggInfo),
    get_dict(op, AggInfo, count),
    get_dict(type, AggInfo, all),
    get_dict(goal_info, AggInfo, GoalInfo),
    get_dict(relations, GoalInfo, [InnerGoal]),
    get_dict(arithmetic, GoalInfo, []),
    get_dict(constraints, GoalInfo, []),
    get_dict(other, GoalInfo, []),
    get_dict(result, AggInfo, ResultVar),
    functor(InnerGoal, InnerPred, InnerArity),
    findall(InnerHead-InnerBody, predicate_clause(Module, InnerPred, InnerArity, InnerHead, InnerBody), InnerClauses),
    InnerClauses \= [],
    compile_typr_per_path_visited(Module:InnerPred/InnerArity, TypedMode, InnerClauses, Options, InnerCode),
    findall((InnerClauseHead, InnerClauseBody), member(InnerClauseHead-InnerClauseBody, InnerClauses), InnerClausePairs),
    is_per_path_visited_pattern(InnerPred, InnerArity, InnerClausePairs, VisitedPos),
    typr_per_path_visited_io_positions(InnerPred/InnerArity, VisitedPos, InputPositions, _OutputPositions),
    Head =.. [_|HeadArgs],
    typr_per_path_count_head_varmap(HeadArgs, HeadVarMap),
    typr_per_path_count_output_name(ResultVar, HeadVarMap, ResultName),
    InnerGoal =.. [_|InnerGoalArgs],
    InputPositions = [DriverPos|_],
    nth1(DriverPos, InnerGoalArgs, DriverArg),
    nth1(VisitedPos, InnerGoalArgs, VisitedArg),
    typr_per_path_count_visited_seed(VisitedArg, DriverArg),
    maplist(typr_per_path_count_call_arg_expr(HeadVarMap, InnerGoalArgs), InputPositions, CallArgExprs),
    atom_string(Pred, PredStr),
    atom_string(InnerPred, InnerPredStr),
    build_typed_arg_list(Pred/Arity, none, Arity, TypedMode, TypedArgList),
    typr_return_annotation(TypedMode, integer, ReturnAnnotation),
    typr_native_call_expression(InnerPredStr, CallArgExprs, InnerCallExpr),
    typr_assignment_line(new, "rows", InnerCallExpr, RowsLine),
    typr_assignment_line(existing, ResultName, "length(rows)", ResultLine),
    atomic_list_concat([RowsLine, ResultLine, ResultName], '\n', BodyText),
    indent_text(BodyText, "\t", IndentedBody),
    format(string(Code),
'# Generated by UnifyWeaver TypR Target - Per-Path Count Aggregate
# Predicate: ~w/~w
# Inner predicate: ~w/~w
...
', [PredStr, Arity, InnerPredStr, InnerArity, InnerCode, PredStr, TypedArgList, ReturnAnnotation, IndentedBody]).

typr_per_path_count_head_varmap(HeadArgs, VarMap) :-
    typr_per_path_count_head_varmap(HeadArgs, 1, [], VarMap).

typr_per_path_count_head_varmap([], _Index, VarMap, VarMap).
typr_per_path_count_head_varmap([Arg|Rest], Index, VarMap0, VarMapOut) :-
    (   var(Arg),
        \+ varmap_contains_var(VarMap0, Arg)
    ->  format(string(ArgName), 'arg~w', [Index]),
        VarMap1 = [Arg-ArgName|VarMap0]
    ;   VarMap1 = VarMap0
    ),
    NextIndex is Index + 1,
    typr_per_path_count_head_varmap(Rest, NextIndex, VarMap1, VarMapOut).

typr_per_path_count_output_name(ResultVar, HeadVarMap, ResultName) :-
    lookup_typr_var(ResultVar, HeadVarMap, ResultName).

typr_per_path_count_visited_seed([DriverArg], DriverArg).

typr_per_path_count_call_arg_expr(HeadVarMap, GoalArgs, Position, Expr) :-
    nth1(Position, GoalArgs, Arg),
    typr_per_path_count_resolve_value(HeadVarMap, Arg, Expr).

typr_per_path_count_resolve_value(VarMap, Value, Expr) :-
    (   var(Value)
    ->  typr_resolve_value(VarMap, Value, Expr)
    ;   atomic(Value)
    ->  typr_resolve_value(VarMap, Value, Expr)
    ).
...
annotation_suffix(Mode, TypeTerm, Annotation) :-
    ...
    ->  true
    ;   ConcreteType = TypeTerm
    ),
    format(string(Annotation), ": ~w", [ConcreteType]).
annotation_suffix(_, _, "").

typr_return_annotation(Mode, TypeTerm, Annotation) :-
    annotation_suffix(Mode, TypeTerm, Annotation).
...
', [PredStr, Arity, PredStr, TypedArgList, ReturnAnnotation, Body]).

...
target_info(info{
    ...
    recursion_patterns: [transitive_closure, linear_recursion, tail_recursion, multicall_linear_recursion, direct_multi_call],
...
% ============================================================================
% TAIL RECURSION - TypR target (delegates to R, rewrites header)
% ============================================================================

:- use_module('../core/advanced/tail_recursion').
:- multifile tail_recursion:compile_tail_pattern/9.

tail_recursion:compile_tail_pattern(typr, PredStr, Arity, BaseClauses, RecClauses, AccPos, StepOp, ExitAfterResult, TypRCode) :-
    tail_recursion:compile_tail_pattern(r, PredStr, Arity, BaseClauses, RecClauses, AccPos, StepOp, ExitAfterResult, RCode),
    r_code_to_typr(PredStr, "tail recursion", RCode, TypRCode).

% ============================================================================
% LINEAR RECURSION - TypR target (delegates to R, rewrites header)
% ============================================================================

:- use_module('../core/advanced/linear_recursion').
:- multifile linear_recursion:compile_linear_pattern/8.

linear_recursion:compile_linear_pattern(typr, PredStr, Arity, BaseClauses, RecClauses, MemoEnabled, MemoStrategy, TypRCode) :-
    linear_recursion:compile_linear_pattern(r, PredStr, Arity, BaseClauses, RecClauses, MemoEnabled, MemoStrategy, RCode),
    r_code_to_typr(PredStr, "linear recursion", RCode, TypRCode).

% ============================================================================
% MULTICALL LINEAR RECURSION - TypR target (delegates to R, rewrites header)
% ============================================================================

:- use_module('../core/advanced/multicall_linear_recursion').
:- multifile multicall_linear_recursion:compile_multicall_pattern/6.

multicall_linear_recursion:compile_multicall_pattern(typr, PredStr, BaseClauses, RecClauses, MemoEnabled, TypRCode) :-
    multicall_linear_recursion:compile_multicall_pattern(r, PredStr, BaseClauses, RecClauses, MemoEnabled, RCode),
    r_code_to_typr(PredStr, "multicall recursion", RCode, TypRCode).

% ============================================================================
% DIRECT MULTI-CALL RECURSION - TypR target (delegates to R, rewrites header)
% ============================================================================
...
direct_multi_call_recursion:compile_direct_multicall_pattern(typr, PredStr, BaseClauses, RecClause, TypRCode) :-
    direct_multi_call_recursion:compile_direct_multicall_pattern(r, PredStr, BaseClauses, RecClause, RCode),
    r_code_to_typr(PredStr, "direct multicall recursion", RCode, TypRCode).

% ============================================================================
% Helper: Convert R-generated code to TypR by replacing the header comment
% ============================================================================

%% r_code_to_typr(+PredStr, +PatternDesc, +RCode, -TypRCode)
%  Takes R code generated by an R-target handler and rewrites the first
%  comment line to indicate TypR target. The body is valid R which TypR
%  can transpile.
r_code_to_typr(PredStr, PatternDesc, RCode, TypRCode) :-
    (   sub_string(RCode, Before, _, _, "\n")
    ->  sub_string(RCode, 0, Before, _, _FirstLine),
        sub_string(RCode, Before, _, 0, RestOfCode),
        format(string(TypRHeader), '# ~w - ~w (TypR, transpiles to R)', [PredStr, PatternDesc]),
        string_concat(TypRHeader, RestOfCode, TypRCode)
    ;   TypRCode = RCode
    ).

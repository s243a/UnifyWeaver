...
:- use_module('../core/clause_body_analysis', [
    ...
    head_conditions/3,
    build_head_varmap/3
    % lookup_var/3 and ensure_var/4 excluded — bash_target has local versions
]).
...
%  Tries native clause body lowering first (multi-clause → if/elif/else),
%  then falls back to single-clause binding-based compilation.
compile_predicate_to_bash(PredIndicator, Options, BashCode) :-
    PredIndicator = Pred/Arity,
    functor(Head, Pred, Arity),
    findall(HC-BC, (clause(user:Head, Body), copy_term((Head,Body),(HC,BC))), Clauses),
    %% Try native clause body lowering first
    (   compile_native_bash(Pred, Arity, Clauses, Options, BashCode)
    ->  true
    ;   Clauses = [H-B]
    ->  compile_rule(H, B, BashCode)
...
%% ============================================================================
%% BASH RETURN METHOD SYSTEM
%% ============================================================================
%%
%% Controls how native-lowered Bash functions return values.
%%
%% Return methods:
%%   echo              — echo to stdout; caller captures with $() (default)
%%   global(VarFmt)    — set global variable; VarFmt is format string
%%                       e.g. '~w_return' → classify_return
%%   nameref           — use Bash 4.3+ nameref (declare -n) on last arg
%%   assoc(ArrayName)  — store in associative array keyed by input args
%%
%% Modes:
%%   requires(Method)  — MUST use this method; fail if not possible
%%   preferred(Method) — use if possible; fall back to default (echo)
%%
%% Usage: assert bash_return_spec/2 before compilation to override defaults.
%%   e.g. assert(bash_target:bash_return_spec(classify/2, requires(global('~w_return'))))

:- dynamic bash_return_spec/2.  % +Pred/Arity, +Spec (requires(Method) | preferred(Method))

%% bash_effective_return_method(+Pred/Arity, +Options, -Method)
%  Resolve the effective return method for a predicate.
bash_effective_return_method(PredSpec, Options, Method) :-
    %% Check options first
    (   member(return_method(M), Options)
    ->  Method = M
    ;   bash_return_spec(PredSpec, Spec)
    ->  (   Spec = requires(Method) -> true
        ;   Spec = preferred(Method) -> true
        ;   Method = echo
        )
    ;   Method = echo  % Default: streaming-compatible
    ).

%% bash_return_var_name(+PredStr, +Method, -VarName)
%  Generate the return variable name for global/assoc methods.
bash_return_var_name(PredStr, global(Format), VarName) :-
    !,
    format(atom(VarName), Format, [PredStr]).
bash_return_var_name(PredStr, global, VarName) :-
    !,
    format(atom(VarName), '~w_return', [PredStr]).
bash_return_var_name(_, _, _) :- fail.

%% ============================================================================
%% NATIVE CLAUSE BODY LOWERING (via clause_body_analysis)
%% ============================================================================

%% compile_native_bash(+Pred, +Arity, +Clauses, +Options, -Code)
%  Generate Bash function with if/elif/else chain from multi-clause predicates.
compile_native_bash(Pred, Arity, Clauses, Options, Code) :-
    Clauses \= [],
    %% Skip pure facts — existing handlers are better
    \+ forall(member(_-Body, Clauses), Body == true),
    atom_string(Pred, PredStr),
    bash_effective_return_method(Pred/Arity, Options, ReturnMethod),
    native_bash_clause_body(PredStr, Arity, Clauses, ReturnMethod, Body),
    ArgCount is Arity - 1,  % last arg is output
    bash_arg_init(ArgCount, ArgInit),
    bash_function_signature(PredStr, ReturnMethod, Arity, SigExtra),
    format(atom(Code),
'#!/usr/bin/env bash
# Generated by UnifyWeaver Bash Target - Native Clause Lowering
# Predicate: ~w/~w
# Return method: ~w
...
', [PredStr, Arity, ReturnMethod, PredStr, ArgInit, SigExtra, Body]).

bash_arg_init(0, "") :- !.
bash_arg_init(N, ArgInit) :-
    numlist(1, N, Indices),
    maplist([I, Line]>>(format(atom(Line), '    local arg~w="$~w"', [I, I])), Indices, Lines),
    atomic_list_concat(Lines, '\n', ArgInit0),
    format(atom(ArgInit), '~w\n', [ArgInit0]).

%% Extra declarations for return methods
bash_function_signature(_, echo, _, "") :- !.
bash_function_signature(_, nameref, Arity, Extra) :-
    !,
    format(atom(Extra), '    local -n __result="$~w"\n', [Arity]).
bash_function_signature(PredStr, global(Fmt), _, Extra) :-
    !,
    bash_return_var_name(PredStr, global(Fmt), _VarName),
    Extra = "".  % global var is set directly, no extra init needed
bash_function_signature(PredStr, global, _, Extra) :-
    !,
    bash_return_var_name(PredStr, global, _VarName),
    Extra = "".
bash_function_signature(_, _, _, "").

%% native_bash_clause_body(+PredStr, +Arity, +Clauses, +ReturnMethod, -Code)
native_bash_clause_body(PredStr, _Arity, [Head-Body], ReturnMethod, Code) :-
    native_bash_clause(Head, Body, Condition, ClauseExpr),
    !,
    bash_return_statement(ReturnMethod, PredStr, ClauseExpr, ReturnStmt),
    (   Condition == 'TRUE'
    ->  format(atom(Code), '~w', [ReturnStmt])
    ;   format(atom(Code),
'    if ~w; then
    ~w
    else
        echo "Error: No matching clause for ~w" >&2
        return 1
    fi', [Condition, ReturnStmt, PredStr])
    ).
native_bash_clause_body(PredStr, _Arity, Clauses, ReturnMethod, Code) :-
    maplist(native_bash_clause_pair, Clauses, Branches),
    Branches \= [],
    branches_to_bash_if_chain(Branches, ReturnMethod, PredStr, IfChain),
    format(atom(Code),
'~w
    else
        echo "Error: No matching clause for ~w" >&2
...
native_bash_clause_pair(Head-Body, branch(Condition, ClauseExpr)) :-
    native_bash_clause(Head, Body, Condition, ClauseExpr),
    !.

%% native_bash_clause(+Head, +Body, -Condition, -Expr)
native_bash_clause(Head, Body, Condition, Expr) :-
    Head =.. [_|HeadArgs],
    length(HeadArgs, Arity),
    clause_body_analysis:build_head_varmap(HeadArgs, 1, VarMap),
    %% Separate output arg (last) from input args
    (   Arity > 1
    ->  append(_, [OutputHeadArg], HeadArgs),
        bash_head_conditions(HeadArgs, 1, Arity, HeadConditions)
    ;   OutputHeadArg = _,
        bash_head_conditions(HeadArgs, 1, Arity, HeadConditions)
    ),
    clause_body_analysis:normalize_goals(Body, Goals),
    (   Goals == []
    ->  bash_resolve_value(VarMap, OutputHeadArg, Expr),
        GoalConditions = []
    ;   (   Arity > 1, nonvar(OutputHeadArg)
        ->  clause_body_analysis:clause_guard_output_split(Goals, VarMap, GuardGoals, OutputGoals),
            maplist(bash_guard_condition(VarMap), GuardGoals, GoalConditions),
            (   OutputGoals == []
            ->  bash_literal(OutputHeadArg, Expr)
            ;   bash_output_code(OutputGoals, VarMap, Expr)
            )
        ;   bash_goal_sequence(Goals, VarMap, GoalConditions, Expr)
        )
    ),
    append(HeadConditions, GoalConditions, AllConditions),
    combine_bash_conditions(AllConditions, Condition).

bash_goal_sequence(Goals, VarMap, Conditions, Code) :-
    clause_body_analysis:classify_goal_sequence(Goals, VarMap, ClassifiedGoals),
    ClassifiedGoals \= [],
    bash_render_classified_last(ClassifiedGoals, VarMap, Conditions, Code),
    !.
bash_goal_sequence(Goals, VarMap, Conditions, Code) :-
    clause_body_analysis:clause_guard_output_split(Goals, VarMap, GuardGoals, OutputGoals),
    maplist(bash_guard_condition(VarMap), GuardGoals, Conditions),
    bash_output_code(OutputGoals, VarMap, Code).

bash_render_classified_last([output_ite(If, Then, Else, _)], VarMap, [], Code) :- !,
    bash_guard_condition(VarMap, If, Cond),
    bash_ite_value(Then, VarMap, ThenVal),
    bash_ite_value(Else, VarMap, ElseVal),
    format(atom(Code), '$(if ~w; then echo ~w; else echo ~w; fi)', [Cond, ThenVal, ElseVal]).
bash_render_classified_last([output(Goal, _, _)], VarMap, [], Code) :- !,
    bash_output_code([Goal], VarMap, Code).
bash_render_classified_last([guard(Goal, _)|Rest], VarMap, [Cond|RestConds], Code) :- !,
    bash_guard_condition(VarMap, Goal, Cond),
    bash_render_classified_last(Rest, VarMap, RestConds, Code).
bash_render_classified_last(_, _, [], "\"\"").

%% bash_head_conditions — exclude last arg (output position)
bash_head_conditions([], _, _, []).
bash_head_conditions([_], _, Arity, []) :- Arity > 1, !.
bash_head_conditions([HeadArg|Rest], Index, Arity, Conditions) :-
    (   var(HeadArg)
    ->  Conditions = RestConditions
    ;   format(atom(ArgName), "$arg~w", [Index]),
        bash_literal(HeadArg, Literal),
        (   number(HeadArg)
        ->  format(atom(Cond), '(( arg~w == ~w ))', [Index, Literal])
        ;   format(atom(Cond), '[[ ~w == ~w ]]', [ArgName, Literal])
        ),
        Conditions = [Cond|RestConditions]
    ),
    NextIndex is Index + 1,
    bash_head_conditions(Rest, NextIndex, Arity, RestConditions).

bash_resolve_value(VarMap, Value, Code) :-
    (   var(Value)
    ->  clause_body_analysis:lookup_var(Value, VarMap, VarName),
        format(atom(Code), '"$~w"', [VarName])
    ;   bash_literal(Value, Code)
    ).

%% bash_guard_condition(+VarMap, +Goal, -CondStr)
bash_guard_condition(VarMap, Goal, CondStr) :-
    Goal =.. [Op, Left, Right],
    bash_cmp_op(Op, BashOp),
    bash_arith_expr(Left, VarMap, LeftStr),
    bash_arith_expr(Right, VarMap, RightStr),
    format(atom(CondStr), '(( ~w ~w ~w ))', [LeftStr, BashOp, RightStr]).
...
%% bash_arith_expr(+Expr, +VarMap, -Str)
%  Generates Bash arithmetic expressions (used inside $(( ... ))).
bash_arith_expr(Var, VarMap, Str) :-
    var(Var), !,
    clause_body_analysis:lookup_var(Var, VarMap, VarName),
    atom_string(VarName, Str).
bash_arith_expr(Expr, VarMap, Str) :-
    Expr =.. [Op, Left, Right],
    bash_arith_op(Op, BashOp),
    !,
    bash_arith_expr(Left, VarMap, LeftStr),
    bash_arith_expr(Right, VarMap, RightStr),
    format(atom(Str), "(~w ~w ~w)", [LeftStr, BashOp, RightStr]).
bash_arith_expr(abs(X), VarMap, Str) :-
    !,
    bash_arith_expr(X, VarMap, XStr),
    format(atom(Str), "(~w < 0 ? -~w : ~w)", [XStr, XStr, XStr]).
bash_arith_expr(-(X), VarMap, Str) :-
    !,
    bash_arith_expr(X, VarMap, XStr),
    format(atom(Str), "(-~w)", [XStr]).
bash_arith_expr(Value, _VarMap, Str) :-
...
%% bash_output_code(+OutputGoals, +VarMap, -Code)
bash_output_code([Goal|_], VarMap, Code) :-
    Goal = (_Result is Expr),
    !,
    bash_arith_expr(Expr, VarMap, ExprStr),
    format(atom(Code), '$(( ~w ))', [ExprStr]).
bash_output_code([Goal|_], VarMap, Code) :-
    Goal = (_Result = Value),
    !,
    bash_resolve_value(VarMap, Value, Code).
bash_output_code([Goal|_], VarMap, Code) :-
    Goal = (Cond -> Then ; Else),
    !,
    bash_guard_condition(VarMap, Cond, CondStr),
    bash_ite_value(Then, VarMap, ThenStr),
    bash_ite_value(Else, VarMap, ElseStr),
    format(atom(Code), '$(if ~w; then echo ~w; else echo ~w; fi)', [CondStr, ThenStr, ElseStr]).
bash_output_code([], _VarMap, "\"\"").

%% bash_ite_value(+Branch, +VarMap, -Str)
bash_ite_value(_Result = Value, VarMap, Str) :- !,
    bash_resolve_value(VarMap, Value, Str).
bash_ite_value(_Result is Expr, VarMap, Str) :- !,
    bash_arith_expr(Expr, VarMap, ExprStr),
    format(atom(Str), '"$(( ~w ))"', [ExprStr]).
bash_ite_value((Cond -> Then ; Else), VarMap, Str) :- !,
    bash_guard_condition(VarMap, Cond, CondStr),
    bash_ite_value(Then, VarMap, ThenStr),
    bash_ite_value(Else, VarMap, ElseStr),
    format(atom(Str), '"$(if ~w; then echo ~w; else echo ~w; fi)"', [CondStr, ThenStr, ElseStr]).
bash_ite_value(Value, _VarMap, Str) :-
    bash_literal(Value, Str).

combine_bash_conditions([], 'TRUE').
combine_bash_conditions(Conditions, Combined) :-
...
%% bash_return_statement(+Method, +PredStr, +Expr, -Statement)
%  Generate the return statement based on the configured return method.
bash_return_statement(echo, _PredStr, Expr, Stmt) :-
    format(atom(Stmt), '    echo ~w', [Expr]).
bash_return_statement(global(Fmt), PredStr, Expr, Stmt) :-
    bash_return_var_name(PredStr, global(Fmt), VarName),
    format(atom(Stmt), '    ~w=~w', [VarName, Expr]).
bash_return_statement(global, PredStr, Expr, Stmt) :-
    bash_return_var_name(PredStr, global, VarName),
    format(atom(Stmt), '    ~w=~w', [VarName, Expr]).
bash_return_statement(nameref, _PredStr, Expr, Stmt) :-
    format(atom(Stmt), '    __result=~w', [Expr]).
bash_return_statement(assoc(ArrayName), _PredStr, Expr, Stmt) :-
    format(atom(Stmt), '    ~w["$*"]=~w', [ArrayName, Expr]).

%% branches_to_bash_if_chain(+Branches, +ReturnMethod, +PredStr, -Code)
%  First branch uses 'if', subsequent use 'elif'.
branches_to_bash_if_chain([First|Rest], ReturnMethod, PredStr, Code) :-
    First = branch(Cond, Expr),
    bash_return_statement(ReturnMethod, PredStr, Expr, ReturnStmt),
    (   Rest == []
    ->  format(atom(Code), '    if ~w; then\n    ~w', [Cond, ReturnStmt])
    ;   branches_to_bash_elif_chain(Rest, ReturnMethod, PredStr, ElifCode),
        format(atom(Code), '    if ~w; then\n    ~w\n~w', [Cond, ReturnStmt, ElifCode])
    ).

branches_to_bash_elif_chain([branch(Cond, Expr)], ReturnMethod, PredStr, Code) :-
    !,
    bash_return_statement(ReturnMethod, PredStr, Expr, ReturnStmt),
    format(atom(Code), '    elif ~w; then\n    ~w', [Cond, ReturnStmt]).
branches_to_bash_elif_chain([branch(Cond, Expr)|Rest], ReturnMethod, PredStr, Code) :-
    bash_return_statement(ReturnMethod, PredStr, Expr, ReturnStmt),
    branches_to_bash_elif_chain(Rest, ReturnMethod, PredStr, RestCode),
    format(atom(Code), '    elif ~w; then\n    ~w\n~w', [Cond, ReturnStmt, RestCode]).

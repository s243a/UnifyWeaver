...
compile_component(Name, Config, _Options, Code) :-
    ...
    % Precompute the title-display flag; an if-then-else term passed directly
    % into a format/3 argument list is NOT evaluated, it is printed verbatim.
    (   Title \= ""
    ->  TitleDisplay = "true"
    ;   TitleDisplay = "false"
    ),
...
", [NameStr, NameStr, ChartTypeStr, NameStr, ChartTypeStr, ResponsiveStr,
    TitleDisplay, Title, XAxisCode, YAxisCode,

...
:- module(sql_target, [
    ...
    compile_facts_to_sql/3,         % +Pred, +Arity, -SQLCode  -- NEW
...
%% ============================================
%% FACT EXPORT - compile_facts_to_sql/3
%% ============================================

%% compile_facts_to_sql(+Pred, +Arity, -SQLCode)
%  Export Prolog facts as SQL INSERT statements.
%  Generates CREATE TABLE and INSERT statements.
%
%  Example:
%    ?- compile_facts_to_sql(parent, 2, Code).
%    Generates:
...
%
compile_facts_to_sql(Pred, Arity, SQLCode) :-
    % Get the predicate name as string
    atom_string(Pred, PredStr),
    
    % Generate column names
    findall(ColName, (
        between(1, Arity, N),
        format(atom(ColName), 'arg~w TEXT', [N])
    ), Columns),
    atomic_list_concat(Columns, ', ', ColumnDefs),
    ...
    findall(InsertStmt, (
        ...
        maplist(format_sql_value, Args, FormattedArgs),
        atomic_list_concat(FormattedArgs, ', ', ValuesStr),
        format(string(InsertStmt), 'INSERT INTO ~w VALUES (~w);', [PredStr, ValuesStr])
    ), InsertStmts),
    atomic_list_concat(InsertStmts, '\n', InsertsCode),
    
    % Generate full SQL
    format(string(SQLCode),
'-- Generated by UnifyWeaver SQL Target - Fact Export
-- Predicate: ~w/~w
...
-- Query all facts
SELECT * FROM ~w;
', [PredStr, Arity, PredStr, ColumnDefs, InsertsCode, PredStr]).

%% format_sql_value(+Arg, -Formatted)
%  Format a single fact argument as an SQL string literal.
format_sql_value(Arg, Formatted) :-
    (   atom(Arg)

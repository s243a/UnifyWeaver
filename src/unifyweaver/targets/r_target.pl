...
compile_predicate_to_r(PredIndicator, Options, RCode) :-
    ...
    %% Try native clause lowering first
    (   Clauses \= [],
        compile_native_r(Pred, Arity, Clauses, RCode)
    ->  true
    ;   %% Fall back to original compilation
...
%% ============================================
%% NATIVE CLAUSE LOWERING (clause_body_analysis)
%% ============================================

compile_native_r(Pred, Arity, Clauses, Code) :-
    atom_string(Pred, PredStr),
    native_r_clause_body(PredStr/Arity, Clauses, FuncBody),
    !,
    Arity1 is Arity - 1,
    r_param_list(Arity1, ParamList),
    format(string(Code),
'# Generated by UnifyWeaver R Target - Native Clause Lowering
# Predicate: ~w/~w

~w <- function(~w) {
~w
}
', [PredStr, Arity, PredStr, ParamList, FuncBody]).

r_param_list(0, "") :- !.
r_param_list(N, ParamList) :-
    findall(Param, (
        between(1, N, I),
        format(string(Param), 'arg~w', [I])
    ), Params),
    atomic_list_concat(Params, ', ', ParamList).

%% native_r_clause_body — single clause
native_r_clause_body(PredSpec, [Head-Body], Code) :-
    native_r_clause(PredSpec, Head, Body, Condition, ClauseCode),
    !,
    (   Condition == "true"
...
%% native_r_clause_body — multi-clause
native_r_clause_body(PredSpec, Clauses, Code) :-
    Clauses = [_,_|_],
    maplist(native_r_clause_pair(PredSpec), Clauses, Branches),
    Branches \= [],
    branches_to_r_if_chain(Branches, PredSpec, Code).

native_r_clause_pair(PredSpec, Head-Body, branch(Condition, ClauseCode)) :-
    native_r_clause(PredSpec, Head, Body, Condition, ClauseCode), !.

%% native_r_clause/5
native_r_clause(_PredSpec, Head, Body, Condition, Code) :-
    Head =.. [_|HeadArgs],
    length(HeadArgs, Arity),
    clause_body_analysis:build_head_varmap(HeadArgs, 1, VarMap),
    (   Arity > 1
    ->  append(_, [OutputHeadArg], HeadArgs),
        r_head_conditions(HeadArgs, 1, Arity, HeadConditions)
    ;   OutputHeadArg = _,
        r_head_conditions(HeadArgs, 1, Arity, HeadConditions)
    ),
    clause_body_analysis:normalize_goals(Body, Goals),
    (   Goals == []
    ->  r_resolve_value(VarMap, OutputHeadArg, Code),
        GoalConditions = []
    ;   (   Arity > 1, nonvar(OutputHeadArg)
        ->  clause_body_analysis:clause_guard_output_split(Goals, VarMap, GuardGoals, OutputGoals),
            maplist(r_guard_condition(VarMap), GuardGoals, GoalConditions),
            (   OutputGoals == []
            ->  r_literal(OutputHeadArg, Code)
            ;   r_output_goals(OutputGoals, VarMap, Code)
            )
        ;   native_r_goal_sequence(Goals, VarMap, GoalConditions, Code)
        )
    ),
    append(HeadConditions, GoalConditions, AllConditions),
    combine_r_conditions(AllConditions, Condition).

native_r_goal_sequence(Goals, VarMap, Conditions, Code) :-
    clause_body_analysis:classify_goal_sequence(Goals, VarMap, ClassifiedGoals),
    ClassifiedGoals \= [],
    r_render_classified_last(ClassifiedGoals, VarMap, Conditions, Code),
    !.
native_r_goal_sequence(Goals, VarMap, Conditions, Code) :-
    clause_body_analysis:clause_guard_output_split(Goals, VarMap, GuardGoals, OutputGoals),
    maplist(r_guard_condition(VarMap), GuardGoals, Conditions),
    r_output_goals(OutputGoals, VarMap, Code).

r_render_classified_last([output_ite(If, Then, Else, _)], VarMap, [], Code) :- !,
    r_guard_condition(VarMap, If, Cond),
    r_branch_value(Then, VarMap, ThenVal),
    r_branch_value(Else, VarMap, ElseVal),
    format(string(Code), 'ifelse(~w, ~w, ~w)', [Cond, ThenVal, ElseVal]).
r_render_classified_last([output(Goal, _, _)], VarMap, [], Code) :- !,
    r_output_goals([Goal], VarMap, Code).
r_render_classified_last([guard(Goal, _)|Rest], VarMap, [Cond|RestConds], Code) :- !,
    r_guard_condition(VarMap, Goal, Cond),
    r_render_classified_last(Rest, VarMap, RestConds, Code).
r_render_classified_last(_, _, [], "NULL").

%% r_head_conditions
r_head_conditions([], _, _, []).
r_head_conditions([_], _, Arity, []) :- Arity > 1, !.
r_head_conditions([HeadArg|Rest], Index, Arity, Conditions) :-
    (   var(HeadArg)
    ...
    r_head_conditions(Rest, NextIndex, Arity, RestConditions).

%% r_guard_condition
r_guard_condition(VarMap, Goal, Condition) :-
    compound(Goal),
    Goal =.. [Op, Left, Right],
    clause_body_analysis:expr_op(Op, StdOp),
    !,
    r_expr(Left, VarMap, RLeft),
    r_expr(Right, VarMap, RRight),
    r_cmp_op(StdOp, ROp),
    format(string(Condition), '~w ~w ~w', [RLeft, ROp, RRight]).

%% r_output_goals
r_output_goals(Outputs, VarMap, Value) :-
    last(Outputs, LastGoal),
    (   clause_body_analysis:goal_output_var(LastGoal, _)
    ->  r_goal_expr(LastGoal, VarMap, Value)
    ;   r_resolve_value(VarMap, LastGoal, Value)
    ).

r_goal_expr(Goal, VarMap, Expr) :-
    Goal = (_Var = RHS), !, r_expr(RHS, VarMap, Expr).
r_goal_expr(Goal, VarMap, Expr) :-
    Goal = (_Var is ArithExpr), !, r_expr(ArithExpr, VarMap, Expr).
r_goal_expr(Goal, VarMap, Expr) :-
    r_resolve_value(VarMap, Goal, Expr).

%% r_expr
r_expr(Var, VarMap, RExpr) :-
    var(Var), !,
    lookup_var(Var, VarMap, Name),
    format(string(RExpr), '~w', [Name]).
r_expr(Expr, _VarMap, RExpr) :-
    number(Expr), !,
    format(string(RExpr), '~w', [Expr]).
r_expr(Expr, VarMap, RExpr) :-
    atom(Expr), lookup_var(Expr, VarMap, Name), !,
    format(string(RExpr), '~w', [Name]).
r_expr(Expr, _VarMap, RExpr) :-
    atom(Expr), !, r_literal(Expr, RExpr).
r_expr(-Expr, VarMap, RExpr) :-
    !, r_expr(Expr, VarMap, Inner),
    format(string(RExpr), '(-~w)', [Inner]).
r_expr(abs(Expr), VarMap, RExpr) :-
    !, r_expr(Expr, VarMap, Inner),
    format(string(RExpr), 'abs(~w)', [Inner]).
r_expr(Expr, VarMap, RExpr) :-
    compound(Expr),
    Expr =.. [Op, Left, Right],
    clause_body_analysis:expr_op(Op, _),
    !,
    r_expr(Left, VarMap, RLeft),
    r_expr(Right, VarMap, RRight),
    r_arith_op(Op, ROp),
...
%% r_resolve_value
r_resolve_value(VarMap, Var, Value) :-
    var(Var), !,
    lookup_var(Var, VarMap, Name),
    format(string(Value), '~w', [Name]).
r_resolve_value(_VarMap, Term, Value) :-
    number(Term), !,
    format(string(Value), '~w', [Term]).
r_resolve_value(VarMap, Term, Value) :-
    atom(Term), lookup_var(Term, VarMap, Name), !,
    format(string(Value), '~w', [Name]).
r_resolve_value(_VarMap, Term, Value) :-
    atom(Term), !, r_literal(Term, Value).
r_resolve_value(VarMap, Term, Value) :-
    r_expr(Term, VarMap, Value).

%% r_branch_value
r_branch_value(Goal, VarMap, Value) :-
    clause_body_analysis:if_then_else_goal(Goal, If, Then, Else), !,
    r_guard_condition(VarMap, If, Cond),
    r_branch_value(Then, VarMap, ThenVal),
    r_branch_value(Else, VarMap, ElseVal),
    format(string(Value), 'ifelse(~w, ~w, ~w)', [Cond, ThenVal, ElseVal]).
r_branch_value((A, B), VarMap, Value) :-
    !, clause_body_analysis:normalize_goals((A, B), Goals), last(Goals, Last),
    r_branch_value(Last, VarMap, Value).
r_branch_value(=(_,Expr), VarMap, Value) :- !, r_expr(Expr, VarMap, Value).
r_branch_value(is(_,Expr), VarMap, Value) :- !, r_expr(Expr, VarMap, Value).
r_branch_value(Goal, VarMap, Value) :- r_expr(Goal, VarMap, Value).

%% r_literal
r_literal(true, 'TRUE') :- !.
r_literal(false, 'FALSE') :- !.
r_literal(Value, Literal) :- number(Value), !, format(string(Literal), '~w', [Value]).
r_literal(Value, Literal) :- atom(Value), !, format(string(Literal), '"~w"', [Value]).
r_literal(Value, Literal) :- string(Value), !, format(string(Literal), '"~w"', [Value]).
r_literal(Value, Literal) :- term_string(Value, S), format(string(Literal), '"~w"', [S]).
...
%% combine_r_conditions
combine_r_conditions([], "true") :- !.
combine_r_conditions([C], C) :- !.
combine_r_conditions([C|Cs], Combined) :-
    combine_r_conditions(Cs, Rest),
    format(string(Combined), '~w && ~w', [C, Rest]).

%% branches_to_r_if_chain
branches_to_r_if_chain(Branches, PredSpec, Code) :-
    r_if_chain_lines(Branches, Lines),
    format(string(Fallback), '    stop("No matching clause for ~w")', [PredSpec]),
    append(Lines, [Fallback], AllLines),
    atomic_list_concat(AllLines, '\n', Code).

r_if_chain_lines([], []).
r_if_chain_lines([branch(Cond, Value)], Lines) :-
    (   Cond == "true"
    ->  format(string(L), '    ~w', [Value]),
        Lines = [L]
    ;   format(string(L1), '    if (~w) {', [Cond]),
        format(string(L2), '        ~w', [Value]),
        Lines = [L1, L2, '    }']
    ).
r_if_chain_lines([branch(Cond, Value)|Rest], Lines) :-
    Rest \= [],
    (   Cond == "true"
    ->  format(string(L), '    ~w', [Value]),
        Lines = [L]
    ;   r_if_chain_lines(Rest, RestLines),
        format(string(L1), '    if (~w) {', [Cond]),
        format(string(L2), '        ~w', [Value]),

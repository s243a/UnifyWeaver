...
linear_fold_jython(PredStr, BaseClauses, MemoEnabled, Code) :-
    ...
    (   MemoEnabled = true ->
        ...
        format(string(MemoCheck), '    if n in ~w_memo: return ~w_memo[n]', [PredStr, PredStr]),
...
', [PredStr, MemoDecl, PredStr, MemoCheck, BaseInput, BaseOutput, JythonOp, BaseOutput, MemoStore, PredStr])
    ;   InputType = list ->
        format(string(Code),
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2:
        items = [int(x) for x in sys.argv[1].split(",")]
        print(~w(items))
', [PredStr, MemoDecl, PredStr, MemoCheck, BaseOutput, JythonOp, BaseOutput, MemoStore, PredStr])

...
', [PackageName, PredicatesCode]),
    ...
    % Reset the atom intern table first so each generation starts clean;
    % the lowered emitter calls intern_atom_go/2 as it walks instructions.
...
~w
~w
', [PackageName, AtomTableCode, LoweredCode]),
...
%  Emit a single Go var(...) block declaring all interned atoms in
%  insertion order. Empty if no atoms were interned. Caller writes the
%  resulting code into a Go source file (typically lowered.go) before
...
emit_atom_table_go(GoCode) :-
    ...
    (   Pairs == []
    ->  GoCode = ""
    ;   maplist(format_atom_decl, Pairs, Lines),
        atomic_list_concat([

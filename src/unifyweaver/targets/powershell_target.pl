...
:- module(powershell_target, [
    ...
    compile_facts_to_powershell/3,      % +Pred, +Arity, -PsCode  -- NEW
...
%% ============================================================================
%% FACT EXPORT - compile_facts_to_powershell/3
%% ============================================================================

%% compile_facts_to_powershell(+Pred, +Arity, -PsCode)
%  Export Prolog facts as a PowerShell class with static fact data.
%  Generates a class with GetAll, Stream, and Contains methods.
%
%  Example:
...
compile_facts_to_powershell(Pred, Arity, PsCode) :-
    ...
    % Format facts as PowerShell array of arrays
    findall(Entry, (
        member(Args, AllFacts),
        maplist(format_ps_fact_arg, Args, FormattedArgs),
        atomic_list_concat(FormattedArgs, ', ', ArgsStr),
        format(string(Entry), '        @(~w)', [ArgsStr])
    ), Entries),
    atomic_list_concat(Entries, ',\n', EntriesCode),
    
    % Generate PowerShell class
    format(string(PsCode),
'# Generated by UnifyWeaver PowerShell Target - Fact Export
# Predicate: ~w/~w
...
# Main execution - print all facts
foreach ($fact in [~w]::Stream()) {
    $fact -join \":\"
}
', [PredStr, Arity, PredUpStr, PredStr, Arity, EntriesCode, PredUpStr, PredUpStr, PredUpStr, PredUpStr]).

%% format_ps_fact_arg(+Arg, -Formatted)
%  Format a single fact argument as a PowerShell string literal.
format_ps_fact_arg(Arg, Formatted) :-
    (   atom(Arg)
...
compile_predicate_to_powershell(PredicateIndicator, Options, PsCode) :-
    ...
    (   Clauses \= [],
        compile_clauses_to_ps(Pred, Arity, Clauses, Options, PsCode)
    ->  true
...
%% ============================================
%% NATIVE CLAUSE LOWERING
%% ============================================

compile_clauses_to_ps(Pred, Arity, Clauses, _Options, Code) :-
    atom_string(Pred, PredStr),
    native_ps_clause_body(PredStr/Arity, Clauses, FuncBody),
    !,
    Arity1 is Arity - 1,
    ps_param_list(Arity1, ParamList),
    format(string(Code),
'# Generated by UnifyWeaver PowerShell Target - Native Clause Lowering
# Predicate: ~w/~w
...
%% native_ps_clause_body — single clause
native_ps_clause_body(PredSpec, [Head-Body], Code) :-
    native_ps_clause(PredSpec, Head, Body, Condition, ClauseCode),
    !,
    (   Condition == "true"
...
%% native_ps_clause_body — multi-clause
native_ps_clause_body(PredSpec, Clauses, Code) :-
    Clauses = [_,_|_],
    maplist(native_ps_clause_pair(PredSpec), Clauses, Branches),
    Branches \= [],
    branches_to_ps_if_chain(Branches, PredSpec, Code).

native_ps_clause_pair(PredSpec, Head-Body, branch(Condition, ClauseCode)) :-
    native_ps_clause(PredSpec, Head, Body, Condition, ClauseCode), !.

%% native_ps_clause/5 — core clause analysis
native_ps_clause(_PredSpec, Head, Body, Condition, Code) :-
    Head =.. [_|HeadArgs],
    length(HeadArgs, Arity),
    build_head_varmap(HeadArgs, 1, VarMap),
    (   Arity > 1
    ->  append(_, [OutputHeadArg], HeadArgs),
        ps_head_conditions(HeadArgs, 1, Arity, HeadConditions)
    ;   OutputHeadArg = _,
        ps_head_conditions(HeadArgs, 1, Arity, HeadConditions)
    ),
    normalize_goals(Body, Goals),
    (   Goals == []
    ->  ps_resolve_value(VarMap, OutputHeadArg, Code),
        GoalConditions = []
    ;   (   Arity > 1, nonvar(OutputHeadArg)
        ->  clause_guard_output_split(Goals, VarMap, GuardGoals, OutputGoals),
            maplist(ps_guard_condition(VarMap), GuardGoals, GoalConditions),
            (   OutputGoals == []
            ->  ps_literal(OutputHeadArg, Code)
            ;   ps_output_goals(OutputGoals, VarMap, Code)
            )
        ;   native_ps_goal_sequence(Goals, VarMap, GoalConditions, Code)
        )
    ),
    append(HeadConditions, GoalConditions, AllConditions),
    combine_ps_conditions(AllConditions, Condition).

native_ps_goal_sequence(Goals, VarMap, Conditions, Code) :-
    classify_goal_sequence(Goals, VarMap, ClassifiedGoals),
    ClassifiedGoals \= [],
    ps_render_classified_last(ClassifiedGoals, VarMap, Conditions, Code),
    !.
native_ps_goal_sequence(Goals, VarMap, Conditions, Code) :-
    clause_guard_output_split(Goals, VarMap, GuardGoals, OutputGoals),
    maplist(ps_guard_condition(VarMap), GuardGoals, Conditions),
    ps_output_goals(OutputGoals, VarMap, Code).

ps_render_classified_last([output_ite(If, Then, Else, _)], VarMap, [], Code) :- !,
    ps_guard_condition(VarMap, If, Cond),
    ps_branch_value(Then, VarMap, ThenVal),
    ps_branch_value(Else, VarMap, ElseVal),
    format(string(Code), 'if (~w) { ~w } else { ~w }', [Cond, ThenVal, ElseVal]).
ps_render_classified_last([output(Goal, _, _)], VarMap, [], Code) :- !,
    ps_output_goals([Goal], VarMap, Code).
ps_render_classified_last([guard(Goal, _)|Rest], VarMap, [Cond|RestConds], Code) :- !,
    ps_guard_condition(VarMap, Goal, Cond),
    ps_render_classified_last(Rest, VarMap, RestConds, Code).
ps_render_classified_last(_, _, [], "''").

%% ps_head_conditions
ps_head_conditions([], _, _, []).
ps_head_conditions([_], _, Arity, []) :- Arity > 1, !.
ps_head_conditions([HeadArg|Rest], Index, Arity, Conditions) :-
    (   var(HeadArg)
    ->  Conditions = RestConditions
    ;   format(string(ArgName), '$arg~w', [Index]),
        ps_literal(HeadArg, Literal),
        format(string(Cond), '~w -eq ~w', [ArgName, Literal]),
        Conditions = [Cond|RestConditions]
    ),
    NextIndex is Index + 1,
    ps_head_conditions(Rest, NextIndex, Arity, RestConditions).

%% ps_guard_condition
ps_guard_condition(VarMap, Goal, Condition) :-
    compound(Goal),
    Goal =.. [Op, Left, Right],
    expr_op(Op, StdOp),
    !,
    ps_expr(Left, VarMap, PLeft),
    ps_expr(Right, VarMap, PRight),
    ps_op(StdOp, POp),
    format(string(Condition), '~w ~w ~w', [PLeft, POp, PRight]).

%% ps_output_goals
ps_output_goals(Outputs, VarMap, Value) :-
    last(Outputs, LastGoal),
    (   goal_output_var(LastGoal, _)
    ->  ps_goal_expr(LastGoal, VarMap, Value)
    ;   ps_resolve_value(LastGoal, VarMap, Value)
    ).

ps_goal_expr(Goal, VarMap, Expr) :-
    Goal = (_Var = RHS), !, ps_expr(RHS, VarMap, Expr).
ps_goal_expr(Goal, VarMap, Expr) :-
    Goal = (_Var is ArithExpr), !, ps_expr(ArithExpr, VarMap, Expr).
ps_goal_expr(Goal, VarMap, Expr) :-
    ps_resolve_value(Goal, VarMap, Expr).

%% ps_expr — expression compilation
ps_expr(Var, VarMap, PExpr) :-
    var(Var), !,
    lookup_var(Var, VarMap, Name),
    format(string(PExpr), '$~w', [Name]).
ps_expr(Expr, _VarMap, PExpr) :-
    number(Expr), !,
    format(string(PExpr), '~w', [Expr]).
ps_expr(Expr, VarMap, PExpr) :-
    atom(Expr), lookup_var(Expr, VarMap, Name), !,
    format(string(PExpr), '$~w', [Name]).
ps_expr(Expr, _VarMap, PExpr) :-
    atom(Expr), !, ps_literal(Expr, PExpr).
ps_expr(-Expr, VarMap, PExpr) :-
    !, ps_expr(Expr, VarMap, Inner),
    format(string(PExpr), '(-~w)', [Inner]).
ps_expr(abs(Expr), VarMap, PExpr) :-
    !, ps_expr(Expr, VarMap, Inner),
    format(string(PExpr), '[Math]::Abs(~w)', [Inner]).
ps_expr(Expr, VarMap, PExpr) :-
    compound(Expr),
    Expr =.. [Op, Left, Right],
    expr_op(Op, _),
...
%% ps_resolve_value
ps_resolve_value(VarMap, Var, Value) :-
    var(Var), !,
    lookup_var(Var, VarMap, Name),
    format(string(Value), '$~w', [Name]).
ps_resolve_value(_VarMap, Term, Value) :-
    number(Term), !,
    format(string(Value), '~w', [Term]).
ps_resolve_value(VarMap, Term, Value) :-
    atom(Term), lookup_var(Term, VarMap, Name), !,
    format(string(Value), '$~w', [Name]).
ps_resolve_value(_VarMap, Term, Value) :-
    atom(Term), !, ps_literal(Term, Value).
ps_resolve_value(VarMap, Term, Value) :-
    ps_expr(Term, VarMap, Value).

%% ps_branch_value
ps_branch_value(Goal, VarMap, Value) :-
    if_then_else_goal(Goal, If, Then, Else), !,
    ps_guard_condition(VarMap, If, Cond),
    ps_branch_value(Then, VarMap, ThenVal),
    ps_branch_value(Else, VarMap, ElseVal),
    format(string(Value), '$(if (~w) { ~w } else { ~w })', [Cond, ThenVal, ElseVal]).
ps_branch_value((A, B), VarMap, Value) :-
    !, normalize_goals((A, B), Goals), last(Goals, Last),
    ps_branch_value(Last, VarMap, Value).
ps_branch_value(=(_,Expr), VarMap, Value) :- !, ps_expr(Expr, VarMap, Value).
ps_branch_value(is(_,Expr), VarMap, Value) :- !, ps_expr(Expr, VarMap, Value).
ps_branch_value(Goal, VarMap, Value) :- ps_expr(Goal, VarMap, Value).

%% ps_literal
ps_literal(true, '$true') :- !.
ps_literal(false, '$false') :- !.
ps_literal(Value, Literal) :- number(Value), !, format(string(Literal), '~w', [Value]).
ps_literal(Value, Literal) :- atom(Value), !, format(string(Literal), '"~w"', [Value]).
ps_literal(Value, Literal) :- string(Value), !, format(string(Literal), '"~w"', [Value]).
ps_literal(Value, Literal) :- term_string(Value, S), format(string(Literal), '"~w"', [S]).

%% ps_op — comparison operators
ps_op('>', '-gt').
ps_op('<', '-lt').
ps_op('>=', '-ge').
ps_op('<=', '-le').
...
%% ps_arith_op — arithmetic operators
ps_arith_op(+, '+').
ps_arith_op(-, '-').
ps_arith_op(*, '*').
ps_arith_op(/, '/').
...
%% combine_ps_conditions
combine_ps_conditions([], "true") :- !.
combine_ps_conditions([C], C) :- !.
combine_ps_conditions([C|Cs], Combined) :-

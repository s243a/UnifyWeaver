...
:- use_module(wam_cpp_templates,
              [cpp_render_template/3, cpp_preflight_stache/0,
               cpp_preflight_lowered_function/0, cpp_preflight_ite_shell/0]).
...
% An asset edit between project preflight and a lowered predicate must not
% become a silently omitted predicate under the default warn policy.
lowered_template_id(lowered_function).
lowered_template_id(lowered_ite).

lowered_template_error(error(cpp_wam_template_load(Id, _, _), _)) :- lowered_template_id(Id).
lowered_template_error(error(cpp_wam_template_empty(Id, _), _)) :- lowered_template_id(Id).
lowered_template_error(error(cpp_wam_template_tags(Id, _), _)) :- lowered_template_id(Id).
lowered_template_error(error(domain_error(cpp_wam_lowered_function_variables, _), _)).
lowered_template_error(error(domain_error(cpp_wam_ite_variables, _), _)).

...
direct_computations_c(Computations, Code) :-
    findall(Line, (
        member(_Var is Expr, Computations),
        direct_translate_expr_c(Expr, CExpr),
        format(string(Line), '    int v_~w = ~w;', [CExpr, CExpr])
    ), Lines),
    atomic_list_concat(Lines, '\n', Code).
...
direct_recursive_calls_c(RecCalls, PredStr, Code) :-
    findall(Line, (
        ...
        RecCall =.. [_Pred, ArgVar, _ResultVar],
        direct_translate_expr_c(ArgVar, ArgExpr),
        format(string(Line), '    int rc_~w = ~w(~w);', [ArgExpr, PredStr, ArgExpr])
    ), Lines),
    atomic_list_concat(Lines, '\n', Code).

direct_aggregation_c(_ is Expr, Code) :-
    direct_translate_agg_c(Expr, CExpr),
    format(string(Code), '    int result = ~w;', [CExpr]).

direct_translate_agg_c(A + B, Expr) :-
    direct_translate_expr_c(A, AE), direct_translate_expr_c(B, BE),

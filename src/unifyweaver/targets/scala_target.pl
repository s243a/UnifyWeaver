...
advanced_recursive_compiler:compile_general_recursive_pattern(scala, PredStr, _Arity, BaseClauses, _RecClauses, Code) :-
    ...
    format(string(Code),
'// Generated by UnifyWeaver Scala Target - General Recursion with Visited Set

def ~w(arg1: String): List[String] = ~w(arg1, Set.empty)

def ~w(arg1: String, visited: Set[String]): List[String] = {
  if (visited.contains(arg1)) Nil
  else if (arg1 == ~w) List(~w)
  else {
    val newVisited = visited + arg1
    ~w(arg1.toString, newVisited)
  }
}',
        [PredStr, WorkerStr,
         WorkerStr,

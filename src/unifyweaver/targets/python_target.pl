...
%% --- Mutual Recursion: memoized functions calling each other ---
%% compile_mutual_pattern(+Target, +Predicates, +MemoEnabled, +MemoStrategy, -Code)

mutual_recursion:compile_mutual_pattern(python, Predicates, MemoEnabled, _MemoStrategy, Code) :-
    mutual_functions_python(Predicates, Predicates, MemoEnabled, FuncCodes),
    atomic_list_concat(FuncCodes, '\n\n', FunctionsCode),
    mutual_dispatch_python(Predicates, DispatchCode),
    format(string(Code),
'#!/usr/bin/env python3
# Generated by UnifyWeaver Python Target - Mutual Recursion (multifile dispatch)
import sys
import functools
...
mutual_functions_python([], _AllPreds, _MemoEnabled, []).
mutual_functions_python([Pred/Arity|Rest], AllPreds, MemoEnabled, [FuncCode|RestCodes]) :-
    atom_string(Pred, PredStr),
    functor(Head, Pred, Arity),
    ...
    (   Arity =:= 1
    ->  mutual_function_arity1_python(PredStr, BaseClauses, RecClauses, AllPreds, MemoEnabled, FuncCode)
    ;   Arity =:= 2
    ->  mutual_function_arity2_python(PredStr, BaseClauses, RecClauses, AllPreds, MemoEnabled, FuncCode)
    ;   format(string(FuncCode), '# Mutual recursion arity ~d not supported for ~w', [Arity, PredStr])
    ),
    mutual_functions_python(Rest, AllPreds, MemoEnabled, RestCodes).

%% Arity-1: boolean mutual (is_even/is_odd pattern)
mutual_function_arity1_python(PredStr, BaseClauses, RecClauses, AllPreds, MemoEnabled, FuncCode) :-
    %% Base cases
    findall(BaseLine, (
    ...
    ->  extract_mutual_rec_info_python(RBody, AllPreds, Guard, CalledPred, Step),
        atom_string(CalledPred, CalledStr),
        (   Guard = (N > Threshold), var(N)
        ->  format(string(RecCode), '    if n > ~w:\n        return ~w(n~w)', [Threshold, CalledStr, Step])
        ;   format(string(RecCode), '    return ~w(n~w)', [CalledStr, Step])
        )
    ;   RecCode = ''
    ),
    (MemoEnabled = true -> MemoLine = '@functools.cache' ; MemoLine = ''),
    format(string(FuncCode),
'~w
def ~w(n):
~w
~w
    return False', [MemoLine, PredStr, BaseCode, RecCode]).

%% Arity-2: value-returning mutual (count_down_even/count_down_odd pattern)
mutual_function_arity2_python(PredStr, BaseClauses, RecClauses, AllPreds, MemoEnabled, FuncCode) :-
    %% Base cases
    findall(BaseLine, (
        member(clause(BHead, true), BaseClauses),
        BHead =.. [_, BInput, BOutput],
        (number(BOutput) -> format(string(BVal), '~w', [BOutput])
        ; atom(BOutput) -> format(string(BVal), '"~w"', [BOutput])
        ; BVal = "None"),
        (var(BInput) -> BaseCond = "True"
        ; number(BInput) -> format(string(BaseCond), 'n == ~w', [BInput])
        ; format(string(BaseCond), 'n == "~w"', [BInput])),
        format(string(BaseLine), '    if ~w:\n        return ~w', [BaseCond, BVal])
    ), BaseLines),
    atomic_list_concat(BaseLines, '\n', BaseCode),
    %% Recursive case
    ...
    ->  RHead =.. [_, _InputVar, _],
        ...
        atom_string(CalledPred, CalledStr),
        (   Guard = (N > Threshold), var(N)
        ->  format(string(GuardLine), '    if n > ~w:', [Threshold])
        ;   GuardLine = ''
        ),
        Indent = '    ',
        (   GuardLine \= ''
        ->  format(string(RecCode), '~w\n~w~wrec_result = ~w(n~w)\n~w~wreturn ~w',
                [GuardLine, Indent, Indent, CalledStr, Step, Indent, Indent, FoldExpr])
        ;   format(string(RecCode), '~wrec_result = ~w(n~w)\n~wreturn ~w',
                [Indent, CalledStr, Step, Indent, FoldExpr])
        )
    ;   RecCode = ''
    ),
    (MemoEnabled = true -> MemoLine = '@functools.cache' ; MemoLine = ''),
...
def ~w(n):
~w
~w', [MemoLine, PredStr, BaseCode, RecCode]).

%% extract_mutual_rec_info_python(+Body, +AllPreds, -Guard, -CalledPred, -Step)
extract_mutual_rec_info_python(Body, AllPreds, Guard, CalledPred, Step) :-
    extract_goals_list(Body, Goals),
    %% Find guard (if any)
    (   member(G, Goals), G =.. [Op, _, _], member(Op, [>, <, >=, =<, =:=])
    ->  Guard = G
    ;   Guard = none
    ),
    %% Find mutual call
    ...
    !,
    %% Extract step — trace through is-bindings if CalledArg is a variable
    resolve_mutual_step(CalledArg, Goals, Step).

%% extract_mutual_rec_info2_python(+Body, +AllPreds, -Guard, -CalledPred, -Step, -FoldExpr)
extract_mutual_rec_info2_python(Body, AllPreds, Guard, CalledPred, Step, FoldExpr) :-
    extract_goals_list(Body, Goals),
    %% Find guard
    (   member(G, Goals), G =.. [Op, _, _], member(Op, [>, <, >=, =<, =:=])
...
%% translate_fold_expr_mutual_python(+Expr, +RecVar, -PyExpr)
translate_fold_expr_mutual_python(A + B, RecVar, PyExpr) :-
    (A == RecVar -> format(string(PyExpr), 'rec_result + ~w', [B])
    ; B == RecVar -> format(string(PyExpr), '~w + rec_result', [A])
    ; format(string(PyExpr), 'rec_result + ~w', [B])), !.
translate_fold_expr_mutual_python(A * B, RecVar, PyExpr) :-
    (A == RecVar -> format(string(PyExpr), 'rec_result * ~w', [B])
    ; B == RecVar -> format(string(PyExpr), '~w * rec_result', [A])
    ; format(string(PyExpr), 'rec_result * ~w', [B])), !.
translate_fold_expr_mutual_python(_, _, "rec_result").

%% mutual_dispatch_python(+Predicates, -DispatchCode)
mutual_dispatch_python(Predicates, DispatchCode) :-
    Predicates = [First/_|_],
    atom_string(First, FirstStr),
    format(string(DispatchCode),
'if __name__ == "__main__":
    if len(sys.argv) >= 2:
        print(~w(int(sys.argv[1])))', [FirstStr]).

%% resolve_mutual_step(+CalledArg, +Goals, -Step)
%  Extract the step expression for the mutual recursive call argument.
%  If CalledArg is a variable bound by an is-expression (N1 is N - 1),
%  trace through to find the step relative to the input.
resolve_mutual_step(CalledArg, _Goals, Step) :-
    CalledArg = (_ - Const), number(Const), !,
    format(string(Step), ' - ~w', [Const]).
resolve_mutual_step(CalledArg, _Goals, Step) :-
    CalledArg = (_ + Const), number(Const), !,
    format(string(Step), ' + ~w', [Const]).
resolve_mutual_step(CalledArg, Goals, Step) :-
    var(CalledArg),
    %% Find is-binding: CalledArg is SomeExpr
    member(CalledArg is Expr, Goals),
    !,
    %% Normalize N + -K to N - K
    (   Expr = (_ + NegConst), number(NegConst), NegConst < 0
    ->  PosConst is abs(NegConst),
        format(string(Step), ' - ~w', [PosConst])
    ;   Expr = (_ - Const), number(Const)
    ->  format(string(Step), ' - ~w', [Const])
    ;   Expr = (_ + Const), number(Const)
    ->  format(string(Step), ' + ~w', [Const])
    ;   Step = ""
    ).
resolve_mutual_step(_, _, "").

...
:- module(graph_generator, [
    ...
    generate_graph_with_layout/3,       % generate_graph_with_layout(+Name, +LayoutPattern, -Code)
    generate_graph_full_styles/2,       % generate_graph_full_styles(+Name, -CssCode)
...
%% generate_graph_with_layout(+Name, +LayoutPattern, -Code)
%  Generate a graph component using a layout pattern from layout_generator.
...
generate_graph_with_layout(Name, LayoutPattern, Code) :-
    ...
    % Set up layout for this graph
    setup_graph_layout(Name, LayoutPattern),
    ...
    format(atom(Code),
'// Generated by UnifyWeaver - Graph Component with Layout
// Graph: ~w
// Layout: ~w
...
export default ~w;
', [Title, LayoutPattern, ComponentName, ComponentName, DataCode, CytoscapeConfig, LayoutJSX, ComponentName]).

%% setup_graph_layout(+Name, +LayoutPattern)
%  Configure layout_generator for a graph component.
setup_graph_layout(Name, sidebar_content) :-
    ...
    layout_generator:assertz(place(Name, graph, [graph_canvas])).

setup_graph_layout(Name, content_sidebar) :-
    layout_generator:declare_layout(Name, grid, [
        areas([["graph", "sidebar"]]),
        columns(["1fr", "320px"]),
        rows(["1fr"]),
...
setup_graph_layout(Name, dashboard) :-
    layout_generator:declare_layout(Name, grid, [
        areas([
            ["header", "header"],
            ["sidebar", "graph"],
    ...
    layout_generator:assertz(place(Name, sidebar, [controls, legend])),
    layout_generator:assertz(place(Name, graph, [graph_canvas])),
    layout_generator:assertz(place(Name, footer, [status_bar])).

setup_graph_layout(Name, single) :-
    layout_generator:declare_layout(Name, grid, [
        areas([["graph"]]),
        columns(["1fr"]),
        rows(["1fr"])
...
setup_graph_layout(Name, _) :-
    % Default to single layout
    setup_graph_layout(Name, single).

%% generate_graph_full_styles(+Name, -CssCode)
%  Generate CSS using both layout_generator and graph-specific styles.
generate_graph_full_styles(Name, CssCode) :-
    graph_spec(Name, Config),

...
%% Rust and C# stages
compile_stage(rust, Predicates, StageNum, _OutputDir, Options, file(FileName, Code), stage(StageNum, rust, FileName, Predicates)) :-
    compile_stage_with_semantic(rust, Predicates, StageNum, Options, Code),
    format(string(FileName), "rust_stage_~w.rs", [StageNum]).
...
%  semantic_compiler and fuzzy predicates through fuzzy_dispatch.
%  Non-semantic predicates are compiled via the regular target compiler.
:- use_module('../core/semantic_compiler').

compile_stage_with_semantic(Runtime, Predicates, StageNum, _Options, Code) :-
    maplist(strip_runtime_prefix, Predicates, Stripped),
    maplist(extract_pred_name, Stripped, PredNames),
    % Partition into semantic and regular predicates
    partition(is_semantic_stage_pred, Stripped, SemanticPreds, RegularPreds),
    % Compile semantic predicates
    maplist(compile_semantic_stage_pred(Runtime), SemanticPreds, SemanticCodes),
    atomic_list_concat(SemanticCodes, '\n', SemanticBlock),
    % Compile fuzzy predicates from regular set
    partition(is_fuzzy_stage_pred, RegularPreds, FuzzyPreds, _OtherPreds),
    maplist(compile_fuzzy_stage_pred(Runtime), FuzzyPreds, FuzzyCodes),
    atomic_list_concat(FuzzyCodes, '\n', FuzzyBlock),
    % Combine
    format(string(Code),
'// Stage ~w (~w) - Predicates: ~w
// Generated by UnifyWeaver cross-runtime pipeline

~w
~w
', [StageNum, Runtime, PredNames, SemanticBlock, FuzzyBlock]).

is_semantic_stage_pred(Name/_Arity) :-
    functor(Goal, Name, _Arity),
    semantic_compiler:is_semantic_predicate(Goal).
is_semantic_stage_pred(_) :- fail.

is_fuzzy_stage_pred(Name/_Arity) :-
    functor(Goal, Name, _Arity),
    semantic_compiler:is_fuzzy_predicate(Goal).
is_fuzzy_stage_pred(_) :- fail.

compile_semantic_stage_pred(Runtime, Name/Arity, Code) :-
    length(Args, Arity),
    Goal =.. [Name | Args],
    semantic_compiler:compile_semantic_call(Runtime, Goal, [], Code).

compile_fuzzy_stage_pred(Runtime, Name/Arity, Code) :-
    length(Args, Arity),
    Goal =.. [Name | Args],
    semantic_compiler:compile_fuzzy_call(Runtime, Goal, Code).

...
if (!$SwiplPath) {
    ...
    Write-Host "[ERROR] Please install SWI-Prolog from: https://www.swi-prolog.org/download/stable" -ForegroundColor Red
    Write-Host "[ERROR] Make sure to add it to PATH during installation" -ForegroundColor Red
    exit 1
}

$SwiplVersion = & swipl --version 2>&1 | Select-String "SWI-Prolog"
Write-Host "[OK] Found: $SwiplVersion" -ForegroundColor Green
...
cd /d "%TEST_ROOT%"
swipl -q -g "asserta(library_directory('src')), asserta(file_search_path(unifyweaver, 'src/unifyweaver'))" -t prolog
"@

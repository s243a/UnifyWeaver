#!/bin/bash
# UnifyWeaver Docker Wine Environment Launcher
...
echo -e "${GREEN}╔════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  UnifyWeaver Wine Environment (Docker)     ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════════════╝${NC}"
echo ""
...
case "$ACTION" in
    build)
        ...
        $DOCKER_COMPOSE build unifyweaver-wine
    ...
    run|start)
        ...
        if docker ps | grep -q unifyweaver-wine; then
            echo -e "${YELLOW}[INFO] Container already running, attaching...${NC}"
            docker exec -it unifyweaver-wine /bin/bash
        else
            # Start container with appropriate compose file
            if [[ "$PERSISTENT_MODE" == "persistent" ]]; then
                $DOCKER_COMPOSE -f docker-compose.yml -f docker-compose.persistent.yml up -d unifyweaver-wine
            else
                $DOCKER_COMPOSE up -d unifyweaver-wine
            fi
...
cd "$SCRIPT_DIR"
    ...
    logs)
        $DOCKER_COMPOSE logs -f unifyweaver-wine
        ;;

    test)
        echo -e "${BLUE}[INFO] Running UnifyWeaver tests in container...${NC}"
        docker exec -it unifyweaver-wine bash -c "

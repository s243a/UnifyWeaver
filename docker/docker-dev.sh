#!/bin/bash
# UnifyWeaver Docker Development Environment Launcher
...
echo -e "${GREEN}╔════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  UnifyWeaver Development Environment (Docker)     ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════════════╝${NC}"
echo ""
...
case "$ACTION" in
    build)
        ...
        $DOCKER_COMPOSE build unifyweaver-dev
    ...
    run|start)
        ...
        if docker ps | grep -q unifyweaver-dev; then
            ...
            docker exec -it unifyweaver-dev /bin/bash
        else
            # Start container with appropriate compose file
            if [[ "$PERSISTENT_MODE" == "persistent" ]]; then
                $DOCKER_COMPOSE -f docker-compose.yml -f docker-compose.persistent.yml up -d unifyweaver-dev
            else
                $DOCKER_COMPOSE up -d unifyweaver-dev
            fi
...
cd "$SCRIPT_DIR"
    ...
    logs)
        $DOCKER_COMPOSE logs -f unifyweaver-dev
        ;;

    test)
        echo -e "${BLUE}[INFO] Running UnifyWeaver tests in container...${NC}"
        docker exec -it unifyweaver-dev bash -c "

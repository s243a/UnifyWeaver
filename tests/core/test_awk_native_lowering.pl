:- module(test_awk_native_lowering, [test_awk_native_lowering/0]).
:- use_module(library(plunit)).
:- use_module('../../src/unifyweaver/targets/awk_target').

test_awk_native_lowering :-
    run_tests([awk_native_lowering]).

:- begin_tests(awk_native_lowering).

% Helper: compile using the public API
compile_awk(Pred/Arity, Code) :-
    awk_target:compile_predicate_to_awk(Pred/Arity, [], Code).
...
test(nested_if_then_else) :-
    assert(user:(range_classify(X, R) :-
        ...
        ; (X =:= 0 -> R = zero
        ; R = positive)))),
    compile_awk(range_classify/2, Code),

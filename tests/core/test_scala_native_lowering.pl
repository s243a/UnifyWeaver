:- module(test_scala_native_lowering, [test_scala_native_lowering/0]).
:- use_module(library(plunit)).
:- use_module('../../src/unifyweaver/targets/scala_target').

test_scala_native_lowering :-
    run_tests([scala_native_lowering]).

:- begin_tests(scala_native_lowering).

% Helper: compile using the public API
compile_sc(Pred/Arity, Code) :-
    scala_target:compile_predicate_to_scala(Pred/Arity, [], Code).
...
test(multi_clause_guard_chain) :-
    ...
    compile_sc(classify/2, Code),
    has(Code, "def classify(arg1: Long): String"),
...
test(single_clause_guard) :-
    ...
    compile_sc(positive/2, Code),
    has(Code, "def positive(arg1: Long): String"),
    has(Code, "arg1 > 0"),
    has(Code, "\"yes\""),
...
test(arithmetic_output) :-
    ...
    compile_sc(double/2, Code),
    has(Code, "def double(arg1: Long): String"),
    has(Code, "(arg1 * 2)"),
    retractall(user:double(_, _)).

test(assignment_output) :-
    assert(user:(identity(X, R) :- R = X)),
    compile_sc(identity/2, Code),
    has(Code, "def identity(arg1: Long): String"),
    has(Code, "arg1"),
...
test(multi_clause_rules) :-
    ...
    compile_sc(color2/2, Code),
    has(Code, "def color2(arg1: Long): String"),
    has(Code, "arg1 == \"red\""),
    has(Code, "arg1 == \"blue\""),
    has(Code, "arg1 == \"green\""),
...
test(if_then_else_simple) :-
    ...
    compile_sc(abs_val/2, Code),
    has(Code, "def abs_val(arg1: Long): String"),
    has(Code, "arg1 >= 0"),
    has(Code, "arg1"),
    has(Code, "(-arg1)"),
...
test(nested_if_then_else) :-
    assert(user:(range_classify(X, R) :-
        ...
        ; (X =:= 0 -> R = zero
        ; R = positive)))),
    compile_sc(range_classify/2, Code),
    has(Code, "def range_classify(arg1: Long): String"),
    has(Code, "arg1 < 0"),
    has(Code, "\"negative\""),
    has(Code, "arg1 == 0"),
    has(Code, "\"zero\""),
...
test(three_way_nested) :-
    assert(user:(sign(X, R) :-
        ...
        ; (X < 0 -> R = negative
        ; R = zero)))),
    compile_sc(sign/2, Code),
    has(Code, "def sign(arg1: Long): String"),
    has(Code, "arg1 > 0"),
    has(Code, "arg1 < 0"),
...
test(guard_with_computation) :-
    ...
    compile_sc(safe_div/3, Code),
    has(Code, "def safe_div(arg1: Long, arg2: Long): String"),
    has(Code, "arg2 > 0"),
...
:- end_tests(scala_native_lowering).

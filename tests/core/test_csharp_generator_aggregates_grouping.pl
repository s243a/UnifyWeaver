...
setup_data :-
    ...
    assertz(user:(gs_count(Key, Count) :- aggregate_all(count, gs_item(Key, _), Key, Count))),
...
cleanup_data :-
    ...
    catch(abolish(user:gs_count/2), _, true),
...
test(aggregate_group_sum, [
    ...
    csharp_target:compile_predicate_to_csharp(gs_count/2, [mode(generator)], CountCode),
    build_program_stub("GsCount_Module", CountStub),
    atom_concat(CountCode, CountStub, CountFull),
    py_call(csharp_test_helper:compile_and_run(CountFull, 'agg_group_count'), CountResult),
    get_dict(success, CountResult, @(true)),
    get_dict(stdout, CountResult, CountOut),
    sub_string(CountOut, _, _, _, "gs_count:a:2"),
    sub_string(CountOut, _, _, _, "gs_count:b:2"),

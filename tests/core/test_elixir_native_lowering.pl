:- module(test_elixir_native_lowering, [test_elixir_native_lowering/0]).
:- use_module(library(plunit)).
:- use_module('../../src/unifyweaver/targets/elixir_target').

test_elixir_native_lowering :-
    run_tests([elixir_native_lowering]).

:- begin_tests(elixir_native_lowering).

% Helper: compile using the public API
compile_ex(Pred/Arity, Code) :-
    elixir_target:compile_predicate_to_elixir(Pred/Arity, [], Code).
...
test(nested_if_then_else) :-
    assert(user:(range_classify(X, R) :-
        ...
        ; (X =:= 0 -> R = zero
        ; R = positive)))),
    compile_ex(range_classify/2, Code),
...
test(guard_with_computation) :-
    ...
    compile_ex(safe_div/3, Code),
    has(Code, "def safe_div(arg1, arg2)"),
    has(Code, "arg2 > 0"),
    retractall(user:safe_div(_, _, _)).

% ============================================================================
% Elixir-specific syntax
% ============================================================================

test(elixir_uses_cond_for_multi_clause) :-
    assert(user:(grade(X, pass) :- X >= 50)),
    assert(user:(grade(X, fail) :- X < 50)),
    compile_ex(grade/2, Code),
    has(Code, "cond do"),
    retractall(user:grade(_, _)).

test(elixir_uses_defmodule) :-
    assert(user:(id2(X, R) :- R = X)),
    compile_ex(id2/2, Code),
    has(Code, "defmodule Generated."),
...
:- end_tests(elixir_native_lowering).

:- module(test_lua_native_lowering, [test_lua_native_lowering/0]).
:- use_module(library(plunit)).
:- use_module('../../src/unifyweaver/targets/lua_target').

test_lua_native_lowering :-
    run_tests([lua_native_lowering]).

:- begin_tests(lua_native_lowering).

% Helper: compile using the public API
compile_lua(Pred/Arity, Code) :-
    lua_target:compile_predicate_to_lua(Pred/Arity, [], Code).
...
% ============================================================================
% Tier 1: Multi-clause predicates → if/elseif/else chains
% ============================================================================

test(multi_clause_guard_chain) :-
    assert(user:(classify(X, small) :- X > 0, X < 10)),
    assert(user:(classify(X, large) :- X >= 10)),
    compile_lua(classify/2, Code),
    has(Code, "function classify(arg1)"),
...
test(single_clause_guard) :-
    ...
    compile_lua(positive/2, Code),
    has(Code, "function positive(arg1)"),
    has(Code, "arg1 > 0"),
    has(Code, "\"yes\""),
...
test(arithmetic_output) :-
    ...
    compile_lua(double/2, Code),
    has(Code, "function double(arg1)"),
    has(Code, "(arg1 * 2)"),
    retractall(user:double(_, _)).

test(assignment_output) :-
    assert(user:(identity(X, R) :- R = X)),
    compile_lua(identity/2, Code),
    has(Code, "function identity(arg1)"),
    has(Code, "arg1"),
...
test(multi_fact_native) :-
    ...
    compile_lua(color2/2, Code),
    has(Code, "function color2(arg1)"),
    has(Code, "arg1 == \"red\""),
    has(Code, "arg1 == \"blue\""),
    has(Code, "arg1 == \"green\""),
...
test(if_then_else_simple) :-
    ...
    compile_lua(abs_val/2, Code),
    has(Code, "function abs_val(arg1)"),
    has(Code, "arg1 >= 0"),
    has(Code, "arg1"),
    has(Code, "(-arg1)"),
...
test(nested_if_then_else) :-
    assert(user:(range_classify(X, R) :-
        ...
        ; (X =:= 0 -> R = zero
        ; R = positive)))),
    compile_lua(range_classify/2, Code),
    has(Code, "function range_classify(arg1)"),
    has(Code, "arg1 < 0"),
    has(Code, "\"negative\""),
    has(Code, "arg1 == 0"),
    has(Code, "\"zero\""),
...
test(three_way_nested) :-
    assert(user:(sign(X, R) :-
        ...
        ; (X < 0 -> R = negative
        ; R = zero)))),
    compile_lua(sign/2, Code),
    has(Code, "function sign(arg1)"),
    has(Code, "arg1 > 0"),
    has(Code, "arg1 < 0"),
...
test(guard_with_computation) :-
    ...
    compile_lua(safe_div/3, Code),
    has(Code, "function safe_div(arg1, arg2)"),
    has(Code, "arg2 > 0"),

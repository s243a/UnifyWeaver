...
setup_test_data :-
    ...
    assertz(user:mode(test_label_cat_reach_param_both(+, +, +, +))),
    assertz(user:(test_label_cat_reach_param_both(X, Y, L, Cat) :-
        test_label_cat_edge(X, Y, L, Cat)
    )),
    assertz(user:(test_label_cat_reach_param_both(X, Z, L, Cat) :-
        test_label_cat_edge(X, Y, L, Cat),
    ...
    assertz(user:test_recursive_label_path_cat_asym(a, b, red, cat1)),
    assertz(user:test_recursive_label_path_cat_asym(b, c, red, cat1)),
    forall(between(1, 80, I),
        (   format(atom(NodeA), 'ar~w', [I]),
            I2 is I + 2000,
            format(atom(NodeB), 'ar~w', [I2]),
            assertz(user:test_recursive_label_path_cat_asym(NodeA, NodeB, blue, cat2))
        )),
...
verify_hash_build_prefers_smaller_non_scan_side :-
    Plan = plan{
        head:predicate{name:test_hash_build_prefers_smaller_non_scan_side, arity:1},
        root:projection{
            type:projection,
            input:join{
                type:join,
                left:aggregate{
                    ...
                    input:relation_scan{
                        type:relation_scan,
                        predicate:predicate{name:test_hashbuild_small, arity:1},
                        width:1
                    },
                    predicate:predicate{name:test_hashbuild_data, arity:2},
                    op:count,
                ...
                right:aggregate{
                    ...
                    input:relation_scan{
                        type:relation_scan,
                        predicate:predicate{name:test_hashbuild_big, arity:1},
                        width:1
                    },
                    predicate:predicate{name:test_hashbuild_data, arity:2},
                    op:count,
...
verify_hash_build_prefers_selective_pattern_scan_wrapped :-
    Plan = plan{
        head:predicate{name:test_hash_build_pattern_scan_selectivity, arity:1},
        root:projection{
            type:projection,
            input:join{
                type:join,
                left:projection{
                    ...
                    input:pattern_scan{
                        type:pattern_scan,
                        predicate:predicate{name:test_hashbuild_pattern_data, arity:2},
                        pattern:[operand{kind:wildcard}, operand{kind:value, value:rare}],
                        width:2
                    },
                    columns:[0],
                    width:1
                ...
                right:projection{
                    ...
                    input:pattern_scan{
                        type:pattern_scan,
                        predicate:predicate{name:test_hashbuild_pattern_data, arity:2},
                        pattern:[operand{kind:wildcard}, operand{kind:wildcard}],
                        width:2
                    },
                    columns:[0],
                    width:1
        ...
        metadata:metadata{modes:[]},
        relations:[
            relation{predicate:predicate{name:test_hashbuild_pattern_data, arity:2}, facts:[[k1, rare], [k2, rare], [k3, common], [k4, common]]}
        ]
    },
    csharp_query_target:plan_module_name(Plan, ModuleClass),
    harness_source_with_strategy_flag(ModuleClass, [], 'KeyJoinHashBuildLeft', HarnessSource),
...
verify_recursive_multi_key_recursive_selection_join_prefers_filtered_build_side :-
    ...
    csharp_query_target:plan_module_name(Plan, ModuleClass),
    (   harness_source_with_strategy_flag_quiet(ModuleClass, [], 'KeyJoinRecursiveBuildFilteredLeft', HarnessSourceLeft),
        maybe_run_query_runtime_with_harness(Plan,
            ['STRATEGY_USED:KeyJoinRecursiveBuildFilteredLeft=true'],
            [],
            HarnessSourceLeft)
...
verify_path_aware_accumulation_plan :-
    ...
    get_dict(head, Root, predicate{name:test_weighted_path, arity:3}),
    get_dict(edge, Root, predicate{name:test_weighted_edge, arity:2}),
    get_dict(auxiliary, Root, predicate{name:test_weighted_cost, arity:2}),
    get_dict(base_expression, Root, expr{type:column, index:3}),
    get_dict(recursive_expression, Root, expr{type:binary,
        op:add,
        left:expr{type:column, index:2},
        right:expr{type:column, index:3}}),
    get_dict(width, Root, 3),
    csharp_query_target:render_plan_to_csharp(Plan, Source),
...
verify_parameterized_grouped_transitive_closure_pairs_strategy_runtime :-
    ...
    get_dict(metadata, Plan, Meta),
    get_dict(modes, Meta, Modes),
    Modes == [input, input, input, input],
    get_dict(root, Plan, transitive_closure_grouped{type:transitive_closure_grouped,
        head:predicate{name:test_label_cat_reach_param_both, arity:4},
        edge:predicate{name:test_label_cat_edge, arity:4},
        group_indices:[2,3],
        width:4
    }),
    csharp_query_target:render_plan_to_csharp(Plan, Source),
    sub_string(Source, _, _, _, 'new int[]{ 0, 1, 2, 3 }'),
    sub_string(Source, _, _, _, 'GroupedTransitiveClosureNode'),
    \+ sub_string(Source, _, _, _, '$need'),
    \+ sub_string(Source, _, _, _, 'ParamSeedNode'),
    csharp_query_target:plan_module_name(Plan, ModuleClass),
    Params = [[a, b, red, cat1], [a, c, red, cat1], [a, d, red, cat1]],
    harness_source_with_strategy_flag_no_reuse(ModuleClass, Params, 'GroupedTransitiveClosurePairsForward', HarnessSource),
    maybe_run_query_runtime_with_harness(Plan,
        ['a,b,red,cat1',
         'a,c,red,cat1',
         'STRATEGY_USED:GroupedTransitiveClosurePairsForward=true'],
        Params,
        HarnessSource).
...
verify_parameterized_reachability_pairs_single_probe_cache_eviction_runtime :-
    ...
    harness_source_with_cache_flag_two_warm_exec_pair_limit(
        ModuleClass,
        WarmParams1,
        WarmParams2,
        ExecParams,
...
verify_parameterized_grouped_transitive_closure_pairs_single_probe_cache_eviction_runtime :-
    csharp_query_target:build_query_plan(test_label_cat_reach_param_both/4, [target(csharp_query)], Plan),
    csharp_query_target:plan_module_name(Plan, ModuleClass),
    WarmParams1 = [[a, c, red, cat1]],
    WarmParams2 = [[a, b, red, cat1]],
    ExecParams = [[a, c, red, cat1]],
    harness_source_with_cache_flag_two_warm_exec_pair_limit(
        ModuleClass,
        WarmParams1,
        WarmParams2,
...
verify_parameterized_grouped_transitive_closure_by_target_seed_cache_lru_recency_runtime :-
    ...
    csharp_query_target:plan_module_name(Plan, ModuleClass),
    WarmParams1 = [[e, adm, cata]],
    WarmParams2 = [[b, adm, cata]],
    TouchParams = [[e, adm, cata]],
    InsertParams = [[c, adm, cata]],
...
verify_parameterized_reachability_pairs_cache_reuse_runtime :-
    csharp_query_target:build_query_plan(test_reachable_param_both/2, [target(csharp_query)], Plan),
    csharp_query_target:plan_module_name(Plan, ModuleClass),
    Params = [[alice, bob], [alice, charlie], [bob, alice]],
    harness_source_with_cache_flag(ModuleClass, Params, 'TransitiveClosureSeeded', HarnessSource),
    maybe_run_query_runtime_with_harness(Plan,
        ['alice,bob',
         'alice,charlie',
         'CACHE_HIT:TransitiveClosureSeeded=true'],

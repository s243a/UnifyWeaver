...
% THE WALKERS (loaded): the doubly-compiled compiler handles ITE codegen
% with labels and init-set intersection, comparison guards, the builtin
% whitelist, list-literal operands, and flat structure head patterns --
% byte-identically to the production cgfull. (The X-temp DEFERRAL paths
% are excluded here pending runtime finding no. 12; their logic is
% byte-exact in SWI -- see the next test.)

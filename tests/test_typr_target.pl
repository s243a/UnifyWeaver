...
test(explicit_mode_emits_declared_scalar_annotations) :-
    ...
    once(sub_string(Code, _, _, _, "let edge_from: [#N, char] <- [];")),
    once(sub_string(Code, _, _, _, "let edge_to: [#N, char] <- [];")),
    ...
    once(sub_string(Code, _, _, _, "let results: [#N, char] <- [];")),
    once(sub_string(Code, _, _, _, "let current <- queue[queue_index];")),
    once(sub_string(Code, _, _, _, "queue_index <- queue_index + 1;")),
...
test(explicit_any_is_preserved_in_infer_mode) :-
    ...
    once(sub_string(Code, _, _, _, "let results: [#N, Any] <- [];")),
    once(sub_string(Code, _, _, _, "let neighbors <- edge_neighbors(current, from_nodes, to_nodes);")),
    \+ sub_string(Code, _, _, _, "@{").
...
test(atom_string_alias_after_native_outputs_stays_native) :-
    clear_type_declarations,
    assertz(user:(atom_string_alias(A, Out) :- atom_string(A, Tmp), Out = Tmp)),
    assertz(type_declarations:uw_type(atom_string_alias/2, 1, atom)),
    assertz(type_declarations:uw_type(atom_string_alias/2, 2, string)),
    once(compile_predicate_to_typr(atom_string_alias/2, [typed_mode(explicit)], Code)),
    once(sub_string(Code, _, _, _, "let atom_string_alias <- fn(arg1: char, arg2: char): char")),
    once(sub_string(Code, _, _, _, "let v3 <- arg1;")),
    once(sub_string(Code, _, _, _, "arg2 <- v3;")),
    \+ sub_string(Code, _, _, _, "(function("),
    \+ sub_string(Code, _, _, _, "Unknown predicate"),
    generated_typr_is_valid(Code, exit(0)).

test(sub_atom_alias_after_native_outputs_stays_native) :-
    clear_type_declarations,
    assertz(user:(sub_atom_alias(A, Out) :- sub_atom(A, 0, 2, _, Tmp), Out = Tmp)),
    assertz(type_declarations:uw_type(sub_atom_alias/2, 1, atom)),
    assertz(type_declarations:uw_type(sub_atom_alias/2, 2, atom)),
    once(compile_predicate_to_typr(sub_atom_alias/2, [typed_mode(explicit)], Code)),
    once(sub_string(Code, _, _, _, "let sub_atom_alias <- fn(arg1: char, arg2: char): char")),
    once(sub_string(Code, _, _, _, "let v3 <- @{ substr(arg1, 1, 2) }@;")),
    once(sub_string(Code, _, _, _, "arg2 <- v3;")),
    \+ sub_string(Code, _, _, _, "(function("),
    \+ sub_string(Code, _, _, _, "Unknown predicate"),
    generated_typr_is_valid(Code, exit(0)).
...
test(transitive_closure_template_is_valid_typr) :-
    ...
    assertz(type_declarations:uw_type(edge/2, 2, atom)),
    once(compile_predicate_to_typr(tc/2, [base_pred(edge), typed_mode(explicit)], Code)),
    once(sub_string(Code, _, _, _, "while (queue_index <= length(queue))")),
    once(sub_string(Code, _, _, _, "let tc_all_from_vectors <- fn(start: char, from_nodes: [#N, char], to_nodes: [#N, char]): [#N, char]")),
    once(sub_string(Code, _, _, _, "while (i <= length(from_nodes)) {")),
    once(sub_string(Code, _, _, _, "while (neighbor_index <= length(neighbors)) {")),
    once(sub_string(Code, _, _, _, "let next_node <- neighbors[neighbor_index];")),
    once(sub_string(Code, _, _, _, "neighbor_index <- neighbor_index + 1;")),
    once(sub_string(Code, _, _, _, "while (queue_index <= length(queue) && !found)")),
    \+ sub_string(Code, _, _, _, "@{"),
    generated_typr_is_valid(Code, exit(0)).
...
test(transitive_closure_seeds_known_base_facts) :-
    ...
    assertz(type_declarations:uw_type(edge/2, 2, atom)),
    once(compile_predicate_to_typr(tc/2, [base_pred(edge), typed_mode(explicit)], Code)),
    once(sub_string(Code, _, _, _, "let edge_from: [#N, char] <- [\"a\", \"b\"];")),
    once(sub_string(Code, _, _, _, "let edge_to: [#N, char] <- [\"b\", \"c\"];")),
    once(sub_string(Code, _, _, _, "tc_all_from_vectors(start, edge_from, edge_to)")).
...
test(recursive_compiler_supports_typr_per_path_visited_recursion) :-
    clear_type_declarations,
    ...
    once(sub_string(Code, _, _, _, "# Generated by UnifyWeaver TypR Target - Per-Path Visited Recursion")),
    once(sub_string(Code, _, _, _, "let category_parent_neighbors <- function(current, from_nodes, to_nodes)")),
    once(sub_string(Code, _, _, _, "let category_parent_from <- [\"a\", \"b\", \"c\", \"b\"];")),
    once(sub_string(Code, _, _, _, "let category_ancestor_worker <- function(current, visited, from_nodes, to_nodes)")),
...
test(recursive_compiler_supports_typr_weighted_per_path_visited_recursion) :-
    clear_type_declarations,
    assertz(user:category_parent(a, b)),
    assertz(user:category_parent(b, c)),
    assertz(user:category_parent(c, a)),
    ...
    once(sub_string(Code, _, _, _, "# Generated by UnifyWeaver TypR Target - Per-Path Visited Recursion")),
    once(sub_string(Code, _, _, _, "let category_ancestor_weight_worker <- function(current, visited, from_nodes, to_nodes)")),
...
test(recursive_compiler_supports_typr_mode_driven_per_path_visited_recursion) :-
    clear_type_declarations,
    assertz(user:category_parent(a, b)),
    assertz(user:category_parent(b, c)),
    assertz(user:category_parent(c, a)),
...
test(recursive_compiler_supports_typr_mode_driven_weighted_per_path_visited_recursion) :-
    clear_type_declarations,
    assertz(user:category_parent(a, b)),
    assertz(user:category_parent(b, c)),
    assertz(user:category_parent(c, a)),

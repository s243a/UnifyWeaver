...
smoke_cases([
    ...
    case('wam_reverse_guard/2', args('[a,b,c]', '[c,b,a]'), "true"),
...
assert_lowered_reverse_builtin_emitted(ProjectDir) :-
    directory_file_path(ProjectDir, 'src/generated/wam_exec_test/core.clj', CorePath),
    read_file_to_string(CorePath, CoreCode, []),
    has(CoreCode, "defn lowered-wam-reverse-guard-2"),
...
assert_lowered_ground_builtin_emitted(ProjectDir) :-
    ...
    has(CoreCode, "defn lowered-wam-atom-string-reverse-mismatch-0"),
    ...
    has(CoreCode, "defn lowered-wam-string-to-atom-reverse-mismatch-0"),

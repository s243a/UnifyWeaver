...
test(atom_literal_preserves_quoted_numeric_atom, [nondet]) :-
...
test(integer_literal_uses_value_intval, [nondet]) :-
...
test(runtime_template_exposes_executable_wam_abi) :-
    ...
    assertion(has_substring(Code, 'data class Instruction')),
    assertion(has_substring(Code, 'class WamProgram')),
    assertion(has_substring(Code, 'class WamRuntime')),
    assertion(has_substring(Code, 'fun unify(left: Value?, right: Value?): Boolean')),
    assertion(has_substring(Code, '"get_variable"')),
...
test(generated_project_compiles_and_runs_fact_and_variable_core, [condition(gradle_available), nondet]) :-
    ...
    setup_call_cleanup(
        ...
        (   wam_kotlin_target:write_wam_kotlin_project(
                [user:kt_fact/2, user:kt_same/2, user:kt_eq/2],

...
%
% Bare scalar-variable patterns: a user scalar compared to an integer literal as
% a rule guard (`n > 2 { … }`, and the reversed `2 < n`). Parses to
% scalar_cmp(Name, Op, Value); the codegen resolves Name to the rule's current
% slot SSA value (rule 0: the loop-head phi; rule N>0: the per-rule input phi) and
% emits an i64 icmp, so it composes with `!`/`&&`/`||` and the other base
% patterns. This is the scalar-read-in-guard plumbing that field-vs-scalar
% patterns (`$1 > n`) build on.
%

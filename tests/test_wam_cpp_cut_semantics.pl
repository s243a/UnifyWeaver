...
%
% test_wam_cpp_cut_semantics.pl
%
% Port of tests/test_wam_javascript_cut_semantics.pl (via the Go / Rust
% ports) to the C++ WAM backend. Every probe pNN is a failure-driven loop
% that prints ALL solutions of pNN_t/1. The SAME clauses run under
% SWI-Prolog (the oracle) and under the compiled C++ binary; stdout must
% match after whitespace / true-false / A-register-dump stripping.
%
% The barrier model these pin down is docs/WAM_BACKEND_CONVENTIONS.md §9:
% cut is a barrier, never a stack wipe; BOTH call and execute set B0 <- B;
% a cut inside call/1, \+, findall/bagof/setof or an if-then-else CONDITION
% is local to that scope and never prunes the caller's alternatives.
%
% The C++ backend is driven at its interpreter lane via the generated
% `main` shim (`vm.query("pNN/0", [])`), with emit_mode(functions) +
% wam_fallback(true) so lowered functions coexist with the interpreter
% labels the drivers reach. Probes whose shapes wam_cpp cannot compile are
% recorded in cut_probe_refused/2 and generation runs with
% on_compile_error(throw), so an un-compilable shape FAILS LOUDLY at build
% time — it never silently drops a predicate and passes vacuously.
% Currently every probe compiles and matches SWI (0 refused).
%
% Three defect classes this corpus surfaced and fixed (each red on the
% pristine tree, green here):
%   * `!` reached as a meta-called goal-term (call/1, call((G,!))) was
%     unhandled by invoke_goal_as_call and failed outright — p09, p10.
%     Fix: a `!` case cutting to the call-scope barrier dispatch_call_meta
%     records (`meta_call_barrier`); call/N is a cut barrier.
%   * `!` inside a findall/bagof/setof goal opened by the SAME clause cut to
%     the clause b0, BELOW the aggregate's generator base, collapsing the
%     aggregate's own backtracking and any choice point a goal before the
%     aggregate left live — p13 (`e(Y), findall(X,(d(X),!),L)`), p29
%     (findall-with-cut nested in \+). Fix: the !/0 builtin never prunes
%     below the innermost open aggregate's base_cp_count.
%
% Skipped automatically when no C++ compiler is available.
%
%   swipl -q -g run_tests -t halt tests/test_wam_cpp_cut_semantics.pl

:- module(test_wam_cpp_cut_semantics,
          [test_wam_cpp_cut_semantics/0]).
...
% ---------------------------------------------------------------------
% The probe program. Same clause terms as the JS / Go / Rust suites.
% ---------------------------------------------------------------------
...
% Shapes wam_cpp cannot compile: Name-Reason. Empty means all 35 compile.
...
% The generated main prints a trailing true/false; strip that and all
% whitespace so only the probe's own write/1 lines are compared.
normalize_output(Raw, Out) :-
    split_string(Raw, "\n", "\r", Lines0),
...
compile_cut_probes(Dir, Bin) :-
    cpp_compiler(CC),
    cut_probe_preds(Preds),
    Dir = 'output/test_wam_cpp_cut_semantics',
    ( exists_directory(Dir) -> delete_directory_and_contents(Dir) ; true ),
    % on_compile_error(throw): an un-compilable probe shape is a LOUD build
    % failure, never a silently dropped predicate that passes vacuously.
    write_wam_cpp_project(Preds,
                          [module_name(cutcpp), emit_main(true),
                           emit_mode(functions), wam_fallback(true),
                           on_compile_error(throw)], Dir),
    !,
    atomic_list_concat([Dir, '/cpp'], CppDir),
    format(atom(BuildCmd),
        '~w -std=c++17 -O0 ~w/main.cpp ~w/generated_program.cpp \c
         ~w/wam_runtime.cpp -o ~w/cutmain 2>&1',
    ...
    (   Status == exit(0)
    ->  true
    ;   format(user_error, '~n[cpp cut-semantics build failed]~n~w~n',
               [BuildOut]),
        throw(cpp_cut_build_failed(Status))
    ),
    atomic_list_concat([CppDir, '/cutmain'], Bin).

test_wam_cpp_cut_semantics :-
    run_tests(wam_cpp_cut_semantics).

:- begin_tests(wam_cpp_cut_semantics, [condition(cpp_compiler(_))]).

test(matches_swi, [setup(install_cut_probes)]) :-
    compile_cut_probes(Dir, Bin),
    !,
    ...
    findall(fail(N, Ctx, Swi, Cpp),
            (   cut_probe(N, Ctx),
                \+ cut_probe_refused(N, _),
                swi_probe_output(N, Swi),
                cpp_probe_output(Bin, N, Cpp),
                Swi \== Cpp
            ), Fails),
    (   Fails == []
    ->  true
    ;   forall(member(fail(N, Ctx, S, C), Fails),
               format(user_error,
                      'CUT PROBE DIVERGENCE ~w (~w)~n  swi: ~q~n  cpp: ~q~n',
                      [N, Ctx, S, C])),
        fail
    ),
    ( exists_directory(Dir) -> delete_directory_and_contents(Dir) ; true ).

:- end_tests(wam_cpp_cut_semantics).

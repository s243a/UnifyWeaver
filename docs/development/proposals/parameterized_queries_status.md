...
- Added query-mode aggregates (`aggregate_all/3,4`, including correlated aggregates) via an `aggregate` plan node and C# `AggregateNode` runtime support.
  ...
  - QueryRuntime memoizes correlated aggregate keys (pattern/parameter tuples) per execution and caches base-relation materializations to avoid repeated scans/subplan evaluation when keys repeat.

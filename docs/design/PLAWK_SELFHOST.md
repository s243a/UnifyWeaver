...
### Stage A — `.wamo` serializer in the loadable subset

Port `wamo_serialize/8` to a loadable grammar `serialize_wamo(Instrs, Meta,
Wamo)`. Input is a **fixed instruction list** given as ground terms (no
codegen yet); output is the `.wamo` byte string. Test: feed the instruction
list for `p(R) :- R = 42`, load the emitted bytes via `@wam_object_eval`,
run the entry → `42`. This is the highest-risk piece (operand encoding) and
the most mechanical, so it goes first and locks the format against a golden
object generated by the host writer.

**Deliverable:** a loaded grammar that emits a valid `.wamo` from a
hand-supplied instruction list. Verified: emitted object loads and runs.

...
- [`WAM_LMDB_LAZY_IMPLEMENTATION_PLAN.md`](WAM_LMDB_LAZY_IMPLEMENTATION_PLAN.md) —
  ...
  already implements much of `cached`).
...
**`eager`** — eager in-process materialisation, with the same memory wall
the Rust bench has.

The Rust bench today is **`eager`**. We have no `lazy` or `cached` variant yet, and
...
`lazy wall ≤ cached wall` only when `θ × p ≤ N × (p − q)`, which simplifies
...
graph locality, θ drops quickly and `cached` wins.

The non-obvious result is that **`lazy` only beats `cached` under specific
...
won't happen anyway. Most real workloads have *some* locality, so `cached`
...
The cost-model resolver should pick **`cached` by default**, **`eager` when
...
the cache that `cached` would maintain has zero hits, and `lazy`'s no-cache
shape is pure win.
...
then within each batch `lazy` is correct, and across batches the cache
...
cache misses. `eager` doesn't care because it pays for materialisation
once anyway.
...
1. *Is demand_set_size × edge_size > available_memory_budget?* → must use lazy (`lazy` or `cached`).
...
The `lazy`/`cached` distinction is what's new here; scan-mode is a related
axis that would be added per-target.

## 6. Cross-language patterns this maps to

`lazy` and `cached` are general patterns. The iterator-based lookup interface
...
tier layers on top via a Decorator pattern: the cache layer wraps a `lazy` source
...
3. **How does this compose with the runtime planner pattern
   ([`QUERY_PLAN_RUNTIME_PHILOSOPHY.md`](QUERY_PLAN_RUNTIME_PHILOSOPHY.md))?**
   The planner is the right place to decide `eager`/`lazy`/`cached` + seek/scan
...
5. **Cache invalidation under graph mutation**: the current Haskell
   the cache layer assumes a static LMDB. Mutating workloads need a
   versioning story.
6. **Cross-target benchmark methodology**: when we measure `lazy`/`cached`/
   scan-mode against `eager`, we need a workload spec that exercises
   each mode's niche. The existing matrix-bench is `eager`-shaped;
   it'd over-amortise.
...
- `docs/design/COST_FUNCTION_PHILOSOPHY.md` — Green's-function and
  ...
  inputs.
- `docs/design/WAM_PERF_OPTIMIZATION_LOG.md` Phase L#7–9 — Haskell `cached`

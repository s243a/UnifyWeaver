...
**Implementation cost is bounded.** A first-pass implementation should keep the window small (W ~ 5–10) and the bookkeeping cheap; specific implementation guidance (deque + running sum-of-squares for O(1) amortised, why to avoid FFT-based spectral techniques) lives in the [implementation plan's out-of-scope section](RECURRENCE_EVALUATION_STRATEGY_IMPLEMENTATION_PLAN.md#out-of-scope-for-this-iteration) where parallel future-work notes are collected.

**Where it would fit in the admissibility framework.** A future `numeric_windowed_rms_bound(WindowSize, RmsThreshold)` recurrence property could be added to the SPEC alongside the existing `numeric_contraction_rate(R)`. The `WindowSize` argument is per-recurrence — different recurrences may need different windows depending on their oscillation profile. A global default (e.g. `W = 10`) would apply when no per-recurrence window is specified, configurable via something like `set_default_windowed_rms_window_size(+N)` so users can tune the default for their workload without touching each recurrence declaration. The admissibility test would gain a fourth termination path:
...
termination_guarantee(Recurrence) :-
    value_domain(Recurrence, combinatorial), !.    % finite-state iteration always halts (in principle):
                                                   ...
                                                   %     cycle-detection. NOTE: cycle-detection is NOT yet
                                                   %     implemented in any UnifyWeaver fixed_point template;
                                                   %     the non-monotone branch is admitted as a future-
                                                   %     compatibility design choice. Do not rely on this
                                                   %     branch for non-monotone inputs until an evaluator
                                                   %     ships with cycle-detection — currently it would loop.
...
**This identification is a conjecture, not a theorem.** Rigorously identifying `r` with the spectral radius of the iteration operator requires a norm-specific argument that has not been constructed in the project to date. The intuition is sound — the friendship-paradox quantity `E[d²]/E[d]` is a known estimator for the spectral radius of a configuration-model random graph's adjacency matrix, and `b_eff` is a directional extension of this — but a precise proof would tighten three things at once: (i) which norm the spectral analysis is being done in, (ii) which operator linearisation of the `d_wPow` iteration is being analysed, and (iii) the assumption that the **weighted-degree distribution is approximately uncorrelated** (i.e. the configuration-model assumption applies to the weighted directed graph the iteration operator is built from — without this, the friendship-paradox estimator may diverge from the spectral radius even in expectation). The rigorous identification is tracked theory work, named explicitly as a future-work item in [book-18-graph-algorithms appendix B.13](../../education/book-18-graph-algorithms/13_appendix_b_internal_theory.md) (`r = b'/(b_eff·D)` as contraction rate). Not blocking; this PR proceeds on the conjecture, with the hedge documented and the dependence on the conjecture made operationally visible (see below).

**Empirical degree-correlation note for Wikipedia.** Assumption (iii) above is the most load-bearing for real-world graphs. The Wikipedia category graph has heavy-tailed degree distributions and moderate degree correlations (high-degree categories are slightly more likely to link to other high-degree categories, and vice versa); whether these correlations are *small enough* to keep the configuration-model assumption operationally valid for `b_eff` has not been formally measured in the project. The empirical tree-likeness measurements on simplewiki and enwiki are consistent with the conjecture holding to within useful precision (the per-pair check from design note §6.3 passes at high rates), but the gap between "consistent with" and "validated" is real. This is one of the empirical work items future iterations should close.
...
> *Confidence-weight constants (e.g. the specific multiplier applied to inferred signals) are defined in the [SPECIFICATION](RECURRENCE_EVALUATION_STRATEGY_SPECIFICATION.md#scoring-example), not here. The philosophy doc describes the principle; the SPEC is the single source of truth for the numeric values.*

...
- **Mock-second-caller test** (`tests/core/test_recurrence_inputs_mock_caller.pl`): a mock caller imitating a hypothetical Haskell-WAM integration exercises `build_recurrence_term/3` and `build_workload_signals/2` with workload signals that have no F# semantics. Catches the failure mode where the helpers were silently shaped by F# as the only first-iteration caller, freezing the API with implicit F#-specific assumptions before the second target validates it. The mock-second-caller doesn't have to *produce* working Haskell code; it just has to exercise the helpers in a way that proves they're not F#-shaped.
  ...
  *Concrete signal terms the mock should pass* (to make "no F# semantics" implementable): the mock constructs a workload signal list containing only signals from the SPEC's target-agnostic vocabulary table — `cardinality(large)`, `query_frequency(high)`, `query_pattern(single_pair)`, `b_eff(15.0)`, `branching_d(4.5)`, `contraction_r(0.04)`, `heuristic_predicate_available(true)`, `heuristic_predicate(my_heuristic/2)`. Critically, no `csr_path(_)` option (which is F#-WAM-specific; the F# target adapter resolves it from `csr_path` to the on-disk CSR location). The mock then asserts that `build_recurrence_term/3` and `build_workload_signals/2` return non-error well-formed terms. If the helpers had been silently shaped by F# (e.g. by assuming a CSR path field is always present or by populating an F#-specific kernel option), this assertion would fail. The mock additionally checks that the helper modules don't import any target-specific modules (verified by reading `:- use_module/1` directives in `recurrence_inputs.pl` and asserting none of the imports name `fsharp` / `wam_fsharp` / `fsharp_wam_target` or analogous Haskell/C target modules).
...
- `tests/core/test_recurrence_evaluation_strategy.pl` — integration test running the full pipeline end-to-end with several realistic workload + recurrence combinations:
  ...
  - **`monotone(false)` recurrence + caller intent requesting cross-class strategy** — full pipeline scenario complementing the Phase 3 unit test. Runs `select_evaluation_strategy/3` end-to-end on a `monotone(false)` recurrence with a caller intent that crosses strategy classes from the cost-model's preference; verifies `step_satisfiability` and `step_third_option` both refuse the cross-class adjustment (per the SPEC's [`monotone(false)` mechanism note](RECURRENCE_EVALUATION_STRATEGY_SPECIFICATION.md#step_satisfiability--adjust-the-unsatisfiable) and the [`step_third_option` monotone(false) clause](RECURRENCE_EVALUATION_STRATEGY_SPECIFICATION.md#step_third_option--search-for-a-strategy-that-satisfies-all-intents)); resolution falls through to `step_caller_wins` with a cross-class warning recorded in the trace. The Phase 3 unit test verifies the resolution-machine behaviour in isolation; this Phase 6 test verifies the full-pipeline integration including signal classification, cost-model evaluation, and trace rendering.

...
| `RS` | ✅ | **input record separator, any literal string — plus paragraph mode.** `BEGIN { RS = "…" }` splits the input on a single- or multi-char literal (`";"`, `"||"`, `"-|-"`) instead of newline. Both runtime readers (`@wam_stream_read_line_transient_value` and its persistent sibling) append each byte and detect a boundary when the RS byte-string is a **suffix** of what has been appended (a `memcmp` against the `@wam_rs_ptr` / `@wam_rs_len` globals, which a `BEGIN { RS = "…" }` sets from a `@.wam_rs_custom` constant at startup via the FS-regex-style seam). RS composes with FS (`BEGIN{RS="||";FS=":"} {print $2}` over `a:1||b:2||c:3` → `1 2 3`). The trailing-CR strip (CRLF handling) is gated on RS being exactly a one-byte newline, so a custom RS keeps a `\r` as data and the default newline still strips CRLF. **An empty `RS` is paragraph mode** (`store i64 0, i64* @wam_rs_len`): each reader skips leading blank lines, treats two adjacent newlines (`"\n\n"`) as the record boundary, collapses runs of blank lines into one boundary, and strips a lone trailing newline at EOF — a paragraph may span multiple lines, and the default whitespace FS splits `$N` across the embedded newlines. Follow-on — **clean rejection, never miscompiled**: a regex `RS` (gawk). `getline`'s own record separator (a separate reader) is also a follow-on |
| `SUBSEP` | ◐ | **multi-dimensional subscripts landed** for the two-field key. `arr[$i,$j]` joins the two field values with SUBSEP into one string key (awk `arr[i SUBSEP j]`): the write counter `arr[$i,$j]++`, the element read `arr[$i,$j]`, and for-in over the joined keys. The join runs in the runtime helper `@wam_intern_subsep_key2` (slice both fields → join with the SUBSEP bytes → intern to one atom id — the same key both write and read build), keyed off `@wam_subsep_ptr`/`@wam_subsep_len` (default the `0x1C` byte, awk's SUBSEP). `BEGIN { SUBSEP = "…" }` overrides it (any string, incl. empty = no separator) via the RS-style begin-globals + begin-instructions seam. Follow-ons — **clean rejection, never miscompiled**: three-plus subscripts (`arr[$i,$j,$k]`), non-field subscripts (`arr[$i,"x"]`), and multi-dim on the other write shapes (`=`, `+=`, `delete`) / binfmt records / `(i,j) in arr` membership |
...
| multi-dim `arr[i,j]` (SUBSEP) | ◐ | `arr[$i,$j]` (two field subscripts joined by SUBSEP): counter `arr[$i,$j]++`, read `arr[$i,$j]`, for-in over joined keys; `BEGIN{SUBSEP="…"}` override. Three-plus / non-field subscripts a follow-on — see the `SUBSEP` row |

...
| `ORS` | ◐ | **output record separator landed (single char).** `BEGIN { ORS = "X" }` sets the byte printed after each `print` (default newline). It is sourced once (`plawk_output_record_separator`) into the `@.plawk_surface_print_newline` terminator global that every print emitter already references, so per-record **and** `END` prints honour it with no change to the emitters; it composes with `OFS` (`OFS` separates a print's fields, `ORS` terminates the record — `BEGIN{OFS="-";ORS=";"} {print $1,$2}` → `a-b;c-d;`). Follow-ons — each a **clean rejection, never a miscompile**: a multi-char `ORS` (the terminator global holds one byte), an empty `ORS` (a zero-length terminator), and a literal `%` (it would corrupt the `printf` terminator) |
| `RS` | ◐ | **input record separator landed (single char).** `BEGIN { RS = "X" }` splits the input stream on byte X instead of newline. The runtime readers (`@wam_stream_read_line_transient_value` and its persistent sibling) now compare each byte against the `@wam_rs_byte` global (default 10) rather than a hardcoded newline, and a single-char RS stores its byte into that global at startup (before the record loop, reusing the FS-regex startup-store seam). RS composes with FS (`BEGIN{RS=";";FS=":"} {print $2}` over `a:1;b:2;c:3` → `1 2 3`). The trailing-CR strip (CRLF handling) is gated on RS being newline, so a custom RS keeps a `\r` as data and the default newline still strips CRLF. Follow-ons — **clean rejection, never miscompiled**: a multi-char / regex RS (a rewrite of the reader loop) and an empty `RS` (paragraph mode). `getline`'s own record separator (the WAM getline builtin, a separate reader) is also a follow-on |
| `SUBSEP` | ❌ | multi-dim `arr[i,j]` subscript joining not yet modelled |

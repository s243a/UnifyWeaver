...
| numeric: `int` `sqrt` `sin` `cos` `exp` `log` `atan2` | ✅ | `int` (truncation) plus the **libm math builtins**: `sqrt`/`sin`/`cos`/`exp`/`log` (unary) and `atan2` (binary). Each parses to `math_call(Fn, Args)` and lowers through the double expression path — the argument is evaluated in f64 (an integer field promotes via `sitofp`, so `sqrt($1)` works), the libm function is called, and the result prints via `%g` (`sqrt(16)` → `4`, `sqrt(2)` → `1.41421`). `log` is the natural logarithm. Arguments may be full arithmetic expressions (`sqrt($1 * 4)`, `sqrt(2^4)`), and a call composes inside larger arithmetic (`2 * sqrt(9)`) and `printf`. A name is only a builtin when directly followed by `(`, so `sine` stays an identifier. Same supported surface as exponentiation (print / printf / arithmetic value positions); guards and double-scalar-reread are the same pre-existing limitations. `rand`/`srand` (PRNG state) ❌ |

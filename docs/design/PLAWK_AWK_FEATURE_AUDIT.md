...
| `RS` `SUBSEP` | ❌ | single newline-delimited record model. `RS` (input record separator) is invasive: the record boundary is a hardcoded newline (`icmp eq i8 %ch, 10`) in the runtime reader `@wam_stream_read_line_transient_value` (plus a CRLF-trim assumption), with no separator seam — a follow-on that needs runtime-reader work |

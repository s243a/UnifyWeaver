...
| string: `length` `substr` `index` `tolower` `toupper` | ✅ | native, allocation-free. **Bare `length`** (no argument) is awk shorthand for `length($0)` — it parses to the same `length(field(0))` AST, so it works in print, arithmetic (`length + 1`), and scalar assignment (`n = length`); `length(...)` / `length (...)` stay calls and `lengthy` stays an identifier. Follow-on: 2-arg `substr(s, m)` (to end of string) |

...
| range pattern (`/a/,/b/`) | ✅ | `start, end { … }` fires for records from a `start` match through an `end` match (inclusive), via a per-rule i1 latch global; re-arms for later ranges, runs to EOF if unterminated. **Endpoints are general base patterns**, each reusing the ordinary pattern-guard lowering: regexes (`/S/,/E/`), expression patterns (`NR==1, NR==3`), field comparisons, and any mix (`NR==2, /end/` or `/b/, NR==4`). A single pattern with no comma is unaffected (falls through to the ordinary rule). Combinator endpoints (`a && b, c`) remain a follow-on |

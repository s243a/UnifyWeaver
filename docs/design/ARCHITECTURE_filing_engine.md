...
- `DECIDED` — a standing decision. Its row includes a one-line evidence
  pointer.
...
Append new rows instead of recycling IDs. When an item changes status, update
...
| ID | Status | Operating shape | Evidence |
|---|---|---|---|
...
| ID | Status | Architectural element | Evidence, owner, or resolution |
|---|---|---|---|
| RANK-001 | `DECIDED` | Frozen e5 cosine is the filing ranker's backbone. The learned μ heads are not the primary ranker. | [Pearltrees candidate-lineage report](../../prototypes/mu_cosine/REPORT_pearltrees_candidate_lineage.md): e5 MRR was 0.294 while μ-head MRRs were at most about 0.11; [Filing v1 report §5](../../prototypes/mu_cosine/REPORT_pearltrees_filing_v1.md) confirms the corrected null. |
| RANK-002 | `DECIDED` | Candidate recall is the current binding ceiling: over the fixed e5 top-50 candidate set, recall of 0.680 means no reranker can recover the remaining 0.320. | [PR #3865 corrected report](https://github.com/s243a/UnifyWeaver/pull/3865), candidate-recall readout. |
...
| GEOM-002 | `DECIDED` | For a retained domain, deleted cut edges become exact zero-bath Dirichlet shunts `β`; they are not silently dropped. A shared union-of-anchors domain uses one precision matrix and one factorization. | [Local grounded diffusion, “The exact Dirichlet cut” and “Multiple anchors and positive semidefiniteness”](LOCAL_GROUNDED_DIFFUSION.md) derives the principal block and requires one-domain/one-factor solves for a coherent PSD geometry. |
...
| GEOM-005 | `DECIDED` | Exact reduction rules are constraints on every candidate skeleton: a one-port subtree may be Kron-reduced to a scalar shunt; an ungrounded dangling subtree has zero DC shunt; eliminating any multi-port subtree, including a tree, creates boundary-to-boundary fill. Fill is not a cycle-only effect. | These are direct Laplacian/Schur-complement identities and refine the exact-marginal alternative deferred in [Local grounded diffusion, “Rejected and deferred alternatives”](LOCAL_GROUNDED_DIFFUSION.md). |
| GEOM-006 | `OPEN` | **Owner: rigor-lane.** Add at most a budgeted sparse set of semantic boundary-closure edges as an outcome-blind approximation to exterior Schur fill. This would be an explicit, boundary-only exception to the rule in `GEOM-001` that semantics only modulates existing edges. Closure must **reallocate** each boundary's cut mass between induced couplings and residual grounding; it must not add couplings on top of the full Dirichlet `β`. | Resolve by matching a larger-domain or exact small-graph Schur reference better than plain Dirichlet at the same node budget while preserving symmetry, nonnegative conductances, M-matrix/SPD safety, and an auditable per-boundary mass ledger. |
...
| TEACH-004 | `DECIDED` | The cheap-judge system is a label factory: retain train-only global affine debiasing, correlated fusion, and the free graph channel as cheap supervision. Debiased Luna is the paid S workhorse when that channel is purchased. | [Cheap-judge baseline](../../prototypes/mu_cosine/REPORT_cheap_judge_baseline.md) records Luna debiasing and the graph-S increment; [post-#3648 validation](../../prototypes/mu_cosine/REPORT_cheap_judge_post3648_validation.md) narrows the economics: there is no universal exact-budget matched-cost spending win across tasks or partitions. |
...
| FUSE-004 | `DECIDED` | μ heads serve calibration, label fusion, and conflict routing—not primary filing ranking. | [Filing v1 report](../../prototypes/mu_cosine/REPORT_pearltrees_filing_v1.md) and [candidate-lineage report](../../prototypes/mu_cosine/REPORT_pearltrees_candidate_lineage.md) show the label-factory value and the ranking null. |
...
| FUSE-006 | `OPEN` | **Owner: rigor-lane.** The dual-space residual state `δ̃` and state-level constraint covariance `R_c` remain parked. Row residual scatter `R̂_row` is an upper bound that mixes item noise with state discrepancy and must not be substituted as a tight `R_c`. | Resolve using replicated or cross-fitted bin/campaign effects that separate row scatter from state discrepancy, followed by held global benefit for the joint direct/logit state. See [bias-state design §3](../../prototypes/mu_cosine/DESIGN_bias_state_augmentation.md). |
...
| ID | Status | Recorded choice | Evidence |
|---|---|---|---|
...
| DR-004 | `DECIDED` | Fit Luna's global affine correction on training rows before covariance fitting; use debiased Luna as the primary paid S measurement and retain graph S as free supervision. This is a role decision, not a universal matched-cost victory. | [Cheap-judge baseline](../../prototypes/mu_cosine/REPORT_cheap_judge_baseline.md) and corrected [post-#3648 validation](../../prototypes/mu_cosine/REPORT_cheap_judge_post3648_validation.md). |
...
| Priority | ID | Status | Question | Owner and resolving evidence |
|---:|---|---|---|---|

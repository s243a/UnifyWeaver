...
| G-P11 | clojurescript | **Dead bindings — CONFIRMED.** `bindings/clojurescript_bindings.pl` (67, key `clojurescript`) is never `use_module`'d, so its `:- initialization(declare_binding(...))` never runs; and compile routes via `compile_predicate_to_clojure` (key `clojure`) + interop-rewrite, so the CLJS key is never consulted. The 67 bindings are inert. Fix = a decision: wire the `clojurescript` binding key into the compile path, OR remove/document them as superseded by the interop rewrite (the file header calls the rewrite the real mechanism). | ⬜ decision | clojure binding path | S–M | opus/main |

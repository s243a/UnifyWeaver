...
- **WAM (lane exhausted for term model/builtins):** AVL assoc (list-based today); LMDB/CSR fact sources (out of scope, matches Lua peer); G-W6 parallelism (deferred). Only remaining string residual: compiled `"foo"` literals collapse to atom (shared WAM constant tokens — needs `wam_target.pl`, cross-target blast radius); plus `\n`/`\t` escapes in `writeq` and brace-term `{a}` special-casing (cosmetic). `op/3`/postfix (D32), real string tag (D34), and `string_length/2`+`writeq`/`~q` quoting (D35) now landed.

...
| G-P7 | typescript (→AJS/VJS) + clojure/CLJS | **Guard-goal codegen gap (RE-SCOPED by recon).** "Constraints" fleet-wide = guard codegen, NOT CLP(FD)/`dif` (no target implements those — drop that framing). TS/Clojure already compile comparison guards via shared `clause_body_analysis.pl`, but `ts_guard_condition/3` (`typescript_target.pl:1171`) and `clojure_guard_condition/3` (`clojure_target.pl:1091`) have **no clause for `\+`/`not` negation or type-check preds** (integer/1, atom/1, …) — classified as guards upstream then fail at render. Minimal slice = add those two clause families to both. Regex `match/2,3` = separate optional follow-up (python `translate_match/6` model). Separate smaller gap: `constraint_analyzer.pl` unique/unordered dedup (rust/go call `get_constraints/2`; TS/CLJS don't). | ⬜ | python_target guard/negation; clause_body_analysis | S–M | opus |

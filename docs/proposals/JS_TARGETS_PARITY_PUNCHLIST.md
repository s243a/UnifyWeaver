...
- **WAM (lane exhausted for term model/builtins):** AVL assoc (list-based today); LMDB/CSR fact sources (out of scope, matches Lua peer); G-W6 parallelism (deferred). String-tag residuals (D34): compiled `"foo"` literals collapse to atom (shared WAM constant tokens — needs `wam_target.pl`); `writeq`/`~q` nested-string quoting; `string_length/2`. `op/3`/postfix (D32) and real string tag (D34) now landed.

...
9. **Tidy the agent relay-prompt docs — DONE (D99, 2026-09-11).** The 27 single-use relay/handoff prompts (25 `GROK_*_PROMPT.md` + `LOCAL_GEMINI_SUPERVISOR_PROMPT.md` + `CODEX_HANDOFF_MODE_DIRECTED_TABLING.md`) were moved to `docs/proposals/archive/agent-prompts/` (with a README) to declutter `docs/proposals/`; git history preserved via `git mv`. Design/reference docs that merely mention an agent (`AGENT_LOOP_*`, `prompt-context-management.md`) stayed put. The three inbound links (from `JS_TARGETS_DELEGATION_PLAN.md`, `PKG_MINING_NOTES.md`, and this item) were repointed.

...
- **Status:** Done — `examples/benchmark/generate_wam_r_effective_distance_benchmark.pl`, guarded Rscript harness block, scale-300 matrix row + subsection in `WAM_CROSS_TARGET_BENCHMARK_RESULTS.md` (hosted-CI NATIVE-HOPS-0 query_ms=270 median / total_ms=776; prior AGG-BATCH 1111/1621; AGGREGATE-LOWER 1564/2077; IDTABLE 3412/4076; reference multiset parity match). Initial BENCH-R branch `cursor/bench-r-effective-distance-f421`; optimized by PERF-R-CA-DIRECT, IDDFS, IDCACHE, STACK, IDTABLE, WAM-STEP (`is_lax` power/unary), AGGREGATE-LOWER (`bulk_collect` + scalar is_lax reduce), AGG-BATCH (typed numeric batch + vectorized is_lax reduce), and NATIVE-HOPS-0 (optional `.Call` CA hop kernel with pure-R fallback). PATHMARK, LOOPBODY, ED-BOUNDARY, LOWERED-DIRECTCALL, CA-BULK-HOPS (post-AGG-BATCH hops_ids ~84% of query; R-level trials ≤~1.016×), POST-NATIVE-0 (post-native residual ~84–90% in `run`/`step`; plan-cache / closed-pow trials ≤~1.03×), BULK-REDUCE-REGION-0 (direct fused ≈2.04× / ≈1.20× smoke but ≈320 LOC soft-stop; thin step-driver ≈0.97×; declined), and BULK-REDUCE-PLAN-1 (compact generate-time plans, 180 LOC fail-closed/transactional; REGATE on `9071b8a8c` official 7×2 ≈1.23×/1.21×, hosted 161/685; pre-review da50306b superseded; retained) profiled.

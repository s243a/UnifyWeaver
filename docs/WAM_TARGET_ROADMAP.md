...
| **F#** | Haskell-shaped target on the .NET runtime | mirrors Haskell coverage | dual: WAM-instr + emitter | LMDB `eager`/`lazy`/`cached` (+ two-level L1/L2); CSR reverse-index reader | shared graph kernels + **bidirectional_ancestor**; TPL `Parallel.map` / negation race | register classic conformance adapter; Elixir-style cost-gate calibration; LMDB scan/segregation once Rust ships reference |
...
| **C++** | systems substrate + ISO-error reference | broad WAM; classic conformance green on onboarding | dual: WAM-instr + lowered emitter (deterministic / clause-1 / ITE) | arity-2 LMDB FactSource (v1) | `call_foreign` surface; not the full shared 7-kernel perf set | keep ISO contract as shared reference; grow shared-kernel parity if used for graph benches; watch compiled-parser compile cost |

...
| **Rust** | hand-tuned FFI kernel route | deterministic only in lowered emitter | dual: WAM-instr + emitter (deterministic only) | partial (LookupSource trait over LMDB lazy/cached; no full FactSource facade) | effective-distance matrix FFI kernel; **bidirectional_ancestor** (F# parity port: calibration + A*-pruned direction-cost search; opt-in `kernel_mode(bidirectional)`) | reverse-CSR child index (today: derived from eager parent table or LMDB `category_child`); distribution-cache phases; FactSource generalisation |

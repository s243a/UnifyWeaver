...
**Output.** Rendered text (template layer), registry-v0.5 canonical process-expression strings
...
2. **`pe_emit`** (`prototypes/mu_cosine/pattern_stache/`) — a template-driven emitter from
   goal terms to v0.5 canonical surface text, byte-exact against the sealed golden bundle.
...
## b. Goal term → v0.5 canonical surface, against a sealed oracle

`pe_emit` turns a goal term into registry-v0.5 canonical text: defaults resolved explicitly,
...
`PROCESS_EXPRESSION_GOLDEN_v4.json`, frozen with a content hash. Read the same bytes straight
from the bundle:

```console
$ python3 -c "import json; d=json.load(open('prototypes/mu_cosine/PROCESS_EXPRESSION_GOLDEN_v4.json')); print([r['canonical_identity_string'] for r in d['rows'] if r['name']=='lineage-haiku'][0])"
lineage(pearltrees,decay=0.85,estimand="ancestry",mu=haiku)
```

**Why sealed oracles:** the emitter is verified by byte equality against all 30 bundle rows
...
### b-addendum. What a registry bump looks like from here

The sealed bundle moves as the registry does. Registry v0.5 added the `cowalk` operator, whose
`walk` and `weight` kwargs are *enumerated* kinds — and whose canonical form injects a registry
default the author never wrote (`weight="uniform"`):

```console
$ swipl -g "use_module('prototypes/mu_cosine/pattern_stache/pe_emit'), pe_semantic(cowalk(enwiki, walk(sibling), estimand(path)), S), write(S), nl" -t halt
cowalk(enwiki,estimand="path",walk="sibling",weight="uniform")
```

**Why this was cheap:** the operator needed one new `{{case}}` in the surface template and its
two enumerated kinds needed one line each — the registry facts themselves arrived through the
generated, hash-checked mirror, regenerated by one command. That is the template-as-library
shape paying off: a new operator is a new case, not a driver change.

Numbers are the exception that needed real work. The v0.5 bundle added a scientific-notation
row, and SWI's own `~w` disagrees with the spelling the sealed renderer uses:

```console
$ swipl -g "use_module('prototypes/mu_cosine/pattern_stache/pe_emit'), pe_semantic(margin(t(1.0e-5)), S), write(S), nl" -t halt
margin(t=1e-05)
```

The goal term carries a *float*, not a spelling — Prolog reads `1e-05` and `1.0e-5` as the same
float — so the surface spelling is computed, by `pe_number.pl`, which implements CPython's
float-repr rule. **Why a whole module for one row:** the divergence is not cosmetic (SWI and
Python even switch to exponential notation at different magnitudes), and the bundle is the
contract.

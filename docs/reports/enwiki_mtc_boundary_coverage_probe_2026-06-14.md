...
The root-sample run answers a different question: it ignores boundary stopping and estimates the full root-reaching search-space size from branch-product weights over sampled simple parent walks. For the same depth-3 boundary layer and depth-6/8 targets, this is a search-space and mean-estimation probe, not a cache-boundary stopping measurement.

| run | mode | parent_filter | target_depth | targets | path_length_budget | samples_per_target | mean_estimated_root_paths | mean_estimated_mean_root_path_length | mean_estimated_root_boundary_hit_fraction |
|-----|------|---------------|-------------:|--------:|-------------------:|-------------------:|--------------------------:|-------------------------------------:|------------------------------------------:|
| enwiki_mtc_root_path_sample_b3_d6_8 | root-sample | root-cone | 6,8 | 4 | 10 | 1000 | 17.688 | 8.179 | 0.905622 |
| enwiki_mtc_root_path_sample_b3_d6_8 | root-sample | root-cone | 6,8 | 4 | 20 | 1000 | 19.924 | 8.323 | 0.900566 |

## Interpretation

For shallow nodes, exact enumeration is the preferred evidence, but it should be root-reachable filtered when the question is about paths to a specific root. The precomputed root-cone filter is the cheaper first approximation for larger runs, provided the report states the cone depth and sampling caps. For cache-boundary claims, sampled evidence should report both the raw random-walk hit rate and a weighted path-space estimate. For full search-space estimates, root-sample mode should walk to root and use the branch-product weight to estimate total root paths and carefully derived metric numerators.
...
- `docs/reports/lmdb_boundary_coverage_probe_summary_enwiki_mtc_root_path_sample_b3_d6_8_20260614T060254Z.md`
- `docs/reports/lmdb_boundary_coverage_probe_enwiki_mtc_root_path_sample_b3_d6_8_20260614T060254Z.jsonl`

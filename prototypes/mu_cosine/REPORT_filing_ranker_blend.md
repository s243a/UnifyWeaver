...
- **Grounded-diffusion screening h_s** — a RAW outcome-blind screening score, NOT a calibrated
  probability or calibrated screening score (the e-fold recipe is infeasible here; below). Dense
  float64 reference (docs/design/LEAKY_GRAPH_DIFFUSION.md): semantic conductance from e5 with
  ℓ = median edge e5 distance (0.5175), floor ε = 0.05. **DIRICHLET boundary (audit finding 1, fixed):** the 5,709 edges cut by the 2-hop
  truncation become shunts to the bath at their 1,385 boundary nodes (same RBF-with-floor
  conductance; 5,709 semantic + 97 topological-c₀ fallback edges [untitled exterior endpoints —
  no embedding; substituted mass 97.0 recorded]; total shunt mass 3,924) — the first run's insulating truncation was a wrong
  ...
  recipe under high surface-to-volume truncation, not hidden: h_s is used as a RAW screening score
  from the minimum-α / boundary-shunt operator — defensible descriptively while grounded and
  passing the numerical diagnostics. The 97 cut edges whose exterior endpoint has no title use the
  topological-c₀ = 1 fallback (no embedding available; fallback count and substituted mass
  recorded) — nothing fails open. Float64 contract passed (condition 3.0e3, reciprocal 3.3e-4). Source: unit current split over the bookmark's
...
| blend | 0.314 ± 0.041 | 0.233 ± 0.037 | 0.391 ± 0.048 |

(± values are spread over OVERLAPPING repeated splits — descriptive, not intervals.)

**Corrected-operator result on an EXPOSED split (seed 0, 192 held; seeds 0–4 were all inspected
during development, so this is NOT a pre-registered confirmatory test — the prospective protocol
(#3875) governs the next run). Paired two-endpoint node-block bootstrap on per-query Δ(1/rank),
95% CI: blend − e5 = +0.023 [−0.028, +0.082] — a null (CI straddles zero).**
SENSITIVITY NOTE: on the pre-fallback operator (97 cut edges omitted, ≈2.5% less boundary mass)
the same statistic was −0.003 [−0.061, +0.055] — the point-estimate SIGN flips under a small
operator perturbation, so the CI width, not the point, is the finding. Both operators are null. The five repeated
splits are OVERLAPPING (held 188–202; only 699/1,200 queries ever held) and are reported only as
descriptive stability: per-split ΔMRR mean +0.0198, [+0.023, +0.036, +0.012, +0.020, +0.008],
5/5 positive on the final operator (4/5 with seed 0 negative on the pre-fallback operator —
split-level signs are also operator-sensitive). The earlier headline ("first ranker to beat e5, 5/5 folds") was computed on the
...
EXPLORATORY (five families, no multiplicity handling — audit finding 4; the exposed-split
full−drop-diffusion bootstrap is likewise null: +0.015 [−0.021, +0.055]):

| family dropped | MRR | Δ |
|---|---|---|
| e5-cos | 0.287 | −0.027 |
| grounded diffusion (h_s) | 0.294 | −0.020 |
| sym4 | 0.309 | −0.005 |
| walk (hit_prob fwd/rev) | 0.313 | −0.000 |
| μ features | 0.312 | −0.001 |
...
exposed-split bootstrap this attribution is suggestive, not established.
...
e5 + graph features with a thin blend, ahead on the descriptive mean (+0.0198) and on 5/5 exposed
splits on the final operator — 4/5 with seed 0 negative on the pre-fallback operator, so
split-level signs are operator-sensitive; prospective test pending under #3875 — but the claimed WIN is retracted: on the correct Dirichlet operator with honest statistics, the exposed-split
bootstrap is null (blend − e5 = +0.023 [−0.028, +0.082]; −0.003 pre-fallback) and the diffusion attribution is

...
def train(args):
    ...
    for step in range(args.steps):
        ...
        if trans_rows and not args.sym_only:
            ...
            if not args.transitive_homo:                       # heteroscedastic (DEFAULT): per-pair scale from chain var
                ...
                s_eff = args.transitive_scale                 # homoscedastic global s (opt-in approximation)
...
def main():
    ...
    ap.add_argument("--transitive-homo", action="store_true", help="force HOMOSCEDASTIC global scale s (the "
                    "approximation). DEFAULT is heteroscedastic — per-pair s_pair = s/√(1+V) from the "
                    "product-propagated chain variance V: more principled, ~free (V is precomputed in the "
                    "triples — no extra forwards), and neutral-to-better. DESIGN §'loss over the DISTRIBUTION'")

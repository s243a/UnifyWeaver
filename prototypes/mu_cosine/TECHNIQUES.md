...
| 2c. **pearltrees** pairs | *(harvest)* `.local/…/fetch_pearltrees_tree.py` → `parse_pearltrees.py` / `gen_pearltrees_pairs.py` | the **harvester is private (`.local`, cookie session)** — see its `.local` skill; the committed **ref-based** tools (`parse_pearltrees.py` DB→typed edges + `SKILL_understand_pearltrees.md`) interpret the harvested data. Pearltrees is the **bridge-rich** corpus (PagePearl `url`s → enwiki) |
| 2d. **mindmap** (human corpus) | `parse_smmx.py` (+ `SKILL_understand_smmx.md`) | parse a SimpleMind `.smmx` into typed edges (`subtopic`/`see_also`/`super_category`/`cloudmapref`/`assoc`), Pearltrees slug identity + enwiki anchors. The SKILL doc lets Haiku read a map directly or via the tool. corpus=mindmap, judge=human |

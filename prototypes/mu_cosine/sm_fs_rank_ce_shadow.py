#!/usr/bin/env python3
"""SHADOW: listwise cross-entropy ranking arm (LINEAGE_RANK) — the owner's framing:
...
def run():
    ...
    for arm in ("rank_ce", "rank_ce_plus_graded"):
        for f in range(5):
            ...
            for step in range(STEPS):
                if arm == "rank_ce_plus_graded" and step % 2 == 1:
    ...
    print(json.dumps({a: float(np.mean(list(v.values()))) for a, v in res.items()} |
                     {"comparators": {"graded_mse": 0.280, "e5_frozen": 0.573}}, indent=1),
          flush=True)

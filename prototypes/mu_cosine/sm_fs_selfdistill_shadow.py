#!/usr/bin/env python3
"""SHADOW: STEP-B SELF-DISTILLATION — rank-CE where the label is e5's OWN top-1 (Kimi test: if the model cannot climb toward 0.573 even when trained to copy e5 exactly, the bottleneck is architecture/optimization, not data or information) — the owner's framing:
    ...
    for arm in ("selfdistill_e5",):
        for f in range(5):
            ...
            for step in range(STEPS):
                ...
                label = int(np.argmax(cosv))                # e5's own choice is the target
            ...
            res[arm+"_top1_agreement_with_e5"][f] = float(np.mean(agree))

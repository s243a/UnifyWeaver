...
**File-naming convention.** Each round's scored set is committed as
`mu_pairs_scored_<round>_<yymmdd-HHMMSS>.tsv`, where `<round>` is the topic tag (`eng`, `enwiki`,
`mathfields`, `stats`, `sysinfo`, `cyber`, …) and `<yymmdd-HHMMSS>` is the round's **first-commit
timestamp rendered in Mountain Time** (`America/Denver`; DST-aware, so summer rounds are MDT/−0600). The
timestamp makes same-day rounds distinguishable and chronologically sortable in a plain `ls`. To recover or
mint a timestamp for an existing file:
`TZ='America/Denver' git log --diff-filter=A --format='%ad' --date=format-local:'%y%m%d-%H%M%S' -- <file> | tail -1`
(use `--follow` once the file has been renamed). The unscored *candidate* files (`mu_pairs_<round>.tsv`) and
the derived `*_cumulative`/`*_prior` sets stay **un-timestamped** — they are gitignored and regenerable, so
only the durable bought-label sets carry a date. After adding a round, list it in `make_cumulative.sh`'s
`ROUNDS` array (by its full timestamped name) so it folds into the replay/dedup cumulative.

Each round's bought labels are committed as `mu_pairs_scored_<round>_<yymmdd-HHMMSS>.tsv` (one source of truth per round;

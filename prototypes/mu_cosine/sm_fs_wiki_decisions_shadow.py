...
STEPS, EVAL_AT = 60000, (20000, 40000, 60000)
...
def run():
    ...
    opt = torch.optim.Adam(head.parameters(), lr=5e-5)
    ...
    for step in range(1, STEPS + 1):
        ...
        if step in EVAL_AT:
            ...
            res[f"wiki_held_step{step}"] = m
            print(f"[wiki] step {step}: held MRR {m:.4f} (floor {floor:.4f}, "
    ...
    res["smfs_zeroshot_wikidelta"] = full_catalog_mrr(head, SQ, SP, sq_ids, sd_col,
                                                      scat_ids, dev)

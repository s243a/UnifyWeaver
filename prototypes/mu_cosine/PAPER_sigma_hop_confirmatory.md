...
> **Status: model-generated draft, repo-internal.** This manuscript was drafted by AI assistants (Codex scaffold,
> 2026-07-07; completed by Claude, 2026-07-08) from the committed pre-registration, runner code, and confirmatory
> report, under the direction of the repository owner. It is kept as the consolidated record of the `Sigma(hop)`
> arc and is **not** being submitted to arXiv or a journal: the project has since moved to the Product-Kalman /
> correlated-PoE direction (`DESIGN_product_kalman_poe.md`), which subsumes this result as its predicted-error
> ingredient. Read it as documentation, not as a peer-reviewed claim.

*Draft, 2026-07-08 (scaffold 2026-07-07). This draft consolidates the exploratory `Sigma(hop)` finding, the
...
## Summary Table: Exploratory vs Confirmatory

| | exploratory (100k_cats) | confirmatory (Behavior slice) |
|---|---|---|
| corpus | original `100k_cats` graph | `enwiki_cats_correct` LMDB, root `Behavior` |
| node overlap with exploratory | — | **0** category titles (verified) |
| pairs (hops 1..5) | ~250 | 250 (50/hop exactly) |
| judge | gpt-5.5-low | gpt-5.5-low (same prompt family) |
| splits | 40 descendant-disjoint | 40 descendant-disjoint |
| specification chosen | **after** iterative exploration | **frozen before** the corpus existed |
| mean held-out NLL gain | +0.094 | **+0.060** |
| hop-shuffle permutation p | 0.001 (K=1000) | **0.000999 (K=1000, floor)** |
| positive splits | — | 33/40 |
| status | post-exploratory | **pre-registered, confirmed** |

The confirmatory gain (+0.060) is smaller than the exploratory (+0.094) — the expected post-exploratory
attenuation — but the frozen decision rule required only `gain > 0` at `p < 0.01`, which it passed at the
permutation floor.

## Figures

*(Generated by `make_sigma_hop_paper_figures.py` from the completed run artifacts. The label-geometry figures are
descriptive views of the RAW fuzzy labels per hop — the confirmatory statistic itself is computed on mean-model
residuals by `sigma_hop_confirmatory.py` and is not re-derived here.)*

**Figure 1 — confirmatory split gains.** Distribution of the 40 descendant-disjoint per-split held-out NLL gains
(constant-Σ − Σ(hop)); 33/40 positive, mean +0.060, far beyond the hop-shuffle null 95th percentile (+0.0005).

![Figure 1](figures/sigma_hop/fig1_split_gains.png)

**Figure 2 — raw-label covariance geometry vs hop, both corpora.** Per-hop σ_D, σ_S, and ρ(D,S) of the raw labels.
The right panel is the notable one: **the monotone ρ(hop) trend replicates qualitatively across the two
independent, non-overlapping corpora** (exploratory −0.83 → +0.25; fresh −0.65 → +0.59) — directional and
symmetric labels *compete* at low hop and *co-occur* at depth in both. The σ panels differ more between corpora,
consistent with the confirmatory claim being about the covariance signal as a whole rather than one universal curve.

![Figure 2](figures/sigma_hop/fig2_geometry_vs_hop.png)

**Figure 3 — fresh-slice (D,S) scatter with 2σ covariance ellipses per hop.** The uncertainty ellipse visibly
rotates with hop: anti-correlated tilt at h=1 (strong directional labels, weak symmetric), near-isotropic by h=3,
positively-tilted by h=5 (weak, co-occurring labels). This is the geometry a constant covariance must average over
and `Sigma(hop)` fits.

![Figure 3](figures/sigma_hop/fig3_fresh_ellipses.png)

*Not regenerated here:* the hop-shuffle null-distribution histogram (the runner records the null mean/95%ile and p
in the result JSON but not the 1000 raw null draws; regenerable by re-running `sigma_hop_confirmatory.py` with the
recorded seeds) and the candidate-slice selection manifest appendix (recorded in
`/tmp/mu_data/sigma_hop_fresh_manifest.json`, including the skipped `Academic_disciplines` candidate).
...
## Disposition (decided 2026-07-08)

This draft stays **in the repository** as the consolidated record of the `Sigma(hop)` arc; it is **not** being
submitted to arXiv or a journal. Rationale: the Product-Kalman / correlated-PoE direction
(`DESIGN_product_kalman_poe.md`) is the better framing of the same problem — `Sigma(hop)` survives inside it as the
predicted-error model `V(hop)` that sets the update gain, so the standalone result is an ingredient, not the story.

If this is ever revisited for submission, the scaffold's original checklist still applies: archive the `/tmp`
artifacts durably (hashes above); confirm the git ordering (preregistration commit `638825558` precedes the
confirmatory run); pin or caveat the judge version (`gpt-5.5-low` is a run label; the hashed raw-response file is
the judge-output record); regenerate the null-distribution figure from the recorded seeds; run an independent-judge
audit sample or keep single-judge framed as a limitation; and decide whether SimpleMind/Pearltrees cross-corpus
tests belong here or in an extension.
